...
//debug_var();
	...
	if ($_SESSION["statut"] == "cpe") {
		if (isset($vers_absence)) {
			$cgt_RA = mysqli_query($GLOBALS["mysqli"], "UPDATE absences_rb SET retard_absence = 'A' WHERE id = '".$vers_absence."'");
		}
		else if (isset($vers_retard)) {
			$cgt_RA = mysqli_query($GLOBALS["mysqli"], "UPDATE absences_rb SET retard_absence = 'R' WHERE id = '".$vers_retard."'");
			$query_recup = mysqli_query($GLOBALS["mysqli"], "SELECT * FROM absences_rb WHERE id = '".$vers_retard."'");
			...
			$query_abs_eleve = @mysqli_query($GLOBALS["mysqli"], $sql_abs_eleve);
			...
			if ($test === 1) {
				// GEES n'a trouvé qu'une seule réponse, on peut donc l'effacer
...
if (isset($choix_creneau)) {
	if (is_numeric($choix_creneau)) {
		...
		$creneaux = mysqli_fetch_array(mysqli_query($GLOBALS["mysqli"], "SELECT heuredebut_definie_periode, heurefin_definie_periode FROM ".$table_ab." WHERE id_definie_periode = '".$choix_creneau."'"));
	...
	$req = mysqli_query($GLOBALS["mysqli"], $sql) OR trigger_error('Impossible de lister les absents.', E_USER_ERROR);
...
for($i = 0; $i < $nbre_rep; $i++) {
	...
	if ($rep_absences[$i]["eleve_id"] != "appel") {
		...
		$rep_nom = mysqli_fetch_array(mysqli_query($GLOBALS["mysqli"], "SELECT nom, prenom, regime FROM eleves e, j_eleves_regime jer
		...
		$req_classe = mysqli_fetch_array(mysqli_query($GLOBALS["mysqli"], "SELECT id_classe FROM j_eleves_classes WHERE login = '".$rep_absences[$i]["eleve_id"]."' AND periode = '".$_periode."'"));
		$rep_classe = mysqli_fetch_array(mysqli_query($GLOBALS["mysqli"], "SELECT classe FROM classes WHERE id = '".$req_classe[0]."'"));
        if ($rep_classe === false) {
            $rep_classe = array();
        }
                // On compte aussi le nombre de classes concernées par cet enseignement
                $query_classe1 = (mysqli_query($GLOBALS["mysqli"], "SELECT id_classe FROM j_groupes_classes WHERE id_groupe = '".$rep_absences[$i]["groupe_id"]."'"));
                $req_classe1 = array();
		...
		$verif_aid = explode("|", $rep_absences[$i]["groupe_id"]);
                if ($verif_aid[0] == "AID"){
                  ...
                  $queryb_classe1 = (mysqli_query($GLOBALS["mysqli"], "SELECT classe FROM classes WHERE id IN ('".implode("','", $req_classe1)."')"));
                  $rep_classe1 = array();
		...
		if ($verif_aid[0] == "AID") {
			...
			$query_classe = (mysqli_query($GLOBALS["mysqli"], "SELECT id_classe FROM j_groupes_classes WHERE id_groupe = '".$rep_absences[$i]["groupe_id"]."'"));
                        $req_classe = array();
                        while ($row = mysqli_fetch_array($query_classe,  MYSQLI_NUM)) {
                          $req_classe[] = $row[0];
                        }
			$queryb_classe = (mysqli_query($GLOBALS["mysqli"], "SELECT classe FROM classes WHERE id IN ('".implode("','", $req_classe)."')"));
                        $rep_classe = array();
	...
	if ($rep_absences[$i]["eleve_id"] == "appel") {
		...
		$rep_matiere = mysqli_fetch_array(mysqli_query($GLOBALS["mysqli"], "SELECT description FROM groupes WHERE id = '".$rep_absences[$i]["groupe_id"]."'"));
...
<?php
	...
	if (date("w") == getSettingValue("creneau_different")) {
		$req_creneaux = mysqli_query($GLOBALS["mysqli"], "SELECT id_definie_periode, nom_definie_periode, heuredebut_definie_periode, heurefin_definie_periode FROM edt_creneaux_bis WHERE type_creneaux != 'pause' ORDER BY heuredebut_definie_periode");
	}
	else {
		$req_creneaux = mysqli_query($GLOBALS["mysqli"], "SELECT id_definie_periode, nom_definie_periode, heuredebut_definie_periode, heurefin_definie_periode FROM edt_creneaux WHERE type_creneaux != 'pause' ORDER BY heuredebut_definie_periode");
	}
