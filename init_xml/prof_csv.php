...
if (!isset($step1)) {
	...
	if ($flag != 0){
		...
		echo "<li>Les professeurs de l'année passée présents dans la base GEES et non présents dans la base CSV de cette année ne sont pas effacés de la base GEES mais simplement déclarés \"inactifs\".</li>";
		echo "</ul>";
	...
	if(mb_strtoupper($dbf_file['name']) == "F_WIND.CSV") {
		...
		echo "<a href='".$_SERVER['PHP_SELF']."?a=a".add_token_in_url()."'>Cliquer ici </a> pour recommencer !</center></p>";
			...
			if ($nb_reg_no != 0) {
				...
				connexion à GEES est son NUMEN.</li>
				<li>Si le NUMEM n'est pas disponible dans le fichier F_wind.csv, GEES génère aléatoirement
				...
				*/
				...
				<ul>
				<li>Lorsqu'un nouveau professeur est inséré dans la base GEES, son mot de passe lors de la première
				connexion à GEES est celui inscrit dans le F_wind.csv.</li>
				<li>Si le mot de passe n'est pas disponible dans le fichier F_wind.csv, GEES génère aléatoirement
				un mot de passe.</li></ul>";
