...
if (isset($is_posted) and ($is_posted == 'debut')) {
    ...
    echo "<form enctype=\"multipart/form-data\" action=\"export_csv_aid.php\" method=post name=formulaire2>";
    echo add_token_field();
    echo "<p>--&gt; Vous voulez laisser <b>GEES</b> définir un identifiant unique pour chaque $nom_generique_aid .";
    echo "<INPUT TYPE=SUBMIT value = 'Valider' /></p>";
...
//*************************************************************************************************
// Début de la procédure dans laquelle on laisse GEES définir un identifiant unique pour chaque AID
//*************************************************************************************************
        ...
        if(!$fp) {
            ...
            while(!feof($fp)) {
                ...
                if ($num == 2) {
                    ...
                    if ($test == 0) {
                        $erreur = 'yes';
                        echo "<p><font color='red'>Erreur dans le fichier à la ligne $row : $data[0] ne correspond à aucun identifiant GEES.</font></p>";
                    }
                }
            }
            fclose($fp);
            ...
            if ($erreur == 'no') {
                ...
                if ($erreur_reg == 'no') {
                    ...
                    echo "<p>Si un(e) $nom_generique_aid existe déjà dans la base GEES, seule une mise à jour des données sera effectuée conformément aux données figurant dans le fichier csv</p>";
                    ...
                    while ($i < $nb_aid) {
                        ...
                        if ($nb_test == 0) {
                            ...
                            $mess = "<font color='green'>Cette activité n'existe pas dans GEES.</font>";

                        } else {

                            $mess = "<font color='blue'>Cette activité existe déjà dans GEES.</font>";
...
if (isset($is_posted) and ($is_posted == 'sans_id_etape_4')) {
    ...
    while ($i < $nb_aid) {
        ...
        if ($nb_test == 0) {
            ...
            $mess = "<font color='blue'>Pas d'enregistrement : cette acticité existait déjà dans GEES !</font>";
...
// Fin de la procédure dans laquelle on laisse GEES définir un identifiant unique pour chaque AID
