...
if($temoin==1){
	...
	elseif($mode==2){
		...
		if(!isset($confirmer)){
			echo "<p><b>ATTENTION:</b> Le mode sans SCONET ne permet pas de mises à jour en cours d'année.<br />\nCela signifie que les corrections effectuées sur votre logiciel de gestion des élèves et responsables (<em>changements d'adresses, corrections,...</em>) ne pourront pas être automatiquement importées dans GEES.<br />Vous aurez donc une double-saisie à effectuer pour gérer ces mises à jour.<br />\nLe mode avec SCONET, lui, permettrait d'importer les corrections en cours d'année.</p>\n";
							...
							while($lig2=mysqli_fetch_object($res2)){
								...
								if(mysqli_num_rows($test)>0){
									...
									if($lig2->nom1!=''){
										...
										$sql="INSERT INTO resp_adr SET adr1='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->adr1)."',adr2='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->adr1_comp)."',cp='$lig2->cp1',commune='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->commune1)."',adr_id='$adr_id'";
										//echo "$sql<br />\n";
									...
									if($lig2->nom2!=''){
										...
										if(($lig2->adr2!=$lig2->adr1)||($lig2->adr2_comp!=$lig2->adr1_comp)||($lig2->cp2!=$lig2->cp1)||($lig2->commune2!=$lig2->commune1)){
											if(($lig2->adr2!='')||($lig2->adr2_comp!='')||($lig2->cp2!='')||($lig2->commune2!='')){
												...
												$sql="INSERT INTO resp_adr SET adr1='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->adr2)."',adr2='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->adr2_comp)."',cp='$lig2->cp2',commune='".mysqli_real_escape_string($GLOBALS["mysqli"], $lig2->commune2)."',adr_id='$adr_id'";
												//echo "$sql<br />\n";
												$res_insert3=mysqli_query($GLOBALS["mysqli"], $sql);
												if(!$res_insert3){
				...
				if(!$fp){
					...
					*/
					...
					if ($nb_reg_no3 != 0) {
						...
						echo "<p>L'importation des personnes (<em>responsables</em>) dans la base GEES a été effectuée avec succès (<em>".$nb_record3." enregistrements au total</em>).</p>\n";
					}
				...
				if(!$fp){
					...
					*/
					...
					if ($nb_reg_no1 != 0) {
						...
						echo "<p>L'importation des relations eleves/responsables dans la base GEES a été effectuée avec succès (<em>".$nb_record1." enregistrements au total</em>).</p>\n";
					}
				...
				if(!$fp){
					...
					*/
					...
					if ($nb_reg_no2 != 0) {
						...
						echo "<p>L'importation des adresses de responsables dans la base GEES a été effectuée avec succès (<em>".$nb_record2." enregistrements au total</em>).</p>\n";
					}
					...
					if(count($tab_elenoet_non_trouves)>0){
						...
						echo "<p>Le fichier 'eleves.csv' fourni contenait des ELENOET d'élèves non présents dans la table 'eleves' de votre base GEES.<br />Ces nouveaux élèves inscrits dans Sconet n'ont pas été créés.<br />Seule la conversion des données existantes a été effectuée.<br />Vous pourrez procéder à un nouvel <a href='maj_import.php'>import par mise à jour</a> pour créer ces élèves.</p>\n";
						echo "<p><br /></p>\n";
