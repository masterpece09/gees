...
if(($_SESSION['statut']=='scolarite')&&(!getSettingAOui('GeesAccesMajSconetScol'))) {
...
if(!isset($step)) {
	...
	if($suhosin_post_max_totalname_length!='') {
		echo "<p class='color:red'>Le module suhosin est activé.<br />\nUn paramétrage trop restrictif de ce module peut perturber le fonctionnement de Gees, particulièrement dans les pages comportant de nombreux champs de formulaire.<br />Cela peut empêcher le bon fonctionnement de la Mise à jour d'après Sconet.</p>\n";
	}
	...
	if ($gepiSettings['unzipped_max_filesize']>=0) {
		echo "<p style=\"font-size:small; color: red;\"><i>REMARQUE&nbsp;:</i> Vous pouvez fournir à Gees le fichier compressé issu directement de SCONET. (Ex : ElevesAvecAdresses.zip)</p>";
	}
...
sinon, Gees le considère comme \"<em>établissement d'origine</em>\".";
				...
				if(in_array($eleves[$i]['eleve_id'],$tab_ele_id)) {
					...
					*/
					...
					if(mysqli_num_rows($test_ele_classe)>0) {
						...
						$info_action_titre="Incohérence Sconet/Gees pour ".remplace_accents(stripslashes($lig_ele->nom)." ".stripslashes($lig_ele->prenom))." ($lig_ele->login)";

						$info_action_texte="L'élève <a href='eleves/modify_eleve.php?eleve_login=$lig_ele->login'>".remplace_accents(stripslashes($lig_ele->nom)." ".stripslashes($lig_ele->prenom))."</a> est inscrit dans une classe dans Gees, alors qu'il n'y est plus dans Sconet.<br />Il faut soit le désinscrire pour les périodes à venir dans Gees, soit le réinscrire dans une classe dans Sconet (<em>selon l'endroit où l'erreur se situe</em>).";
	...
	setTimeout(\"test_stop('$suite')\",3000);
		...
		case "2b":
			...
			if(mysqli_num_rows($res)==0) {
				...
				echo "<p>La date de sortie de l'établissement notés dans Sconet est enregistrée dans GEES.</p>\n";
...
// 20120919
				...
				if((mysqli_num_rows($test)>0)||($temoin_chgt_ancien_etab=="y")||($temoin_init_date_entree=="y")||($temoin_diff_mail_compte_vs_sconet=="y")) {
					...
					if((!isset($tab_ele_id_diff))||(!in_array($tab_ele_id[$i], $tab_ele_id_diff))) {
						// Contrôler si une date de sortie est enregistrée dans Gees et pas dans Sconet:
						...
						if(mysqli_num_rows($test)>0){
							...
							//echo "<input type='hidden' name='tab_ele_id_diff[]' value='".$tab_ele_id[$i]."' />\n";
							$sql="UPDATE tempo4 SET col3='modif' WHERE col1='maj_sconet_eleves' AND col2='$tab_ele_id[$i]';";
							if($tab_ele_id[$i]==$eleve_id_debug) {echo "Date de sortie dans Gees, mais pas dans sconet.<br />";echo "$sql<br />\n";}
							$update=mysqli_query($GLOBALS["mysqli"], $sql);
							//echo "<br />\n";
							// Pour le cas où on est dans la dernière tranche:
							$tab_ele_id_diff[]=$tab_ele_id[$i];
				...
				for($i=0;$i<count($tab_ele_id);$i++){
					...
					if(!in_array($tab_ele_id[$i],$tab_ele_id_diff)) {
						...
						if(mysqli_num_rows($test_clas1)==0) {
							// L'élève n'est dans aucune classe dans Gees, mais inscrit dans une classe dans le XML uploadé.
							if($cpt==0){
								echo "<p>Une ou des différences ont été trouvées dans la tranche étudiée à cette phase.";
								echo "<br />\n";
								echo "En voici le(s) ELE_ID: ";
							...
							if(($auth_sso=='lcs')||($gepi_non_plugin_lcs_mais_recherche_ldap)) {
								...
								if(!check_format_login($mode_generation_login_eleve)) {
									...
									if(mysqli_num_rows($test_ia)==0) {
										enregistre_infos_actions("Format des logins générés","Le format des logins générés par Gees pour les différentes catégories d'utilisateurs doit être contrôlé et revalidé dans la page <a href='./gestion/param_gen.php#format_login_pers'>Configuration générale</a>",array("administrateur"),'statut');
									}
								}
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	function modif_case(rang,type,statut){
		...
		case "9":
			...
			//echo "<input type='hidden' name='is_posted' value='yes' />\n";
			if ($gepiSettings['unzipped_max_filesize']>=0) {
				echo "<p style=\"font-size:small; color: red;\"><i>REMARQUE&nbsp;:</i> Vous pouvez fournir à Gees le fichier compressé issu directement de SCONET. (Ex : ResponsablesAvecAdresses.zip)</p>";
			}
					...
					foreach ($objet_personnes->children() as $personne) {
						...
						*/
						...
						if ($nb_err != 0) {
							...
							echo "<p>L'importation des personnes (responsables) dans la base GEES a été effectuée avec succès (<em>".$stat." enregistrements au total</em>).</p>\n";
			...
			*/
				...
				*/
				...
				if ($nb_err!=0) {
					...
					echo "<p>L'importation des relations eleves/responsables dans la base GEES a été effectuée avec succès (<em>".$stat." enregistrements au total</em>).</p>\n";
		...
		case "12":
			...
			*/
				...
				if ($nb_err != 0) {
					...
					echo "<p>L'importation des adresses de responsables dans la base GEES a été effectuée avec succès (<em>".$stat." enregistrements au total</em>).</p>\n";
	...
	setTimeout(\"test_stop('13a')\",3000);
		...
		case "13a":
			...
			if(mysqli_num_rows($test)==0) {
				...
				echo "<th title='Responsabilité actuellement présente dans Gees'>Base</th>\n";
					...
					if(mysqli_num_rows($res_rp)>0) {
						...
						while($lig_rp=mysqli_fetch_object($res_rp)) {
							...
							*/
							...
							if($chaine_ele!="") {
								echo "<span title='Responsabilité actuellement présente dans Gees'>";
								echo $chaine_ele;
								echo "</span>";
							}
							echo "</td>\n";
...
";
				...
				echo "<ul>
<li>Lors du dédoublonnage dans Sconet, il peut arriver que le responsable conservé (pers_id) ne soit pas celui qui a été pris en compte dans Gees pour la distribution des comptes d'utilisateurs aux parents.<br />Il faut alors recoller les morceaux dans Gees.</li>
<li>Si un seul enregistrement subsiste dans Sconet/Siècle pour un responsable, il est recommandé de conserver dans Gees ce responsable et de lui affecter l'ancien login si le responsable disposait d'un compte pour se connecter dans Gees.</li>
				...
				while($lig1=mysqli_fetch_object($test)){
					...
					if(mysqli_num_rows($res_pers1)==0){
						...
						if(mysqli_num_rows($res_ele)==0) {
							...
							$ligne_parent.="<span title='Responsabilité présente dans la base Gees'>";
...
// A VERIFIER: La liste des suppression est incomplete (20120331)
		...
		case "16a":
							...
							if(mysqli_num_rows($res3)>0){
								while($lig3=mysqli_fetch_object($res3)) {
									if(!in_array($lig3->pers_id, $tab_pers_id_affiche)) {
										...
										$res_e=mysqli_query($GLOBALS["mysqli"], $sql);
										if(mysqli_num_rows($res_e)>0) {
											$chaine.="<span title='Responsabilité actuellement présente dans Gees'>";
											$cpt_ele=0;
											while($lig_e=mysqli_fetch_object($res_e)) {
												if($cpt_ele>0) {$chaine.="<br />";}
												$chaine.=$lig_e->nom." ".$lig_e->prenom;
			...
			if($chaine!="") {
				...
				<li>Si les responsables n'ont pas de compte d'utilisateur pour se connecter dans Gees, cette phase ne présente pas franchement d'intérêt.<br />L'objectif est ici d'éviter de changer le responsable qui dispose déjà d'un compte d'utilisateur.<br />Un nouveau login pourrait sinon être généré.</li>
				</ul>\n";
...
<input type='button' id='input_button' value='Valider' style='display:none;' onclick=\"check_champ_coche()\" /></p>\n";
				...
				//for($i=0;$i<count($pers_modif);$i++){
					...
					// Témoin pour permettre de ne pas afficher la ligne si les adresses de deux responsables associés sont identiques mais avec des adr_id différents dans Sconet alors que la correction (fusion des adr_id) a été effectuée dans Gees.
