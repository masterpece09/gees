					...
					if(!$update) {
						...
						if($mode=="") {
							...
							if(($envoi_mail_actif=='y')&&($mail_dest!="")) {
								...
								$subject = "[GEES]: Remplacement attribué à ".$designation_user;
...
if((isset($_GET['annuler_remplacement']))) {
	...
	if(!$res) {
		...
		if(($envoi_mail_actif=='y')&&($mail_dest!="")) {
			...
			$designation_user=civ_nom_prenom($login_user);
			$subject = "[GEES]: Remplacement annulé";
			$texte_mail="Bonjour ".$designation_user.",
...
if($mode=="") {
	...
	echo "<p>Si des remplacements de cours (<em>dans le passé</em>) ont eu lieu sans pour autant être validés dans Gees, vous pouvez les attribuer/valider maintenant à des fins de statistiques/totaux dans le cas où des rémunérations de remplacements sont prévues.</p>";
