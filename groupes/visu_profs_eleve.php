...
if (
	($_SESSION['statut'] == "responsable" AND getSettingValue("GeesAccesEquipePedaParent") != "yes") OR
	($_SESSION['statut'] == "eleve" AND getSettingValue("GeesAccesEquipePedaEleve") != "yes") OR
		...
		if(mysqli_num_rows($req)>0) {
			...
			if ($cpe->email!="" AND $cpe->show_email == "yes" AND (
				($_SESSION['statut'] == "responsable" AND
						(getSettingValue("GeesAccesEquipePedaEmailParent") == "yes" OR
						getSettingValue("GeesAccesCpePPEmailParent") == "yes"))
				OR
				($_SESSION['statut'] == "eleve" AND
					(getSettingValue("GeesAccesEquipePedaEmailEleve") == "yes" OR
					getSettingValue("GeesAccesEquipePedaEmailEleve") == "yes")
					)
				)){
				echo "<a href='mailto:".$cpe->email."?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] eleve : ".$prenom_eleve . " ".$nom_eleve)."'>".affiche_utilisateur($cpe->cpe_login,$id_classe)."</a>";
			} else {
		...
		while ($groupe = mysqli_fetch_object($groupes)) {
		    ...
		    while($lig_prof=mysqli_fetch_object($result_prof)){
				...
				if($lig_prof->email!="" AND $lig_prof->show_email == "yes" AND
					(($_SESSION['statut'] == "responsable" AND
						(getSettingValue("GeesAccesEquipePedaEmailParent") == "yes"
							OR
						 (getSettingValue("GeesAccesCpePPEmailParent") == "yes" AND mysqli_num_rows($res_pp)>0)
						 )
		    		) OR (
					  $_SESSION['statut'] == "eleve" AND
						(getSettingValue("GeesAccesEquipePedaEmailEleve") == "yes"
							OR
						 (getSettingValue("GeesAccesCpePPEmailEleve") == "yes" AND mysqli_num_rows($res_pp)>0)
						 )
					)
					)){
		            echo "<a href='mailto:$lig_prof->email?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] eleve : ".$prenom_eleve . " " . $nom_eleve)."'>".affiche_utilisateur($lig_prof->login,$id_classe)."</a>";
		        }
		        else{
