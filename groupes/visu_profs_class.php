...
if((isset($id_classe))&&(is_numeric($id_classe))) {
	...
	if(($_SESSION['statut']=='professeur')&&(getSettingValue("GeesAccesVisuToutesEquipProf")!="yes")){
	...
	if((getSettingValue("GeesAccesVisuToutesEquipCpe") == "yes") AND $_SESSION['statut']=='cpe'){
	...
	if($acces_classe=="y") {
		...
		if(mysqli_num_rows($result_cpe)>0){
			...
			while($lig_cpe=mysqli_fetch_object($result_cpe)) {
				...
				if($lig_cpe->email!=""){
					$tab_enseignements[$cpt]['prof'][$cpt2]['designation_prof_mailto']="<a href='mailto:$lig_cpe->email?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] classe=".$classe['classe'])."' title=\"Envoyer un mail\">".my_strtoupper($lig_cpe->nom)." ".casse_mot($lig_cpe->prenom,'majf2')."</a>";
		...
		while($lig_grp=mysqli_fetch_object($result_grp)){
			...
			while($lig_prof=mysqli_fetch_object($result_prof)){
				...
				if($lig_prof->email!=""){
					$tab_enseignements[$cpt]['prof'][$cpt2]['designation_prof_mailto']="<a href='mailto:$lig_prof->email?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] classe=".$classe['classe'])."' title=\"Envoyer un mail\">".my_strtoupper($lig_prof->nom)." ".casse_mot($lig_prof->prenom,'majf2')."</a>";
...
if((isset($_GET['export_prof_suivi']))&&(isset($export))&&($export=='csv')) {
	...
	if(($_SESSION['statut']=='scolarite')&&(getSettingValue("GeesAccesVisuToutesEquipScol") =="yes")){
		$sql="SELECT DISTINCT c.id,c.classe FROM classes c ORDER BY c.classe";
	}
	if(($_SESSION['statut']=='cpe')&&(getSettingValue("GeesAccesVisuToutesEquipCpe") =="yes")){
		$sql="SELECT DISTINCT c.id,c.classe FROM classes c ORDER BY c.classe";
	}
	if(($_SESSION['statut']=='professeur')&&(getSettingValue("GeesAccesVisuToutesEquipProf") =="yes")){
		$sql="SELECT DISTINCT c.id,c.classe FROM classes c ORDER BY c.classe";
...
if(isset($id_classe)){
	...
	<!--tr-->
		...
		if(count($tabmail)>0){
			...
			echo "<p>Envoyer un <a href='mailto:$chaine_mail?".rawurlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] classe ".$classe['classe'])."'>mail à tous les membres de l'équipe</a>.</p>
<script type='text/javascript'>
	if(document.getElementById('span_mail')) {
		document.getElementById('span_mail').innerHTML=\" <a href='mailto:$chaine_mail?".rawurlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES] classe ".$classe['classe'])."' title='Envoyer un mail à tous les membres de l équipe'><img src='../images/icons/courrier_envoi.png' class='icone16' alt='Mail' /></a>\";
	}
</script>\n";
		...
		echo "</a></td>\n";
				...
				echo "<td>";
				if($lig_cpe->email!=""){
					echo "<a href='mailto:$lig_cpe->email?".urlencode("subject=[GEES] classe=".$classe['classe'])."'>".my_strtoupper($lig_cpe->nom)." ".casse_mot($lig_cpe->prenom,'majf2')."</a>";
					$tabmail[]=$lig_cpe->email;
				}
				else{
		...
		while($lig_grp=mysql_fetch_object($result_grp)){
			...
			while($lig_prof=mysql_fetch_object($result_prof)){
				if($lig_prof->email!=""){
					echo "<a href='mailto:$lig_prof->email?".urlencode("subject=[GEES] classe=".$classe['classe'])."'>".my_strtoupper($lig_prof->nom)." ".casse_mot($lig_prof->prenom,'majf2')."</a>";
					$tabmail[]=$lig_prof->email;
				}
				else{
		...
		if(count($tabmail)>0){
			...
			echo "<p>Envoyer un <a href='mailto:$chaine_mail?".rawurlencode("subject=[GEES] classe ".$classe['classe'])."'>mail à tous les membres de l'équipe</a>.</p>\n";
		}
		*/
	}
}
