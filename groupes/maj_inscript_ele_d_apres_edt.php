...
if((isset($_POST['valider_ec3']))&&(isset($id_groupe))&&(isset($_POST['id_nom_edt']))) {
	...
	if($_POST['id_nom_edt']=="") {
		...
		if((isset($_POST['mode_js']))&&($_POST['mode_js']='y')) {
			echo "Aucun regroupement EDT n'est associé à ce groupe Gees";
			die();
		}
	}
	else {
...
if((isset($_GET['div_suppr_assoc_regroupement']))&&(isset($_GET['id_groupe']))&&(preg_match("/^[0-9]{1,}$/", $_GET['id_groupe']))&&(isset($_GET['id_edt_tempo']))&&(preg_match("/^[0-9]{1,}$/", $_GET['id_edt_tempo']))) {
	...
	$res_grp=mysqli_query($GLOBALS["mysqli"], $sql);
	if(mysqli_num_rows($res_grp)==0) {
		echo "<p style='color:red'>L'identifiant de groupe Gees et celui EDT ne correspondent pas.</p>";
	}
	else {
...
if((isset($_GET['maj_composition_groupe']))&&(isset($_GET['id_groupe']))&&(preg_match("/^[0-9]{1,}$/", $_GET['id_groupe']))&&(isset($_GET['id_edt_tempo']))&&(preg_match("/^[0-9]{1,}$/", $_GET['id_edt_tempo']))&&(isset($_GET['num_periode']))&&(preg_match("/^[0-9]{1,}$/", $_GET['num_periode']))) {
	...
	if(mysqli_num_rows($res_grp)==0) {
		...
		echo "<p style='color:green'>L'identifiant de groupe Gees et celui EDT correspondent.</p>";
		...
		//echo "<br />Temps écoulé ".($ts-$ts0)."<br />";
		if(mysqli_num_rows($res_grp)==0) {
			echo "<p style='color:red'>Aucun regroupement EDT n'est associé au groupe Gees n°".$_GET['id_groupe'].".</p>";
		}
		elseif(mysqli_num_rows($res_grp)>1) {
			echo "<p style='color:red'>Plusieurs regroupements EDT sont associés au groupe Gees n°".$_GET['id_groupe'].".<br />Vous devez commencer par supprimer la ou les associations en trop.<br />Les regroupements associés sont&nbsp;:<br />";
			...
			*/
			...
			if(mysqli_num_rows($res_ele)==0) {
				...
				// Faut-il vider le groupe Gees?
			}
			else {

				// Le groupe Gees actuel:
...
// Si $lig_ele->n_national est vide, il faut tenter d'identifier autrement l'élève (nom, prénom, date de naissance).
						...
						if(isset($tmp_tab['periode'][$num_periode]['id_classe'])) {
							...
							if(!in_array($id_classe, $reg_clazz)) {
								...
								if(mysqli_num_rows($res_per)==0) {
									$inscrire="n";
									echo "<br /><span style='color:red'>La classe de l'élève ".$lig_ele->nom." ".$lig_ele->prenom." n'a pas de période (<em>l'élève ne peut pas être ajouté au groupe Gees</em>)</span>";
									...
									if($lig_per->maxper==count($current_group["periodes"])) {
										...
										$inscrire="n";
										echo "<br /><span style='color:red'>La classe de l'élève ".$lig_ele->nom." ".$lig_ele->prenom." n'a pas le même nombre de périodes que les autres classes du groupe (<em>l'élève ne peut pas être ajouté au groupe Gees</em>)</span>";
							...
							echo "<br /><span style='color:red'>L'élève ".$lig_ele->nom." ".$lig_ele->prenom." n'est dans aucune classe (<em>il ne peut pas être ajouté au groupe Gees</em>)</span>";
				...
				for($loop=0;$loop<count($old_reg_eleves[$num_periode]);$loop++) {
					if(!in_array($old_reg_eleves[$num_periode][$loop], $reg_eleves[$num_periode])) {
						// Un élève est supprimé du groupe Gees
				...
				if($update) {
					...
					if($envoi_mail_actif=='y') {
						...
						if($destinataire_mail!="") {
							$sujet_mail="[GEES] Modification des affectations élèves dans un de vos enseignements";
...
$res=mysqli_query($GLOBALS["mysqli"], $sql);
if(mysqli_num_rows($res)>0) {
	echo " | <a href='".$_SERVER['PHP_SELF']."?action=editer_ec2'> Contrôler les associations groupe Gees/nom de regroupement EDT </a>";
}
...
if($action=="") {
	...
	<fieldset class='fieldset_opacite50'>
		...
		<p style='text-indent:-4em; margin-left:4em; margin-top:1em;'><em>NOTE&nbsp;:</em> Le fichier EXP_ELEVE.xml importé va être traité en identifiant les élèves par leur numéro INE (<em>N_NATIONAL dans le fichier XML et no_gep dans Gees</em>).<br />Les groupes d'élèves EDT auxquels appartient chaque élève sont donnés dans la liste CLASSE où les groupes sont séparés par une virgule (<em>le premier groupe est généralement la classe de l'élève et les suivants désignent des groupes d'élèves</em>).<br />
		Lors de l'import du EXP_Cours.xml d'EDT, les associations groupe_Gees/nom_de_groupe_EDT ont été enregistrées.<br />
...
elseif($action=="comparer") {
	...
	echo "<h2>Différences EDT/Gees sur la période $num_periode</h2>";
...
<a href=\"javascript:afficher_tous_div_regroup('none')\">N'afficher que les regroupements avec différence trouvée</a></p>";
	...
	// Si $lig_ele->n_national est vide, il faut tenter d'identifier autrement l'élève (nom, prénom, date de naissance).
		...
		<thead>
			<tr>
				...
				<th>Enseignement Gees<br />".$tab_info_grp[$current_id_groupe];
	...
	*/
								...
								if($tab_ele_regroupement_edt['id_classe'][$loop]!="") {
									if(!isset($tab_test_association_grp_classe[$tab_ele_regroupement_edt['id_classe'][$loop]])) {
										...
										if(mysqli_num_rows($test_grp_clas)==0) {
											$tab_test_association_grp_classe[$tab_ele_regroupement_edt['id_classe'][$loop]]=" <img src='../images/icons/flag2.gif' class='icone16' alt='ATTENTION' title=\"Cette classe n'est pas associée au groupe Gees.\" />";
										}
										else {
				...
				<td style='vertical-align:top;'>
					<div id='div_suppr_assoc_".$current_id_groupe."_".$current_id_temp."' style='float:right; width:10em;'>
						<a href='#' onclick=\"suppr_assoc_regroupement_edt($current_id_groupe, $current_id_temp);return false;\" title=\"Si l'association entre le regroupement d'élèves EDT et le groupe Gees vous semble erroné, vous pouvez supprimer l'association.\nLors de la prochaine mise à jour de l'emploi du temps dans Gees d'après le EXP_COURS.xml de EDT, une nouvelle association vous sera proposée.\" target='_blank'>Supprimer l'association</a>
					</div>
	...
	*/
					...
					<!--p style='color:red'>AJOUTER UN LIEN ou CHECKBOX pour mettre à jour l'enseignement Gees d'après le regroupement EDT (id_tempo=$current_id_temp et id_groupe=$current_id_groupe)<br />
...
elseif($action=="editer_ec2") {
	...
	echo "<p class='bold'>Voici les associations groupe Gees/nom de regroupement EDT</p>";
...
<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' id='form_envoi_xml' method='post'>
	<fieldset class='fieldset_opacite50'>
		...
		<table class='boireaus boireaus_alt resizable sortable'>
			<thead>
				<tr>
					...
					<th colspan='4'>Groupes Gees associés</th>
				</tr>
				<tr>
					<th>
		...
		while($lig=mysqli_fetch_object($res)) {
			...
			if(mysqli_num_rows($res2)>0) {
				...
				<tr class='white_hover'>
					...
					<td style='text-align:left'><a href='../groupes/edit_group.php?id_groupe=".$lig2->id_groupe."' title=\"Voir l'enseignement Gees dans un nouvel onglet\" target='_blank'>".get_info_grp($lig2->id_groupe)."</a><a name='id_groupe_".$lig2->id_groupe."'></a></td>
		...
		<p style='margin-left:4em; text-indent:-4em; margin-top:1em;'><em>NOTES&nbsp;:</em> Seuls les enseignements Gees, pour lesquels une association avec un regroupement EDT existe, sont proposés pour la mise à jour des affectations d'élèves.<br />
...
elseif((isset($id_groupe))&&($action=="editer_ec3")) {
	...
	if(mysqli_num_rows($res)==0) {
		...
		if(count($tab_assoc)>1) {
			echo "<p style='color:red; text-indent:-6em; margin-left:6em;'>ANOMALIE&nbsp;: Le groupe/enseignement Gees est associé à ".count($tab_assoc)." regroupements EDT (<em>";
