...
<?php
	if(acces("/groupes/maj_inscript_ele_d_apres_edt.php", $_SESSION['statut'])) {
		...
		if(mysqli_num_rows($test)>0) {
			echo "| <a href='../groupes/maj_inscript_ele_d_apres_edt.php' title=\"Si vous avez importé votre emploi du temps depuis le fichier EXP_Cours.xml d'EDT, vous pouvez mettre à jour les inscriptions d'élèves dans les groupes Gees à l'aide de l'export EXP_Eleves.xml d'EDT.\">Màj inscriptions élèves d'après EDT </a>";
		}
	}

	if(acces("/groupes/export_groupes_sconet.php", $_SESSION['statut'])) {
		echo "| <a href='../groupes/export_groupes_sconet.php' title=\"Exporter les groupes Gees vers Sconet.\">Export groupes vers Sconet </a>";
	}
...
if ($nombre_lignes != 0) {
	...
	echo "<p class='grand'>Attention : aucune classe n'a été définie dans la base GEES !</p>\n";
