...
if (isset($id_classe)) {
	...
	if ($_SESSION['statut'] == "professeur" AND getSettingValue("GeesAccesMoyennesProfToutesClasses") != "yes") {
...
if ($_SESSION['statut'] == "professeur" AND getSettingValue("GeesAccesMoyennesProfTousEleves") != "yes" AND getSettingValue("GeesAccesMoyennesProfToutesClasses") != "yes") {
...
//Célestin Récupération des moyenne 15-03-2016
$eleve_moyenne_sexe = affiche_tableau($nb_lignes_tableau, $nb_col, $ligne1, $col, $larg_tab, $bord,0,1,$couleur_alterne);
...
/**
 * Célestin 15-03-2015
 * Affichage des statistiques des moyennes d'une séquence
 */

    //Récupération des sexe des élèves
    for($i=1; $i<count($eleve_moyenne_sexe['eleve'])-3; $i++){
        $eleve_moyenne_sexe['sexe'][$i] = getEleveSexe($eleve_moyenne_sexe['eleve'][$i], $id_classe);
        //echo "<br/>FIN ".$eleve_moyenne_sexe['eleve'][$i]."-".$eleve_moyenne_sexe['sexe'][$i]."<br/>";
    }
    
    $eleve_par_sexe = nombreEleveParSexe($id_classe);
    ...
    $note["[10-21["] = nombreDeNoteEntre($eleve_moyenne_sexe, 10, 21);
    $note['[0-5[']   = nombreDeNoteEntre($eleve_moyenne_sexe, 0, 5);
    $note["[5-7["]   = nombreDeNoteEntre($eleve_moyenne_sexe, 5, 7);
    $note["[7-9["]   = nombreDeNoteEntre($eleve_moyenne_sexe, 7, 9);
    $note["[9-10["]  = nombreDeNoteEntre($eleve_moyenne_sexe, 9, 10);
    $note["[10-12["] = nombreDeNoteEntre($eleve_moyenne_sexe, 10, 12);
    $note["[12-14["] = nombreDeNoteEntre($eleve_moyenne_sexe, 12, 14);
    $note["[14-20]"] = nombreDeNoteEntre($eleve_moyenne_sexe, 14, 21);
    
    //echo "CLASSES-".$id_classe;
    echo "<br/><br/><br/><br/>";
...
//echo var_dump($eleve_moyenne_sexe);

require("../lib/footer.inc.php");

//Célestin
    //Détermine le nombre de notes par sexe sur une plage
    function nombreDeNoteEntre($eleve_moyenne_sexe, $min, $max){
        
        $nbrHomme = 0;
        $nbrFemme = 0;
        for($i=1; $i<count($eleve_moyenne_sexe['eleve'])-3; $i++){
            if($min<= $eleve_moyenne_sexe['moyenne'][$i] && $eleve_moyenne_sexe['moyenne'][$i]< $max ){
                if($eleve_moyenne_sexe["sexe"][$i]=="F"){
                    $nbrFemme ++;
                }else{
                    $nbrHomme++;
                }
        ...
        return array('femme'=>$nbrFemme ,'homme'=>$nbrHomme);
    }
    ...
    //Détermine le sexe d'un élève
    function getEleveSexe($eleve, $currentClasse){
        ...
        $sql = "SELECT `sexe` "
                . "FROM `eleves`, `j_eleves_classes` "
                . "WHERE `id_classe` = :classe AND `j_eleves_classes`.`login` = `eleves`.`login` "
                . "AND CONCAT_WS(' ',`nom`,`prenom`) LIKE :eleve"
                . "";
