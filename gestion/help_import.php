...
--> <B>Classe (fac.)</B> : le nom court d'une classe déjà définie dans la base GEES ou bien le caractère - si l'élève n'est pas affecté à une classe.<br />
...
--> <B><?php echo ucfirst(getSettingValue("gepi_prof_suivi")); ?></B> : l'identifiant d'un <?php echo getSettingValue("gepi_prof_suivi"); ?> déjà défini dans la base GEES ou bien le caractère - si l'élève n'a pas de <?php echo getSettingValue("gepi_prof_suivi"); ?>.<br />
--> <B>Identifiant de l'établissement d'origine </B> : le code RNE identifiant chaque établissement scolaire et déjà défini dans la base GEES, ou bien le caractère - si l'établissement n'est pas connu.<br /></p>

<p class='bold'>IDENTIFIANT</p>
<p>Identifiant de l'élève : il peut s'agir de n'importe quelle suite de caractères et/ou de chiffres sans espace. Si ce format n'est pas respecté, la suite de caractères ??? apparaît à la place de l'identifiant. Les identifiants qui apparaissent en rouge correspondent à des noms d'utilisateur déjà existants dans la base GEES. Les données existantes seront alors écrasées par les données présentes dans le fichier à importer !</p>
<p class='bold'>Nom</p>
...
<p>Prénom de l'élève. Même remarque que pour le nom. Les noms et prénoms qui apparaissent en bleu correspondent à des élèves existant dans la base GEES et portant les mêmes noms et prénoms.</p>
<p class='bold'>Sexe</p>
...
<br />--> le nom court d'une classe déjà définie dans la base GEES
<br />--> ou bien le caractère - si l'élève n'est pas affecté à une classe.
<br />Si la classe n'est pas définie dans la base GEES, celle-ci sera considérée comme erronée.
...
<p>L'identifiant d'un <?php echo getSettingValue("gepi_prof_suivi"); ?> déjà défini dans la base GEES ou bien le caractère - si l'élève n'a pas de <?php echo getSettingValue("gepi_prof_suivi"); ?>.
...
<p>Le code RNE identifiant chaque établissement scolaire et déjà défini dans la base GEES, ou bien le caractère - si l'établissement n'est pas connu.<br /></p>
