...
".add_token_field()."
	...
	<input type=\"file\" size=\"65\" name=\"nomenclature_xml_file\" /></p>\n";
			if ($gepiSettings['unzipped_max_filesize']>=0) {
				echo "	<p style=\"font-size:small; color: red;\"><em>REMARQUE&nbsp;:</em> Vous pouvez fournir à Gees le fichier compressé issu directement de SCONET. (<em>Ex&nbsp;: Nomenclature.zip</em>)</p>";
			}
			echo "
	<input type='checkbox' name='remplacer' id='remplacer' value='y' /><label for='remplacer'> Remplacer les enregistrements déjà présents dans Gees.</label><br />
				...
				if(!$res_copy) {
					...
					*/
					...
					if($nb_matiere_deja>0) {
						echo "<p>$nb_matiere_deja matières déjà présente(s) dans Gees a(ont) été trouvée(s) dans le XML.</p>";
					}
					if($nb_matiere_reg>0) {
					...
					*/
					...
					if($nb_mef_deja>0) {
						echo "<p>$nb_mef_deja MEF déjà présente(s) dans Gees a(ont) été trouvée(s) dans le XML.</p>";
					}
					if($nb_mef_reg>0) {
