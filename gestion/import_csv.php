...
if (!isset($is_posted) or (isset($is_posted) and ($is_posted == 'R')) ) {
    ...
    <p><b>Remarque importante</b> : vous allez importer dans la base GEES des données "élève" à partir d'un fichier au format csv (séparateur point-virgule).<br />
    ...
    echo "--> <B>Classe (fac.)</B> : le nom court d'une classe déjà définie dans la base GEES ou bien le caractère - si l'élève n'est pas affecté à une classe.<br />";
    ...
    echo "--> <B>".ucfirst(getSettingValue("gepi_prof_suivi"))."</B> : l'identifiant d'un ".getSettingValue("gepi_prof_suivi")." déjà défini dans la base GEES ou bien le caractère - si l'élève n'a pas de ".getSettingValue("gepi_prof_suivi").".<br />";
    echo "--> <B>Identifiant de l'établissement d'origine </B> : le code RNE identifiant chaque établissement scolaire et déjà défini dans la base GEES, ou bien le caractère - si l'établissement n'est pas connu.<br /></p>";
} else {
        ...
        if(!$fp) {
            ...
            if ($row > 0) {
                if ($test_login_existant == "oui") {
                    echo "<p>--> Les identifiants qui apparaissent en rouge correspondent à des identifiants déjà existant dans la base GEES. Les données existantes seront donc écrasées par les données présentes dans le fichier à importer !</p>";
                }
                if ($test_nom_prenom_existant == 'yes') {
                    echo "<p>--> Les noms et prénoms qui apparaissent en bleu correspondent à des élèves déjà présents dans la base GEES et portant les mêmes noms et prénoms.
                    <br />Si le nouvel identifiant est différent, un nouvel élève sera crée. Sinon, les données de GEES seront modifiées. </p>";
                }
                if ($valid == '1') {
                    ...
                    echo "<p>AVERTISSEMENT : Les symboles ??? signifient que le champ en question n'est pas valide. L'opération d'importation des données ne peut continuer normalement. Veuillez corriger le fichier à importer ou bien effectuer les opérations nécessaires dans la base GEES !<br /></p>";
                    echo "</FORM>";
                }
            } else {
