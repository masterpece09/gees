...
if (isset($_POST['duree'])) {
    ...
    if (!saveSetting(("duree_conservation_logs"), $_POST['duree'])) {
        ...
        $msg.= "La durée de conservation des connexions a été enregistrée.<br />Le changement sera pris en compte après la prochaine connexion à GEES.<br />";
    }
}
...
if (isset($_POST['GeesResp_obtenir_compte_et_motdepasse'])) {
	check_token();

	if (!saveSetting("GeesResp_obtenir_compte_et_motdepasse", $_POST['GeesResp_obtenir_compte_et_motdepasse'])) {
	...
	if($nb_mode_information==0) {
		...
		if (!saveSetting("GeesResp_obtenir_compte_et_motdepasse", "no")) {
			$msg.= "Il y a eu un problème lors de l'enregistrement du paramètre d'activation/désactivation de la procédure de demande de compte/mot de passe.<br />";
		} else {
			$msg.= "L'enregistrement du paramètre d'activation/désactivation de la procédure de demande de compte/mot de passe a été effectué avec succès.<br />";
		}
...
echo "<input type='radio' name='GeesResp_obtenir_compte_et_motdepasse' value='no' id='GeesResp_obtenir_compte_et_motdepasse_no'";
if (!getSettingAOui("GeesResp_obtenir_compte_et_motdepasse")) echo " checked ";
echo " /> <label for='GeesResp_obtenir_compte_et_motdepasse_no' style='cursor: pointer;'>Désactiver l'accès au formulaire de demande de compte/mot de passe pour les responsables</label>\n";

echo "<br /><input type='radio' name='GeesResp_obtenir_compte_et_motdepasse' value='yes' id='GeesResp_obtenir_compte_et_motdepasse_yes'";
if (getSettingAOui("GeesResp_obtenir_compte_et_motdepasse")) echo " checked ";
echo " /> <label for='GeesResp_obtenir_compte_et_motdepasse_yes' style='cursor: pointer;'>Activer l'accès au formulaire de demande de compte/mot de passe pour les responsables</label></p>\n";
...
//
// Cette option n'est proposée que si les mots de passe sont éditables dans Gees
//
...
echo add_token_field();
echo "<p><b>ATTENTION : </b>En validant le bouton ci-dessous, <b>tous les utilisateurs</b> dont le mot de passe est éditable par Gees (les utilisateurs locaux, ou bien tous les utilisateurs si un accès LDAP en écriture a été configuré) seront amenés à changer leur mot de passe lors de leur prochaine connexion.</p>\n";
...
echo "<form action=\"options_connect.php\" name=\"form_saisie_mail\" method=\"post\">
	...
	<p style='text-indent: -7em;margin-left:7em; color:red; margin-top:1em;'><em>ATTENTION&nbsp;:</em> Si votre Gees est installé en DMZ de votre établissement et que l'accès internet est temporairement coupé (**), le test DNS risque de ralentir l'affichage des pages pour les statuts cochés ci-dessus.<br />
	En cas de coupure de l'accès internet, désactivez le test DNS pour éviter ce problème.<br />
	(**) Les machines de l'établissement conservent un accès à Gees en DMZ, mais seul l'accès de l'intérieur de l'établissement fonctionne alors, sans communication avec l'extérieur.</p>
	</fieldset>
</form>\n";
...
echo "<p><span style='color: red'><strong>Attention !</strong></span> Ne modifiez ces paramètres que si vous savez vraiment ce que vous faites ! Si vous activez l'authentification SSO et que vous ne pouvez plus vous connecter à Gees en administrateur, vous pouvez utiliser la variable \$block_sso dans le fichier /lib/global.inc pour désactiver le SSO et rebasculer en authentification locale. Il est donc vivement recommandé de créer un compte administrateur local (<em>dont le login n'interfèrera pas avec un login SSO</em>) avant d'activer le SSO.</p>\n";
echo "<p>Gees permet d'utiliser plusieurs modes d'authentification en parallèle. Les combinaisons les plus courantes seront une authentification locale avec une authentification LDAP, ou bien une authentification locale et une authentification unique (<em>utilisant un serveur d'authentification distinct</em>).</p>\n";
echo "<p>Le mode d'authentification est explicitement spécifié pour chaque utilisateur dans la base de données de Gees. Assurez-vous que le mode défini correspond effectivement au mode utilisé par l'utilisateur.</p>\n";

echo "<p>Dans le cas d'une authentification externe (<em>LDAP ou SSO</em>), aucun mot de passe n'est stocké dans la base de données de Gees.</p>\n";
echo "<p>Si vous paramétrez un accès LDAP en écriture, les mots de passe des utilisateurs pourront être modifiés directement à travers Gees, même pour les modes LDAP et SSO. L'administrateur pourra également éditer les données de base de l'utilisateur (<em>nom, prénom, email</em>). Lorsque vous activez l'accès LDAP en écriture, assurez-vous que le paramétrage sur le serveur LDAP permet à l'utilisateur de connexion LDAP de modifier les champs login, mot de passe, nom, prénom et email.</p>\n";
...
echo " /> <label for='label_auth_locale' style='cursor: pointer;'>Authentification autonome (sur la base de données de Gees)</label>\n";
...
echo "<p>Si les champs ci-dessous sont renseignés, Gees essaiera systématiquement de mettre à jour les informations de l'utilisateur à partir des attributs transmis par le serveur CAS.</p>";
...
Dans ce cas, il peut être commode de conserver des mots de passe dans la base Gees pour vos utilisateurs<br />
(<em>il n'est cependant pas possible d'assurer une synchronisation des mots de passe entre le SSO (ENT ou autre) et la base Gees</em>).<br />
<input type='checkbox' name='auth_sso_ne_pas_vider_MDP_gepi' value='y' id='auth_sso_ne_pas_vider_MDP_gepi'".(getSettingAOui('auth_sso_ne_pas_vider_MDP_gepi') ? " checked='checked' " : "")." /><label for='auth_sso_ne_pas_vider_MDP_gepi' style='cursor: pointer;'> Ne pas vider les mots de passe dans la base Gees lorsque l'on passe du mode d'authentification 'gepi' au mode 'sso'.</label></p>
...
<input type='checkbox' name='autoriser_sso_password_auth' value='y' id='autoriser_sso_password_auth'".(getSettingAOui('autoriser_sso_password_auth') ? " checked='checked' " : "")." /><label for='autoriser_sso_password_auth' style='cursor: pointer;'> Autoriser l'authentification par (compte;mot de passe) sur la base Gees pour des comptes dont le mode d'authentification est 'sso'.</label><br />
