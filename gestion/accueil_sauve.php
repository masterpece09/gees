...
if ($test_write == 'no') {
    ...
    echo "<p>Vous ne pouvez donc pas accéder aux fonctions de sauvegarde/restauration de GEES.
...
if (isset($action) and ($action == 'restaure_confirm'))  {
	...
	echo "<blockquote>\n";
		...
		//echo "<span style='color:red; text-decoration:blink; font-weight:bolder;'> -> </span>préciser si le fichier à restaurer est codé en UTF8 (sauvegarde GEES >=1.6.0) <input type='radio' name='char_set' value='utf8'  checked='checked'> ou en ISO (sauvegarde GEES <=1.5.5)<input type='radio' name='char_set' value='latin1'>\n";
...
if (isset($action) and ($action == 'restaure'))  {
	...
	if($restauration_old_way=='y') {
		...
		if ($offset!=-1) {
			...
			echo "<div class=\"center\"><p>Restauration terminée en ".$h." h ".$m." min ".$s." s.<br /><br />Votre session GEES n'est plus valide, vous devez vous reconnecter<br /><a href = \"../login.php\">Se connecter</a></p></div>\n";
			require("../lib/footer.inc.php");
			die();
		}
		//===============================================
	...
	function shutdown() {
		...
		echo "<br /><p style='padding-left: 1em;'><a href='../login.php'>Votre session Gees n'est plus valide, vous devez vous reconnecter.</a></p>";
...
value VARCHAR(255) NOT NULL) ENGINE=MyISAM CHARACTER SET utf8 COLLATE utf8_general_ci;";
			...
			if(mysqli_num_rows($res)>0) {
				...
				echo "<div  class=\"center\"><strong>><p>Restauration terminée.<br /><br />Votre session GEES n'est plus valide, vous devez vous reconnecter<br /><a href = \"../login.php\">Se connecter</a></p></div>\n";
		...
		if($succes_etape!="y") {
			...
			if(mysqli_num_rows($res)==0) {
				echo "<div id='div_fin_restauration' class='infobulle_corps' style='position:absolute; top: 200px; left:100px; border:1px solid black; width: 30em;'>\n";
					...
					echo "<p>Restauration terminée en ".$h." h ".$m." min ".$s." s.<br /><br />Votre session GEES n'est plus valide, vous devez vous reconnecter<br /><a href=\"../login.php\">Se connecter</a></p>\n";
					//echo "<p><em>NOTE:</em> J'ai un problème bizarre! Alors que le lien pointe bien vers ../login.php, je me retrouve un dossier plus haut sur un logout.php hors du dossier de Gees si bien que j'obtiens un 404 Not Found???</p>\n";
					echo "</div>\n";
...
if (isset($action) and ($action == 'dump'))  {
    ...
    if (!isset($_GET["duree"])&&is_file($filename)){
        ...
        if ($percent >= 0) {
            ...
            <br/>A la fin de la sauvegarde, Gees vous proposera automatiquement de télécharger le fichier.
...
<p>Deux méthodes de sauvegarde sont disponibles : l'utilisation de la commande système mysqldump ou bien le système intégré à Gees.<br/>
...
<p>
    ...
    </b>pour utiliser la commande système mysqldump lorsque Gees est hébergé sous Windows il faut au préalable copier le fichier "mysqldump.exe" dans le dossier "gestion" de Gees. Ce fichier "mysqldump.exe" se trouve généralement dans le sous-dossier "bin" du dossier d'installation de MySQL.
</p>
<?php
	}
?>
...
if ($n > 0) {
    ...
    echo "<p>Le tableau ci-dessous indique la liste des fichiers de restauration actuellement stockés dans le répertoire \"backup\" à la racine de GEES.</p>\n";
...
echo "<br /><hr />";
echo "<h3 id=\"zip\">Créer une archive (Zip) de dossiers de Gees</h3>\n";
