...
if(!isset($num_periode)) {
	...
	if(!isset($id_classe)) {
		...
		if ((($_SESSION['statut']=="cpe")&&(getSettingValue('GeesAccesAbsTouteClasseCpe')=='yes'))||($_SESSION['statut']!="cpe")) {
					...
					if(isset($eleves[$i]['elenoet'])){
						...
						if(mysqli_num_rows($test)>0){
							...
							elseif(mysqli_num_rows($res1)>1){
								...
								$acces_a_cet_eleve="y";
								if (($_SESSION['statut']=="cpe")&&(getSettingValue('GeesAccesAbsTouteClasseCpe')!='yes')) {
		...
		elseif($etape==2) {
			...
			for($i=0;$i<count($id_classe);$i++){
				...
				if(mysqli_num_rows($test_ver)>0) {
					if((($_SESSION['statut']=="cpe")&&(getSettingValue('GeesAccesAbsTouteClasseCpe')=='yes'))||($_SESSION['statut']=='secours')) {
