	...
	DecocheColonneSelectEleves($i, 0);";
			...
			for($i=$indice_premiere_matiere;$i<=$indice_max_matieres;$i++) {
				...
				if(($tabmatieres[$i][0]!="")&&($tabmatieres[$i]['socle']=='n')) {
					...
					if($temoin_assoc=='n') {
						//echo "<span style='color:red;'>La matière Notanet ".$tabmatieres[$i][0]." n'est associée à aucune matière Gees. Avez-vous correctement effectué l'<a href='select_matieres.php?type_brevet=$extract_mode'>étape 2</a>&nbsp;?</span><br />\n";
						echo "<span style='color:red;'>La matière Notanet ".$tabmatieres[$i][0]." n'est associée à aucune matière Gees.</span><br />\n";
						$cpt_non_assoc++;
					}
				}
			}
