	...
	if(!$res_nettoyage){
			...
			if($tabmatieres[$j][0]!=''){
				//if(($tabmatieres[$j]['socle']=='n') {
					if(isset($id_matiere[$j])){
						if($debug_ajout_matiere=="y") {
							echo "<p>La matière n°$j a une ou des matières Gees sélectionnées&nbsp;:<br />";
						}
						...
						if($debug_ajout_matiere=="y") {
							echo "<p>La matière n°$j n'a pas de matière Gees sélectionnée&nbsp;:<br />";
						}
		...
		if(($j_matiere!='')&&($matiere_a_ajouter!='')&&($mode_matiere_a_ajouter!='')) {
			$sql="INSERT INTO notanet_corresp SET notanet_mat='".$tabmatieres[$j_matiere][0]."',
													...
													type_brevet='$type_brevet';";
			if($debug_ajout_matiere=="y") {
				echo "<p>Ajout de la matière Gees choisie pour la matière notanet n°$j_matiere&nbsp;:<br />";
				echo "$sql<br />";
			}
			//echo "$sql<br />";
			$res_insert=mysqli_query($GLOBALS["mysqli"], $sql);
...
C'est ce qui est proposé pour la note d'EPS qui doit correspondre à la moyenne de trois APSA et non à la moyenne des trois trimestres.";
		...
		echo "<th rowspan='2'>Matière GEES</th>\n";
					...
					*/
					...
					if(mysqli_num_rows($res_test)>0) {
						...
						while($lig_tmp=mysqli_fetch_object($res_test)) {
							...
							if(mysqli_num_rows($test_matiere)==0) {echo "<img src='../images/icons/ico_attention.png' width='22' height='19' title=\"Cette matière ne correspond plus à une matière GEES cette année (un nouveau nom de matière existe peut-être cette année).\" alt=\"Cette matière ne correspond plus à une matière GEES cette année (un nouveau nom de matière existe peut-être cette année).\" />\n";}
							else {
	...
	document.getElementById('td_mode_matiere_extract_moy_$j').title='Aucune matière Gees n\'est associée.\\n\\nMême si c\'est une matière avec note saisie plutôt qu\'extraite des bulletins, commencez par associer la matière du professeur qui fera la saisie.';
</script>";
					}
					echo "<p align='center'>";
	...
	<ol>
		<li>créer dans Gees une matière <strong>EPS_brevet</strong> distincte de la matière <strong>EPS</strong> pour éviter des problèmes d'identification de l'enseignement à prendre en compte par le module notanet</li>
		<li>associer à cette matière les professeurs d'EPS (<em>au moins ceux qui enseignent en 3ème</em>)</li>
		<li>créer dans Gees, pour chacune des classes de 3ème, un enseignement EPS_brevet, coefficient 0 avec le même professeur que l'enseignement d'EPS</li>
		...
		if(($type_brevet==2)||($type_brevet==3)||($type_brevet==4)||($type_brevet==5)||($type_brevet==6)) {
				...
				Pourtant, l'application académique Notanet n'accepte pas que la matière 104 soit alors déclarée comme Non dispensée et donc n'apparaisse pas dans le fichier CSV généré par Gees.<br />
				Dans ce cas, il conviendra d'associer la même matière Gees pour les deux matières Notanet LV1 (103) et SCPHY (104).<br />
