				...
				*/
				...
				if(mysqli_num_rows($res_notanet)>0) {
					...
					if($check_app=='y') {
						...
						// Est-ce qu'on pourrait associer deux fois une matière Gees à des matières notanet différentes?
	...
	if($check_app=='y') {
		if(count($tab_prof_manques)==0) {
			...
			foreach($tab_prof_manques as $login_prof => $tab_prof) {
				...
				$message.="\nSi ce signalement vous parait être une erreur, merci d'en alerter l'administrateur Gees.\n";
				...
				if($tab_prof_manques[$login_prof]['email']!="") {
					if(check_mail($tab_prof_manques[$login_prof]['email'])) {
						$tab_num_mail[]=$num;
						$sujet_mail="[Gees]: Appreciations des Fiches Brevet manquantes";
				...
				if(!in_array($num, $tab_num_mail)) {
					...
					echo "<span id='mail_envoye_$num'><a href='#' onclick=\"envoi_mail($num);return false;\" title=\"Envoyer depuis Gees, le message ci-contre à $info_prof\">Envoyer</a></span>";
				}
				echo "</td>\n";
				echo "</tr>\n";
