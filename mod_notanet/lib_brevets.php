...
/*
		case 2:
			...
			//           GEES ne doit donner que des notes sur 20.
...
function tab_extract_moy($tab_ele,$id_clas) {
	...
	// On y récupère (pour chaque matière Notanet) la liste des matières Gees associées à une matière Notanet
...
//echo "<tr><td colspan='5'>\$statut_matiere[$j]=".$statut_matiere[$j]."</td></tr>";
		...
		// Utile pour repérer les matières Notanet associées à deux matières Gees pour lesquelles l'élève a une note
		$liste_matieres_gepi="";
		//if($tabmatieres[$j][0]!=''){
		if(($tabmatieres[$j][0]!='')&&($statut_matiere[$j]!='non dispensee dans l etablissement')){
			...
			if($tabmatieres[$j]['socle']=='n') {
				...
				// Boucle sur les différentes matières Gees associées à matière Notanet
							...
							if(mysqli_num_rows($resultat_notes)>0){
								...
								while($ligne_notes=mysqli_fetch_object($resultat_notes)){
									...
									if(($ligne_notes->note!="")&&($ligne_notes->note!="-")){
										...
										// ... mais cela ne devrait pas arriver, ou alors la base GEES n'est pas nette.
...
// Liste de codes du module OOo
	...
	[eleves.101.2]  ->      Nom de matière Gees
