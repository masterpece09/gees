...
$fb_gab_perso=getSettingValue("fb_gab_perso");
	...
	if($alt==1){echo " style='background: white;'";}else{echo " style='background: silver;'";}
	echo ">\n";
	echo "<td valign='top'>Gabarits : <br />Vous pouvez utiliser les gabarits intégrés à Gees<br />(<em>construits à partir des fiches brevets de Rouen</em>) <br />ou utiliser le module openDocument pour enregistrer vos propres gabarits</td>\n";
	echo "<td>";
	...
	echo "<label for='fb_gab_perso_0'> Gabarits Gees </label><br />";
	echo "</td>\n";
	echo "</tr>\n";
...
$fb_dezip_ooo=getSettingValue("fb_dezip_ooo");
	...
	echo "<td valign='top'>Décompresseur d'archive : <br />Gees a besoin d'un décompresseur d'archive pour créer les fiches brevets. Vous devez sélectionner celui que vous voulez utiliser.</td>\n";
	echo "</td>";
	echo "<td>";
...
<ul>
	...
	<p style='text-indent:-4em;margin-left:4em;'><em>Remarque&nbsp;:</em> Si vous obtenez une erreur, avec un <strong>Gabarit personnel</strong>, commencez par tester si le modèle officiel Gees permet de régler le problème.</p></li>
</ul>";
...
<ul>
	...
	Vous pouvez y choisir d'utiliser un modèle personnalisé en sélectionnant <strong>Gabarit personnel</strong> plutôt que <strong>Gabarit Gees</strong>.<br />
	...
	for($j=$indice_premiere_matiere;$j<=$indice_max_matieres;$j++){
		if($tabmatieres[$j][0]!=''){
			...
			if(mysqli_num_rows($res)>0){
				...
				// On ne récupère qu'une seule des matières Gees associées, là.
