...
<?php
	...
	if((getSettingValue('use_sso')=="lcs")||(getSettingValue('use_sso')=="ldap_scribe")) {
		...
		Si vous initialisez l'année avec le mode XML, vous ne pourrez pas utiliser les comptes de votre serveur LCS/SCRIBE par la suite pour accéder à GEES.<br />Réfléchissez-y à deux fois avant de poursuivre.</p>\n";
		echo "<br />\n";
	}
...
<!--/p-->
<ul>
<li><p>Au cours de la procédure, le cas échéant, certaines données de l'année passée seront définitivement effacées de la base GEES (<em>élèves, notes, appréciations, ...</em>) . Seules seront conservées les données suivantes, qui seront seulement mises à jour si nécessaire :<br /><br />
...
<li>
    ...
    <li><p><a name="eleves_classes"></a><a href='eleves_classes.php'>Procéder à la cinquième phase</a> d'affectation des élèves aux classes  : le fichier <b>g_eleves_classes.csv</b> requis.
    	...
    	<br/><em>Remarque&nbsp;:</em> cette opération créé automatiquement les classes dans Gees, mais ne leur attribue qu'un nom court (identifiant). Vous devrez ajouter le nom long par l'interface de gestion des classes.<br />&nbsp;</p></li>
...
<p>Il vous sera par ailleurs possible d'effectuer toutes les modifications nécessaires au cas par cas par le biais des outils de gestion inclus dans <b>GEES</b>.</p></li>
</ul>
<p><br /></p>
