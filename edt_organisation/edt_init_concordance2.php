...
if ($etape != NULL) {
	...
	if ($etape != 12 AND $etape != 5) {
			$values = NULL;
		// C'est le cas général pour enregistrer les concordances entre le fichier csv et Gees
		...
		if ($etape == 6 OR $etape == 8 OR $etape == 9 OR $etape == 11) {
			...
			$enre = $deja = 0;
		// Ce sont les salles. On va enregistrer celles qui ne sont pas encore dans Gees
		for($a = 0; $a < $nbre_lignes; $a++){
			$nom_export[$a] = isset($_POST["nom_export_".$a]) ? $_POST["nom_export_".$a] : NULL;
