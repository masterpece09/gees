...
function renvoiDebut($id_creneau, $heure_deb, $jour){
	...
	// Nombre de mn de l'horaire de Gees
	$table = nomTableCreneau($jour);
	if ($id_creneau != '') {
...
function renvoiIdGroupe($prof, $classe_txt, $matiere_txt, $grp_txt, $partie_txt, $type_import){
	// $prof est le login du prof tel qu'il existe dans Gees, alors que les autres infos ne sont pas encore "concordés"
...
/*
 * Fonction qui teste si une salle existe dans Gees et qui l'enregistre si elle n'existe pas
...
/*
 * Fonction qui teste si une salle existe dans Gees
 * $numero est le numéro de la salle
*/
...
function rechercheCreneauCsv2($creneau){
	...
	if (isset($test1[1])) {
		...
		if ($duree_demandee == $duree_base) {
			// ALors la durée est de 1 créneau donc 2 pour Gees
			$retour["duree"] = 2;
		...
		// ça veut dire que le cours commence au début du créneau et dure 1 créneau (donc 2 pour Gees)
		$retour["duree"] = '2';
		$retour["debut"] = '0';
	}
	return $retour;
