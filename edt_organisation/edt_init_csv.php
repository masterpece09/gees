...
$compter_echecs = 2; // pour afficher à la fin le message : Tous ces cours ont bien été enregistrés.
	...
	// Initialisation de l'EdT (fichier g_edt.csv). Librement copié du fichier init_csv/eleves.php
        // On va donc afficher le contenu du fichier tel qu'il va être enregistré dans Gees
    ...
    $rep_prof = mysqli_fetch_array($req_prof);
		...
		$rep_matiere = mysqli_fetch_array($sql_matiere);
			if ($rep_matiere["nom_complet"] == "") {
				$probleme .= "<p>Gees ne retrouve pas la bonne mati&egrave;re.</p>\n";
			}
	...
	$req_groupe = mysqli_query($GLOBALS["mysqli"], "SELECT jgp.id_groupe FROM j_groupes_professeurs jgp, j_groupes_classes jgc, j_groupes_matieres jgm WHERE jgp.login = '".$prof_login."' AND jgc.id_classe = '".$rep_classe["id"]."' AND jgm.id_matiere = '".$matiere."' AND jgp.id_groupe = jgc.id_groupe AND jgp.id_groupe = jgm.id_groupe");
    		...
    		if ($rep_groupe == "") {
				$probleme .= "<p>Gees ne retrouve pas le bon enseignement.</p>\n";
			} else {
		...
		if ($rep_groupe[0] != "" AND $rep_jour != "" AND $rep_heuredebut != "" AND $probleme == "") {
			...
			echo "<br /><span class=\"refus\">Ce cours n'est pas reconnu par Gees :</span>\n".$message."(".$message1.")".$probleme.".";
...
if ($aff_infos != "oui") {
	...
	$contenu_matiere = "Attention de bien respecter le nom court utilis&eacute; dans Gees. Il est de la forme $aff1_forme_matiere pour $aff2_forme_matiere.";
...
<hr />
	...
	<p><span class="red">Attention</span> de bien respecter les heures, jour, nom de mati&egrave;re,... de Gees que vous avez pr&eacute;cis&eacute; auparavant.
...
<a href="#ancre1" onclick="javascript:changerDisplayDiv('aide_initcsv');" name="ancre1">
	...
	<hr />
	<span class="red">Attention</span>, ces champs ont des r&egrave;gles &agrave; suivre : il faut respecter la forme retenue par Gees
	<br />
