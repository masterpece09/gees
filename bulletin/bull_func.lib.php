		...
		//if(($bull_affiche_abs_tot=='y')||($bull_affiche_abs_nj=='y')||($bull_affiche_abs_ret=='y')) {
			...
			if($bull_affiche_abs_tot=='y') {
				if ($tab_bull['eleve'][$i]['eleve_absences'] == '0') {
					...
					echo "<i>Nombre d'heures d'absence";
			...
			elseif($bull_affiche_abs_nj=='y') {
				if ($tab_bull['eleve'][$i]['eleve_nj'] == '0') {
					...
					echo "<i>Nombre d'heures d'absence!! non justifiées&nbsp;: <b>".$tab_bull['eleve'][$i]['eleve_nj']."</b>";
				}
				echo ".";
			}
...
// Célestin 23-12-2015 ICI function d'inpression des bulletins
function bulletin_pdf($tab_bull,$i,$tab_rel) {
	...
	*/
	...
	for($num_resp_bull=0;$num_resp_bull<$nb_bulletins;$num_resp_bull++) {
		...
		// $fomule = 'Bulletin à conserver précieusement. Aucun duplicata ne sera délivré. - GEES : solution libre de gestion et de suivi des résultats scolaires.'
                $bull_formule_bas = 'Bulletin à conserver précieusement. Aucun duplicata ne sera délivré. - GEES Solution de gestion et de suivi des résultats scolaires.';
		$pdf->Cell(0,4.5, ($bull_formule_bas),0,0,'C');
		//================================
                
                //Célestin 2015-12-24                
                //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE
                enteteSuperieurdroite();
                
                //Célestin 25-01-2016
                //INITIALISATION DES VARIABLE DEVANT CONTENIR LES SOMMES DES NOTES COEFICIÉ DE L'ÉLÈVE
                $note_coeficie1 = 0;
                $note_coeficie2 = 0;
                $note_coeficie3 = 0;
                ...
                $periode = $tab_bull['bull_prefixe_periode'].unhtmlentities($tab_bull['nom_periode']);
                //verticalLine($periode);                
		...
		if($tab_modele_pdf["active_bloc_datation"][$classe_id]==='1') {
			...
			$pdf->SetFont('DejaVu', 'B', 14);
                        //$periode = $tab_bull['bull_prefixe_periode'].unhtmlentities($tab_bull['nom_periode']);
			$pdf->Cell(130,7, 'BULLETIN '.($tab_bull['bull_prefixe_periode'].unhtmlentities($tab_bull['nom_periode'])),0,2,'C');
			$taille_texte = '8';
			$type_texte = '';
		...
		*/
		...
		if($tab_modele_pdf["active_bloc_note_appreciation"][$classe_id]==='1' and $nb_matiere!='0') {
			...
			$pdf->SetFont('DejaVu','',10);
			$pdf->Cell($tab_modele_pdf["largeur_matiere"][$classe_id], $hauteur_entete,($tab_modele_pdf["titre_entete_matiere"][$classe_id]),1,0,'L');
					...
					while (!empty($ordre_moyenne[$cpt_ordre]) ) {
						...
						if($tab_modele_pdf["active_moyenne_eleve"][$classe_id] === '1' and $tab_modele_pdf["active_moyenne"][$classe_id] === '1' and $ordre_moyenne[$cpt_ordre] === 'eleve' ) {
							...
							($tab_modele_pdf["moyennes_annee"][$classe_id]!='y')) {
								...
								$pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $valeur.$fleche_evolution,1,0,'C',$tab_modele_pdf["active_reperage_eleve"][$classe_id]);
                                                                
                                                                //Célestin 2016-01-02
                                                                //On affiche l'appréciation
                                                                $apprciation = mention($valeur.$fleche_evolution);
                                                                $pdf->setX(182);
                                                                $pdf->Cell(20, $espace_entre_matier/$nb_sousaffichage,$apprciation ,0,0,'C',0);
                                                                
                                                                
                                                                //Célestin 2015-152-30
                                                                //ICI ON CALCUL ET ON MET LES NOTES DES SEQUENCES
                                                                if(preg_match('/^Trim\w* N°1/',trim($periode)) || preg_match('/^Trim\w* N°2/',trim($periode))){                                                                    
                                                                    $note = noteSequentielle($tab_bull['eleve'][$i]['login'], $periode, $info_nom_matiere);
                                                                    $pdf->setX(89);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[0],0,0,'C',0);

                                                                    $pdf->setX(110);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[1],0,0,'C',0);
                                                                    
                                                                    if($note[0]!="-"){
                                                                        $note_coeficie1 = $note_coeficie1 + $note[0]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie1 = $total_coeficie1 + $tab_bull['coef_eleve'][$i][$m];
                                                                    } 
                                                                    
                                                                    if($note[1]!="-"){
                                                                        $note_coeficie2 = $note_coeficie2 + $note[1]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie2 = $total_coeficie2 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                        
                                                                    
                                                                }  elseif (preg_match('/^Annuel/',trim($periode))) {
                                                                    $note = noteSequentielle($tab_bull['eleve'][$i]['login'], $periode, $info_nom_matiere);
                                                                    ...
                                                                    $pdf->setX(65);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[2],0,0,'C',0);
                                                                    
                                                                    $pdf->setX(75);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[3],0,0,'C',0);
                                                                    
                                                                    $pdf->setX(85);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[4],0,0,'C',0);
                                                                    
                                                                    $pdf->setX(95);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[5],0,0,'C',0);
                                                                    
                                                                    $pdf->setX(105);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[6],0,0,'C',0);
                                                                    
                                                                    $pdf->setX(115);
                                                                    $pdf->Cell($tab_modele_pdf["largeur_d_une_moyenne"][$classe_id], $espace_entre_matier/$nb_sousaffichage, $note[7],0,0,'C',0);
                                                                    ...
                                                                    if($note[1]!="-"){
                                                                        $note_coeficie1 = $note_coeficie1 + $note[1]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie1 = $total_coeficie1 + $tab_bull['coef_eleve'][$i][$m];
                                                                    } 
                                                                    
                                                                    if($note[2]!="-"){
                                                                        $note_coeficie2 = $note_coeficie2 + $note[2]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie2 = $total_coeficie2 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                    
                                                                    if($note[3]!="-"){
                                                                        $note_coeficie3 = $note_coeficie3 + $note[3]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie3 = $total_coeficie3 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                    
                                                                    //26-01-2016
                                                                    if($note[4]!="-"){
                                                                        $note_coeficie4 = $note_coeficie4 + $note[4]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie4 = $total_coeficie4 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                    
                                                                    if($note[5]!="-"){
                                                                        $note_coeficie5 = $note_coeficie5 + $note[5]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie5 = $total_coeficie5 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                    
                                                                    if($note[6]!="-"){
                                                                        $note_coeficie6 = $note_coeficie6 + $note[6]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie6 = $total_coeficie6 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
                                                                    
                                                                    if($note[7]!="-"){
                                                                        $note_coeficie7 = $note_coeficie7 + $note[7]*$tab_bull['coef_eleve'][$i][$m];
                                                                        $total_coeficie7 = $total_coeficie7 + $tab_bull['coef_eleve'][$i][$m];
                                                                    }
			...
			if ( $tab_modele_pdf["active_moyenne"][$classe_id] === '1' and $tab_modele_pdf["active_moyenne_general"][$classe_id] === '1' ) {
				...
				if ($affiche_deux_moy_gen==1) {
					$pdf->Cell($tab_modele_pdf["largeur_matiere"][$classe_id], $tab_modele_pdf["hauteur_entete_moyenne_general"][$classe_id], ("Moy.gén.coef."),1,0,'C', $tab_modele_pdf["couleur_moy_general"][$classe_id]);
                                        ...
                                        $pdf->SetFont('DejaVu','',8);
					$pdf->Cell($tab_modele_pdf["largeur_matiere"][$classe_id]  , $tab_modele_pdf["hauteur_entete_moyenne_general"][$classe_id], ("Moyenne générale"),1,0,'L', $tab_modele_pdf["couleur_moy_general"][$classe_id]);
					$pdf->SetFont('DejaVu','',10);
                                        
                                        //Célestin 25-01-2016
                                        //Affichage des moyennes des séquences                                        
                                        
                                        if(preg_match('/^Annuel$/',trim($periode))){
                                            
                                            $pdf->SetXY(57, $Y_note_moy_app);                                        
                                            
                                            if($total_coeficie1!=0){
                                                $moy_seq = $note_coeficie1 / $total_coeficie1;                                        
                                            }else{
                                                $moy_seq = "";
                                            }
                                            
                                            $pdf->Cell(5,5,  utf8_decode(round($moy_seq,2)),0,0,'C',0);
                                            
                                            $pdf->SetXY(67, $Y_note_moy_app);                                        
                                            
                                            if($total_coeficie2!=0){
                                                $moy_seq = $note_coeficie2 / $total_coeficie2;
                                            }else{
                                                $moy_seq = "";
                                            }
                                            ...
                                            $pdf->SetXY(77, $Y_note_moy_app);                                        
                                            if($total_coeficie3!=0){
                                                $moy_seq = $note_coeficie3 / $total_coeficie3; 
                                            }else{
                                                $moy_seq = "";
                                            }
                                            ...
                                            $pdf->SetXY(87, $Y_note_moy_app);
                                            if($total_coeficie4!=0){
                                                $moy_seq = $note_coeficie4/$total_coeficie4;
                                            }else{
                                                $moy_seq = "";
                                            }
                                            ...
                                            $pdf->SetXY(97, $Y_note_moy_app);                                        
                                            if($total_coeficie5!=0){
                                                $moy_seq = $note_coeficie5 / $total_coeficie5;
                                            }else{
                                                $moy_seq = "";
                                            }
                                            ...
                                            $pdf->SetXY(107, $Y_note_moy_app);                                        
                                            
                                            if($total_coeficie6!=0){
                                                $moy_seq = $note_coeficie6 / $total_coeficie6;                                        
                                            }else{
                                                $moy_seq = "";
                                            }
                                            ...
                                            $pdf->SetXY(117, $Y_note_moy_app);
                                            
                                            if($total_coeficie7!=0){
                                                $moy_seq = $note_coeficie7/$total_coeficie7;
                                            }else{
                                                $moy_seq = "";
                                            }
                                            
                                            $pdf->Cell(5,5, utf8_decode(round($moy_seq,2)),0,0,'C',0);                                        
                                        }elseif(preg_match('/^Trim\w* N°1/',trim($periode)) || preg_match('/^Trim\w* N°2/',trim($periode))){                                                                                        
                                            $pdf->SetXY(91, $Y_note_moy_app);                                        
                                            if($total_coeficie1!=0){
                                                $moy_seq = $note_coeficie1 / $total_coeficie1;                                        
                                            }else{
                                                $moy_seq = "";
						...
						*/
						...
						if ($tab_bull['moy_min_classe']!='-') {
							//$moyenne_min=$tab_moy_min_classe[$classe_id][$id_periode];
                                                         //present_nombre(preg_replace("/,/",'.',$tab_bull['moy_min_classe'])
                                                         $moyenne_min =  present_nombre(preg_replace("/,/",'.',$tab_bull['moy_min_classe']), $tab_modele_pdf["arrondie_choix"][$classe_id], $tab_modele_pdf["nb_chiffre_virgule"][$classe_id], $tab_modele_pdf["chiffre_avec_zero"][$classe_id]);
				...
				$pdf->SetFillColor(0, 0, 0);
                        ...
                        //Célestin 2015-12-30
                        //ICI ON AJOUTE LES COLONES DES SÉQUENCES EN CAS DE BULLETIN TRIMESTRILLE
                        verticalLine($periode);   
                        
                        //Célestin 2015-12-26
                        //AFFICHAGE DU RÉSUME DE LA CLASSE
                        eleveClasse($rangEleve, $moyenneEleve, $moyenne_min, $moyenne_max, $moyenne_classe );

                        //AFFICHAGE DES CADRES POUR VISA
                        afficherCadresVisa();
				...
				*/
				...
				if($tab_modele_pdf["afficher_abs_tot"][$classe_id]=='1') {
					if($tab_bull['eleve'][$i]['eleve_absences'] != '?') {
						if($tab_bull['eleve'][$i]['eleve_absences'] == '0')
						{
							...
							$info_absence="<i>Nombre d'heures d'absence ";
				...
				elseif($tab_modele_pdf["afficher_abs_nj"][$classe_id]=='1') {
					if($tab_bull['eleve'][$i]['eleve_nj'] == '0')
					{
						...
						$info_absence="<i>Nombre d'heures d'absence!! non justifiées ";
...
$hauteur_pris_app_abs=0;
				...
				$pdf->SetFont('DejaVu','',10);
                        ...
                        //Célestin 2015-12-26eleveRetards
                        //AFFICHAGE DE LA DECISION DU CONSEIL DE LA CLASSE
                        $eleveAbsences =$tab_bull['eleve'][$i]['eleve_absences'];
                        $eleveAbsencesNJ = $tab_bull['eleve'][$i]['eleve_nj'];
                        $eleveAbsencesJustifier = $eleveAbsences - $eleveAbsencesNJ;
                        $eleveRetards = $tab_bull['eleve'][$i]['eleve_retards'];
                        decisionConseilClasse($moyenneEleve, $eleveAbsencesJustifier, $eleveAbsencesNJ, $eleveRetards, $periode);
	...
	*/
	...
	//if(isset($tab_rel['eleve'][$i]['login'])) {
		...
		*/
		...
		$pdf->Cell(90,5,'Né'.$e_au_feminin.' le '.$tab_rel['eleve'][$i]['naissance'].', '.regime($tab_rel['eleve'][$i]['regime']),0,2,'');//ado
		...
		$pdf->SetFont('DejaVu','',10);
		$pdf->Cell(90,5,''.$annee_scolaire,0,2,'');
		...
		*/
		...
		$pdf->SetFont('DejaVu','',8);
		$pdf->Cell(200,5,'GEES - Solution libre de Gestion des élèves par Internet',0,1,'');
...
//Célestin 2015-12-24
//Fonction qui affiche l'entête sup droite
function enteteSuperieurdroite() {
    header( 'content-type: text/html; charset=utf-8' );
    global  $pdf;
    //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (REP DU CAMEROUN)
    $pdf->SetXY(90,4);  
    $pdf->SetFont('Helvetica','B',10);
    $pdf->Cell(158,8,'REPUBLIQUE DU CAMEROUN',0,0,'C');
                
    //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (Paix-Travail-Patrie)
    $pdf->SetXY(90,9);  
    $pdf->SetFont('Helvetica','i',6);
    $pdf->Cell(158,8,'Paix-Travail-Patrie',0,0,'C');
    
    //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (Ministère de l'enseignement secondaire)
    $pdf->SetXY(90,14);  
    $pdf->SetFont('Helvetica','',8);
    $pdf->Cell(158,8,utf8_decode('MINISTERE DES ENSEIGNEMENTS SECONDAIRES'),0,0,'C');

   //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (**************************************)
    $pdf->SetXY(90,20);  
    $pdf->SetFont('','',7);
    $pdf->Cell(158,7,'*************************************',0,0,'C');

    
    //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (Diocèse de Nkongsamba)
    $pdf->SetXY(90,24);  
    $pdf->SetFont('Helvetica','B',10);
    $pdf->Cell(158,8,utf8_decode('Diocèse de Nkongsamba'),0,0,'C');
    
    //ICI ON AJOUTE L'ENTÊTE SUPERIEUR DROITE (Secrétariat de l'education)
    $pdf->SetXY(90,29);  
    $pdf->SetFont('Helvetica','',8);
    $pdf->Cell(158,8,utf8_decode("Sécrétariat à l'education"),0,0,'C');

}

//Célestin 2015-12-24
//Cette fonction AFFICHAGE DE LA DECISION DU CONSEIL DE LA CLASSE
function decisionConseilClasse($moyenneEleve, $eleveAbsencesJustifier, $eleveAbsencesNJ, $eleveRetards, $periode){
    global $pdf;
    ...
    $pdf->SetXY(5,215);
    $pdf->SetFont('','B','8');
    $pdf->Cell(100,5,'Décisions du conseil de classe',1,0,'C',1);
    ...
    $pdf->SetXY(5,220);
    $pdf->SetFont('','','');
    $pdf->Cell(50,5,utf8_decode('DISCIPLINE'),1,0,'C',1);
    $pdf->SetX(55);
    $pdf->Cell(50,5,utf8_decode('TRAVAIL'),1,0,'C',1);
    $pdf->Ln(); // Retour à la ligne
    
    $pdf->SetXY(5,225);
    $pdf->Cell(20,10,utf8_decode('Absences(h)'),1,0,'L',0);
        
    $pdf->SetX(25);
    $pdf->Cell(10,5,utf8_decode('J'),1,0,'C',0); 
    
    $pdf->SetX(35);
    $pdf->Cell(20,5,$eleveAbsencesJustifier,1,0,'C',0);
    
    $pdf->SetX(55);
    $pdf->SetFont('','B','8');
    $pdf->Cell(50,10,  mention($moyenneEleve),1,0,'C',0);
    $pdf->SetFont('','','');
    $pdf->SetXY(25,230);
    $pdf->Cell(10,5,utf8_decode('NJ'),1,0,'C',0);
    $pdf->SetX(35);
    $pdf->Cell(20,5,$eleveAbsencesNJ,1,0,'C',0);
    
    
    $pdf->SetXY(5,235);
    $pdf->Cell(20,10,utf8_decode('Retards'),1,0,'L',0);
    
    $pdf->SetX(25);
    $pdf->Cell(10,5,utf8_decode('T'),1,0,'C',0);    
    $pdf->SetX(35);
    $pdf->Cell(20,5,$eleveRetards,1,0,'C',0);
    
    $pdf->SetX(55);
    
    if(!preg_match('/^Annuel$/',trim($periode))){
        $pdf->Cell(10,5,utf8_decode('Un effort s\'impose en '),0,0,'L',0);
    }    
    
    $pdf->SetX(55);
    $pdf->Cell(50,25,'',1,0,'L',0);
    ...
    $pdf->SetXY(5,245);
    $pdf->Cell(30,5,utf8_decode('Avertissements'),1,0,'L',0);
    $pdf->SetX(35);
    $pdf->Cell(20,5,'',1,0,'C',0);
    
    $pdf->SetXY(5,250);
    $pdf->Cell(30,5,'Blâmes de conduite',1,0,'L',0);
    $pdf->SetX(35);
    $pdf->Cell(20,5,'',1,0,'C',0);
    
    $pdf->SetXY(5,255);
    $pdf->Cell(30,5,utf8_decode('Exclusions (en jours)'),1,0,'L',0);
    $pdf->SetX(35);
    $pdf->Cell(20,5,'',1,0,'C',0);
    
    $pdf->SetXY(5,260);
    $pdf->SetFont('','B','8');
    $pdf->Cell(100,5,utf8_decode('Visa des Parents ou Tuteurs'),1,0,'C',1);
    
    $pdf->SetXY(5,265);
    $pdf->Cell(100,15,'',1,0,'C',0);    
    
    //VERROUILLAGE
    /*$now = date('Y-m-d',time());
    if(){
        $con = new Connexion();    
        $sql = "RENAME TABLE utilisateurs TO utilisateur";    
...
//Célestin 2015-12-26
// CETTE FONCTION AFFICHE LE RECAPITULATIF DE LA CLASSE ET DE L'ELEVE
function eleveClasse($rangEleve, $moyenneEleve, $minClasse, $maxClasse, $moyenneClasse ) {
    global $pdf;
    $pdf->SetDrawColor(183); // Couleur du fond
    $pdf->SetFillColor(221); // Couleur des filets
    $pdf->SetTextColor(0); // Couleur du texte
    $pdf->SetXY(110,215);
    $pdf->SetFont('','B','8');
    $pdf->Cell(48,5,'Elève',1,0,'C',1);
    
    $pdf->SetX(158);
    $pdf->Cell(47,5,utf8_decode('Classe'),1,0,'C',1);
    
            
    $pdf->SetXY(110,220);
    $pdf->Cell(48,15,'',1,0,'C',0);
    
    $pdf->SetX(158);
    $pdf->Cell(47,15,'',1,0,'C',0);
    
    $pdf->SetX(110);
    $pdf->SetFont('','','');
    $pdf->Cell(10,5,'Rang',0,0,'C',0);
    $pdf->SetX(125);
    $pdf->SetFont('','B','');
    $pdf->Cell(10,5,utf8_decode($rangEleve),0,0,'C',0);
    
    $pdf->SetXY(110,230);
    $pdf->SetFont('','','');
    $pdf->Cell(10,5,'Moy',0,0,'C',0);
    $pdf->SetX(125);
    $pdf->SetFont('','B','');
    $pdf->Cell(10,5,utf8_decode($moyenneEleve),0,0,'C',0);
    
    $pdf->SetXY(158,220);
    $pdf->SetFont('','','');
    $pdf->Cell(10,5,'Min',0,0,'C',0);
    $pdf->SetX(175);
    $pdf->SetFont('','B','');
    $pdf->Cell(10,5,utf8_decode($minClasse),0,0,'C',0);
    ...
    $pdf->SetX(175);
    $pdf->SetFont('','B','');
    $pdf->Cell(10,5,utf8_decode($moyenneClasse),0,0,'C',0);
    
    $pdf->SetXY(158,230);
    $pdf->SetFont('','','');
    $pdf->Cell(10,5,'Max',0,0,'C',0);
    $pdf->SetX(175);
    $pdf->SetFont('','B','');
    $pdf->Cell(10,5,  utf8_decode(round($maxClasse,2)),0,0,'C',0);
    
}

//Célestin 2015
//Cette fonction affiche les cadres des visa
function afficherCadresVisa() {
    global $pdf;
    $pdf->SetDrawColor(183); // Couleur du fond
    $pdf->SetFillColor(221); // Couleur des filets
    $pdf->SetTextColor(0); // Couleur du texte
    $pdf->SetXY(110,235);
    $pdf->SetFont('','B','8');
    $pdf->Cell(95,10,'Observations et Visa du Surveillant Général',1,0,'C',1);
   // echo 'Texte accentué.';
    ...
    $pdf->SetXY(110,260);
    $pdf->SetFont('','B','8');
    $pdf->Cell(95,5,utf8_decode('Visa du Principal'),1,0,'C',1);
    ...
    /*
    $block_date = date_create("2016-05-01");
...
function mention($moyenne) {
    $mension = "NOT IMPLEMENT";
    switch ($moyenne) {
        case ($moyenne>=0 && $moyenne <3):
            $mension = "NUL";
            break;
        case ($moyenne>=3 && $moyenne <6):
            $mension = "TRES FAIBLE";
            break;
        case ($moyenne>=6 && $moyenne <8):
            $mension = "FAIBLE";
            break;
        case ($moyenne>=8 && $moyenne <9):
            $mension = "INSUFFISANT";
            break;
        case ($moyenne>=9 && $moyenne <10):
            $mension = "MEDIOCRE";
            break;
...
//
//Dessigne une ligne vertical pour les séquences

function  verticalLine($periode){
    
    global $pdf;    
    
    if(preg_match('/^Trim\w* N°1$/',trim($periode))){
        $pdf->SetDrawColor(0,0,0);
        $pdf->Line(85, 72, 85, 212);
        $pdf->Line(106, 72, 106, 212);
        $pdf->SetXY(85, 72);
        $pdf->Cell(10,8,"Seq 1",0,0,'C',0);
        $pdf->SetXY(110, 72);
        $pdf->Cell(10,8,"Seq 2",0,0,'C',0);
    }elseif(preg_match('/^Trim\w* N°2$/',trim($periode))){
        $pdf->SetDrawColor(0,0,0);
        $pdf->Line(85, 72, 85, 212);
        $pdf->Line(106, 72, 106, 212);
        $pdf->SetXY(89, 72);
        $pdf->Cell(10,8,"Seq 3",0,0,'C',0);
        $pdf->SetXY(110, 72);
        $pdf->Cell(10,8,"Seq 4",0,0,'C',0);
    }elseif(preg_match('/^Annuel$/',trim($periode))){
        $pdf->SetDrawColor(0,0,0);
        $pdf->Line(55, 72, 55, 212);
        $pdf->Line(65, 72, 65, 212);        
        $pdf->Line(75, 72, 75, 212);
        $pdf->Line(85, 72, 85, 212);
        $pdf->Line(95, 72, 95, 212);
        $pdf->Line(105, 72, 105, 212);                
        $pdf->Line(115, 72, 115, 212);
        ...
        $pdf->SetXY(95, 72);
        $pdf->Cell(10,8,"Seq 5",0,0,'C',0);
        
        $pdf->SetXY(105, 72);
        $pdf->Cell(10,8,"Seq 6",0,0,'C',0);
        
        $pdf->SetXY(115, 72);
        $pdf->Cell(10,8,"Tim 3",0,0,'C',0);
    }
}

//Célestin 2015-12-30
//CETTE FONCTION RETOURNE LA NOTE D'UN ELEVE A UNE PERIODE
function  getNote($periode, $eleve, $matiere, $devoir){
  

/*    $link = mysql_connect('localhost', 'root', ''); 
    mysql_select_db('geesado', $link);
    $sql    = "SELECT note, statut
    FROM cn_notes_devoirs, cn_devoirs, cn_conteneurs, cn_cahier_notes, periodes, j_eleves_classes
    WHERE j_eleves_classes.login = '".$eleve."' 
    AND cn_notes_devoirs.id_devoir = cn_devoirs.id
    AND cn_devoirs.nom_court = ".$devoir." 
    AND cn_conteneurs.id = cn_devoirs.id_conteneur
    AND cn_conteneurs.nom_court = '".$matiere."' 
    AND cn_cahier_notes.id_cahier_notes = cn_conteneurs.id
    AND cn_cahier_notes.periode = periodes.num_periode
    AND periodes.nom_periode LIKE '".$periode."' 
    ...
    fich_debug_bull("<br/><br/> REQ".  $sql);
    
    $result = mysql_query($sql, $link);
    $nombre = 0;
    while ($row = mysql_fetch_assoc($result)) {
        $note = $row['note'];
        fich_debug_bull("<br/> -$eleve-NOTE- ".  $row['note']);
	if( $row['statut']!="abs" && $row['statut']!= "v"){
		$nombre = 1;
	}	
    }
*/
    ...
    $sql = "SELECT statut, note "
    . "FROM cn_notes_devoirs, cn_devoirs, cn_conteneurs, cn_cahier_notes, periodes, j_eleves_classes "
    . "WHERE j_eleves_classes.login = :eleve "
    . "AND cn_notes_devoirs.id_devoir = cn_devoirs.id "
    . "AND cn_devoirs.nom_court LIKE :devoir "
    . "AND cn_conteneurs.id = cn_devoirs.id_conteneur "
    . "AND cn_conteneurs.nom_court = :matiere "
    . "AND cn_cahier_notes.id_cahier_notes = cn_conteneurs.id "
    . "AND cn_cahier_notes.periode = periodes.num_periode "
...
//Célestin 2015-12-30
//Cette fonction retourne les notes séquentielle en fonction du trimestre

function noteSequentielle($eleve, $periode, $nomMatiere){
    
    $notes = "";
    
    if(preg_match('/^Trim\w* N°1$/',trim($periode))){
        $notes[0] = getNote('S%quentiel%1', $eleve, $nomMatiere, '1');
        $notes[1] = getNote('Trimestriel%1', $eleve, $nomMatiere, '2');
    }elseif(preg_match('/^Trim\w* N°2$/',trim($periode))){
       $notes[0] = getNote('S%quentiel%3', $eleve, $nomMatiere, '3');
       $notes[1] = getNote('Trimestriel%2', $eleve, $nomMatiere, '4');
    }elseif(preg_match('/^Annuel$/',trim($periode))){
        $notes[1] = getNote('S%quentiel%1', $eleve, $nomMatiere, '1');
        $notes[2] = getNote('Trimestriel%1', $eleve, $nomMatiere, '2');
        $notes[3] = getNote('S%quentiel%3', $eleve, $nomMatiere, '3');
        $notes[4] = getNote('Trimestriel%2', $eleve, $nomMatiere, '4');       
        $notes[5] = getNote('S%quentiel%5', $eleve, $nomMatiere, '5');
        $notes[6] = getNote('Annuel', $eleve, $nomMatiere, '6');
        ...
        fich_debug_bull("trim 1 ".$notes[0]);
        ...
        fich_debug_bull(" trim 2 ".$notes[1]);
        ...
        //26-01-16
        $notes[3] = getNote('Annuel', $eleve, $nomMatiere, '5');
        $notes[4] = getNote('Annuel', $eleve, $nomMatiere, '6');
        */
        fich_debug_bull(" trim 3 ".$notes[2]);
        
    }else{
        fich_debug_bull("PERIODE ".$periode);
    }
