...
for($j = 0; $j < count($matieres_classe); $j++){
    ...
    //Mis à jour des rang
    updateRang($matiere, $classe,$periode);
}
...
//Célestin 310516
//Cette function calcule les rang des élève
function updateRang($matiere, $classe, $periode){
    
    $groupe = getGroupe($matiere, $classe);
    $con = new Connexion();
    
    $sql = "SELECT matieres_notes.note, matieres_notes.login "
        . "FROM  matieres_notes "
        . "WHERE matieres_notes.id_groupe = :groupe "
        . "AND matieres_notes.periode = :periode "
        . "ORDER BY note DESC";
    ...
    //Adonia :utilisation de ma fonction : rangNotesEleves
    $notes=rangNotesEleves($notes,$k);
    
    while($k>0){
        $sql = "UPDATE matieres_notes SET rang = :rang "
                . "WHERE matieres_notes.id_groupe = :groupe "
                . "AND matieres_notes.periode = :periode "
                . "AND matieres_notes.login = :login ";
        ...
        $stmt->bindValue(':groupe', $groupe);   
	//debut Adonia		
        $stmt->bindValue(':rang',$notes[$k-1]["rang"] );  
	//fin Adonia		
        $stmt->bindValue(':login', $notes[$k-1]["login"]);    
        $stmt->execute();
        $k--;
    }
...
//Adonia 010616
//Cette function calcule les rang des élèves 
function rangNotesEleves($notes,$taille){
	$i=0;
	$rang=1;
	$note_pred = -1;
	 while($i<$taille){
		if($note_pred!=-1 && $note_pred!=$notes[$i]["note"])
			$rang++;
		$notes[$i]["rang"]=$rang;
		$note_pred=$notes[$i]["note"];
		$i++;
	}
	return $notes;
}