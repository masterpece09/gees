					...
					if(!$res) {
						...
						if($envoi_mail_actif=='y') {
							...
							if(mysqli_num_rows($req)>0) {
								...
								$sujet_mail="[GEES] Autorisation exceptionnelle de saisie/correction de notes de bulletins";
...
if(($_SESSION['statut']=='administrateur')&&(isset($_GET['definir_message']))) {
	...
	echo "<p>Par défaut le message reçu par un professeur exceptionnellement autorisé à saisir en retard ou corriger des notes de bulletins est le suivant&nbsp;:<br />\n";
	...
	if($message_autorisation_exceptionnelle_bull_note!='') {
		...
		$texte_mail="Bonjour/Bonsoir\n\nVous avez jusqu'au TELLE DATE TELLE HEURE\npour saisir/corriger une ou des notes de bulletins pour l'enseignement XXXXXXXXXX\nen TELLE(S) CLASSE(S) en période NUMERO_PERIODE.\n\n";
		$texte_mail.="<b>$message_autorisation_exceptionnelle_bull_note</b>\n";
		$texte_mail.="\nCordialement.";
	...
	echo "<textarea name='no_anti_inject_message_autorisation_exceptionnelle_bull_note' rows='2' cols='100'>$message_autorisation_exceptionnelle_bull_note</textarea>\n";
	echo "<br />\n";
...
if(!isset($id_classe)) {
	...
	echo " | <a href='autorisation_exceptionnelle_saisie_app.php'>Autoriser la modification d'appréciations des bulletins</a>\n";
	echo "</p>\n";

	echo "<h2>Autoriser la modification de moyennes des bulletins</h2>";
...
elseif((!isset($id_groupe))||(!isset($periode))) {
	...
	echo "<p>Pour quel enseignement souhaitez-vous autoriser un enseignant à effectuer des modifications de notes de ses bulletins&nbsp;?</p>\n";
	...
	echo " | <a href='autorisation_exceptionnelle_saisie_app.php?id_classe=$id_classe'>Autoriser la modification d'appréciations des bulletins</a>\n";
	echo "</p>\n";
