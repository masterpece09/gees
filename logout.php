...
if (getSettingValue('gepiEnableIdpSaml20') == 'yes' && (!isset($_REQUEST['idploggedout']))) {
		...
		$auth = new SimpleSAML_Auth_GeesSimple();
...
//$message = "<h1 class='gepi'>Déconnexion</h1>";
    ...
    if (!isset($_GET['auto']) || !$_GET['auto']) {
    	$session_gepi->close(0);
        $message .= "Vous avez fermé votre session GEES.";
        //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
    } else if ($_GET['auto']==2) {
        $session_gepi->close($_GET['auto']);
        $message .= "Vous avez été déconnecté. Il peut s'agir d'une mauvaise configuration de la variable \$GeesPath dans le fichier \"connect.inc.php\"<br />
        <a href='aide_gepipath.php'><b>Aide à la configuration de \$GeesPath</b></a>";
        //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
    } else if ($_GET['auto']==3) {
        ...
        if (sql_query1($sql)) {
           ...
           $session_gepi->close($_GET['auto']);
           $message .= "Votre session GEES a expiré car le temps maximum (".getSettingValue("sessionMaxLength")." minutes) sans échange avec le serveur a été atteint.<br /><br />Date et heure de la déconnexion : ".$date_fermeture."";
           //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
        } else {
           $message .= "<h1 class='gepi'>Fermeture d'une fenêtre GEES</h1>";
           $titre= "Fermeture d'une fenêtre GEES";
           /*
			$message .= "A l'heure ci-dessous, une fenêtre GEES s'est automatiquement fermée par mesure de sécurité car
           ...
           */
			$message .= "A l'heure ci-dessous, une fenêtre GEES s'est automatiquement fermée par mesure de sécurité. Le temps maximum de ".getSettingValue("sessionMaxLength")." minutes sans échange avec le serveur a sans doute été atteint.<br /><br />
           Heure et date de fermeture de la fenêtre : ".$date_fermeture;
           //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
        }
    } else {
        $session_gepi->close($_GET['auto']);
        $message .= "Votre session GEES a expiré, ou bien vous avez été déconnecté.<br />";
