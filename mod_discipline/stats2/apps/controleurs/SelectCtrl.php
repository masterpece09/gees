...
class SelectCtrl extends Controleur {
  ...
  function  __construct() {
    ...
    $this->du=isset($_POST['du'])? $_POST['du']:(isset($_GET['du'])? $_GET['du']:Gees_Date::get_date_begin_yearschool());
  ...
  private function set_stats_periode_selected() {
    if($this->id_calendrier) {
      ...
      $_SESSION['stats_periodes']=Gees_Date::get_begin_end_month($this->month_selected);
    }else {
  ...
  private function verif_date_selected() {
    ...
    if(!Gees_Date::isValid_fr($this->du)) {
      ...
      return($this->du=Gees_Date::get_date_begin_yearschool());
    }
    else if(!Gees_Date::isValid_fr($this->au)) {
    ...
    else if (Gees_Date::compare_date($this->du,$this->au)) {
      ...
      return Array($this->du=Gees_Date::get_date_begin_yearschool(),$this->au=date('d/m/Y'));
    }
  }
  private function get_debut_fin_periode($num) {
    foreach($this->periodes_calendrier as $value) {
      if ($value['id_calendrier']==$num) {
        return Array('periode'=>$num,'du'=>Gees_Date::format_date_iso_fr($value['jourdebut_calendrier']),
                'au'=>Gees_Date::format_date_iso_fr($value['jourfin_calendrier']));
      }
    }
  }
