...
// Relevés de notes cumulées
    ...
    if ($result) {    
        if(getSettingAOui('GeesAccesEvalCumulEleve')) {
...
/***** Gestion des messages *****/
	...
	(($_SESSION['statut']=='cpe')&&(getSettingAOui('GeesAccesPanneauAffichageCpe')))) {
...
/***** Tri des menus *****/
  ...
  protected function administration() {
	if ($this->statutUtilisateur == 'administrateur'){
	  ...
	  $this->creeNouveauItem('/gestion/index.php',
			  "Gestion générale",
			  "Pour accéder aux outils de gestion (sécurité, configuration générale, bases de données, initialisation de GEES).");
	  ...
	  $this->creeNouveauItem('/a_lire.php',
			"À lire",
			"Quelques fichiers concernant Gees.");
  ...
  private function saisie(){
	...
	if(getSettingAOui('active_bulletins')) {
		...
		if ((($this->test_prof_suivi != "0") and (getSettingValue("GeesRubConseilProf")=='yes'))
				or (($this->statutUtilisateur=='scolarite') and (getSettingValue("GeesRubConseilScol")=='yes'))
				or (($this->statutUtilisateur=='cpe') and ((getSettingValue("GeesRubConseilCpe")=='yes')||(getSettingValue("GeesRubConseilCpeTous")=='yes')))
	...
	$conditions_ects = ($this->gepiSettings['active_mod_ects'] == 'y' AND
		  (($this->test_prof_suivi != "0" and $this->gepiSettings['GeesAccesSaisieEctsPP'] =='yes'
			  AND $this->test_prof_suivi_ects != "0")
		  OR ($this->statutUtilisateur == 'professeur'
			  AND $this->gepiSettings['GeesAccesSaisieEctsProf'] =='yes'
			  AND $this->test_prof_ects != "0")
		  OR ($this->statutUtilisateur=='scolarite'
			  AND $this->gepiSettings['GeesAccesSaisieEctsScolarite'] =='yes'
  ...
  private function cahierTexteCPE(){
	...
	getSettingValue("active_cahiers_texte")=='y' AND (
		($this->statutUtilisateur == "cpe"
			AND getSettingValue("GeesAccesCdtCpe") == 'yes'
			AND getSettingValue("GeesAccesCdtCpeRestreint") != 'yes')
		OR ($this->statutUtilisateur == "scolarite"
			AND getSettingValue("GeesAccesCdtScol") == 'yes'
			AND getSettingValue("GeesAccesCdtScolRestreint") != 'yes')
	));
  ...
  protected function cahierTexteCPE_Restreint(){
	...
	$condition = (
	getSettingValue("active_cahiers_texte")=='y' AND (
		($this->statutUtilisateur == "cpe" AND getSettingValue("GeesAccesCdtCpeRestreint") == 'yes')
		OR ($this->statutUtilisateur == "scolarite" AND getSettingValue("GeesAccesCdtScolRestreint") == 'yes')
	));
  ...
  private function trombinoscope(){
	...
	if(($this->statutUtilisateur=='eleve')) {
	  $GeesAccesEleTrombiTousEleves=getSettingValue("GeesAccesEleTrombiTousEleves");
	  $GeesAccesEleTrombiElevesClasse=getSettingValue("GeesAccesEleTrombiElevesClasse");
	  $GeesAccesEleTrombiPersonnels=getSettingValue("GeesAccesEleTrombiPersonnels");
	  $GeesAccesEleTrombiProfsClasse=getSettingValue("GeesAccesEleTrombiProfsClasse");

	  if(($GeesAccesEleTrombiTousEleves!="yes")&&
			($GeesAccesEleTrombiElevesClasse!="yes")&&
			($GeesAccesEleTrombiPersonnels!="yes")&&
			($GeesAccesEleTrombiProfsClasse!="yes")) {
		$affiche = 'no';
	  }else {
		...
		if (($active_module_trombino_pers!='y')
				&&($GeesAccesEleTrombiTousEleves!="yes")
				&&($GeesAccesEleTrombiElevesClasse!="yes")) {
		  $affiche = 'no';
		}
	  }
  ...
  protected function releve_notes(){
	...
	$condition = ((getSettingValue("active_carnets_notes")=='y')
	  AND
        ((($this->statutUtilisateur == "scolarite") AND (getSettingValue("GeesAccesReleveScol") == "yes"))
        OR
        (
        ($this->statutUtilisateur == "professeur") AND
            (
            (getSettingValue("GeesAccesReleveProf") == "yes") OR
            (getSettingValue("GeesAccesReleveProfTousEleves") == "yes") OR
            (getSettingValue("GeesAccesReleveProfToutesClasses") == "yes") OR
            ((getSettingValue("GeesAccesReleveProfP") == "yes") AND ($this->test_prof_suivi != "0"))
            )
        )
        OR
        (($this->statutUtilisateur == "cpe") AND ((getSettingValue("GeesAccesReleveCpe") == "yes") OR (getSettingValue("GeesAccesReleveCpeTousEleves") == "yes")))));

	$condition2 = ($this->statutUtilisateur != "professeur" OR
				(
				$this->statutUtilisateur == "professeur" AND
				(
	            	(getSettingValue("GeesAccesMoyennesProf") == "yes") OR
	            	(getSettingValue("GeesAccesMoyennesProfTousEleves") == "yes") OR
	            	(getSettingValue("GeesAccesMoyennesProfToutesClasses") == "yes")
				)
				)
			);
  ...
  protected function releve_ECTS(){
	...
	$condition = ($this->gepiSettings['active_mod_ects'] == 'y'
			and ((($this->test_prof_suivi != "0")
				and ($this->gepiSettings['GeesAccesEditionDocsEctsPP'] =='yes')
				and $this->test_prof_ects != "0")
			  or (($this->statutUtilisateur=='scolarite')
				and ($this->gepiSettings['GeesAccesEditionDocsEctsScolarite'] =='yes')
				and $this->test_scol_ects != "0")
			  or ($this->statutUtilisateur=='secours')  )
			);
	...
	$recap_ects = ($this->gepiSettings['active_mod_ects'] == 'y'
					and (
					  ($this->statutUtilisateur == 'professeur'
						and $this->gepiSettings['GeesAccesRecapitulatifEctsProf'] == 'yes'
						and $this->test_prof_ects != '0')
					  or ($this->statutUtilisateur == 'scolarite'
						and $this->gepiSettings['GeesAccesRecapitulatifEctsScolarite'] == 'yes'
						and $this->test_scol_ects != '0')
					)
				  );
	if ($recap_ects)
  ...
  private function cahierTexteFamille(){
	...
	$condition = (
	getSettingValue("active_cahiers_texte")=='y' AND (
		($this->statutUtilisateur == "responsable" AND getSettingValue("GeesAccesCahierTexteParent") == 'yes')
		OR ($this->statutUtilisateur == "eleve" AND getSettingValue("GeesAccesCahierTexteEleve") == 'yes')
	));
  ...
  private function releveNotesFamille(){
   ...
   $condition = (
		getSettingValue("active_carnets_notes")=='y' AND (
			($this->statutUtilisateur == "responsable" AND getSettingValue("GeesAccesReleveParent") == 'yes')
			OR ($this->statutUtilisateur == "eleve" AND getSettingValue("GeesAccesReleveEleve") == 'yes')
			));
  ...
  private function equipePedaFamille(){
	...
	$condition = (
			($this->statutUtilisateur == "responsable"
			  AND getSettingValue("GeesAccesEquipePedaParent") == 'yes')
			OR ($this->statutUtilisateur == "eleve"
			  AND getSettingValue("GeesAccesEquipePedaEleve") == 'yes')
			);
  ...
  private function bulletinFamille(){
	...
	if(getSettingAOui('active_bulletins')) {
		$condition = (
				($this->statutUtilisateur == "responsable"
				  AND getSettingValue("GeesAccesBulletinSimpleParent") == 'yes')
				OR ($this->statutUtilisateur == "eleve"
				  AND getSettingValue("GeesAccesBulletinSimpleEleve") == 'yes')
				);
  ...
  private function graphiqueFamille(){
	...
	if(getSettingAOui('active_bulletins')) {
		$condition = (
				($this->statutUtilisateur == "responsable" AND getSettingValue("GeesAccesGraphParent") == 'yes')
				OR ($this->statutUtilisateur == "eleve" AND getSettingValue("GeesAccesGraphEleve") == 'yes')
				);
  ...
  protected function bulletins(){
	...
	if(getSettingAOui('active_bulletins')) {
		...
		if (($this->statutUtilisateur=='administrateur')&&(getSettingAOui('GeesAdminValidationCorrectionBulletins'))&&(acces('/saisie/validation_corrections.php', 'administrateur'))) {
			$afficher_correction_validation="n";
		...
		if ((($this->test_prof_suivi != "0")
				and (getSettingValue("GeesProfImprBul")=='yes'))
				or ($this->statutUtilisateur!='professeur')){
		...
		//            GeesAccesRestrAccesAppProfP : peut saisir les avis du conseil de classe pour sa classe
			if ((($this->test_prof_suivi != "0")
							AND ($this->statutUtilisateur=='professeur')
							AND (getSettingValue("GeesAccesRestrAccesAppProfP")=='yes'))
					 OR ($this->statutUtilisateur=='scolarite')
		...
		if ((($this->test_prof_suivi != "0")
						AND ($this->statutUtilisateur=='professeur')
						AND (getSettingValue("GeesProfImprBul")=='yes')
						AND (getSettingValue("GeesProfImprBulSettings")=='yes'))
				OR (($this->statutUtilisateur=='scolarite')
						AND (getSettingValue("GeesScolImprBulSettings")=='yes'))
				OR (($this->statutUtilisateur=='administrateur')
						AND (getSettingValue("GeesAdminImprBulSettings")=='yes'))
				OR (($this->statutUtilisateur=='cpe')
						AND (getSettingValue("GeesCpeImprBulSettings")=='yes'))
						) {
	...
	if(getSettingAOui('active_bulletins')) {
		if ((($this->test_prof_suivi != "0")
					AND (getSettingValue("GeesProfImprBul")=='yes'))
				OR (($this->statutUtilisateur=='cpe')
					AND (getSettingValue("GeesCpeImprBul")=='yes'))
  ...
  private function impression(){
	...
	if(getSettingAOui('active_bulletins')) {
		...
		$conditions_bulsimples = (
		    ...
		    (
				($this->statutUtilisateur == "professeur") AND
			        (
			        (getSettingValue("GeesAccesBulletinSimpleProf") == "yes") OR
			        (getSettingValue("GeesAccesBulletinSimpleProfTousEleves") == "yes") OR
			        (getSettingValue("GeesAccesBulletinSimpleProfToutesClasses") == "yes")
			        )
		    	)
		    )
		    );
	...
	if ((($this->statutUtilisateur=='cpe')&&(getSettingAOui('GeesAccesTouteFicheEleveCpe')))||
	...
	if(getSettingValue("active_cahiers_texte")=="y") {
		if(($this->statutUtilisateur=="professeur") OR
			(($this->statutUtilisateur=="cpe")&&((getSettingValue("GeesAccesCdtCpe")=="yes")||(getSettingValue("GeesAccesCdtCpeRestreint")=="yes"))) OR
			(($this->statutUtilisateur == "scolarite")&&((getSettingValue("GeesAccesCdtScol")=="yes")||(getSettingValue("GeesAccesCdtScolRestreint")=="yes")))) {
				$this->creeNouveauItem("/cahier_texte_2/see_all.php",
	...
	if(getSettingAOui('active_bulletins')) {
		if(($this->statutUtilisateur=='scolarite')||
			...
			(($this->statutUtilisateur=='cpe') AND ((getSettingAOui('GeesRubConseilCpeTous'))||(getSettingAOui('GeesRubConseilCpe'))))
		){
	...
	if(getSettingAOui('active_bulletins')) {
		...
		elseif(($this->statutUtilisateur=='professeur')&&(getSettingValue("GeesAccesBulletinSimplePP")=="yes")) {
	...
	if(($this->statutUtilisateur=='professeur')&&(getSettingValue('GeesAccesGestElevesProfP')=='yes')) {
  ...
  protected function modeleOpenOffice(){
	...
	if (getSettingValue("active_mod_ooo")=='y') {
	  ...
	  $this->creeNouveauItem("/mod_ooo/index.php",
			"Modèle Open Office",
			"Gérer les modèles Open Office dans Gees et Utiliser les formulaires de saisie");
	}
		...
		if (getSettingValue("rss_cdt_eleve") == 'y' AND $this->statutUtilisateur == "eleve") {
			...
			if (getSettingValue("rss_acces_ele") == 'direct') {
				...
				$this->canal_rss=array("lien"=>$uri_el["uri"] ,
										...
										<br />
										<br />
										<em style='font-size:small'>Avec cette URL, vous pourrez consulter les travaux à faire sans devoir vous connecter dans Gees.<br />Firefox, Internet Explorer,... savent lire les flux RSS.<br />Il existe également des lecteurs de flux RSS pour les SmartPhone,...</em>");
			}
...
//insert into setting set name='active_mod_epreuve_blanche', value='y';
  ...
  protected function cahierTexte_Visa(){
	...
	if (getSettingValue("GeesAccesCdtVisa")=='yes') {
