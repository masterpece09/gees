...
class infos{
	...
	function versionPhp(){
		...
		if ($version == 5) {
			$retour = '<span style="color: green;">'.phpversion().' (Gees nécessite php 5.2.x minimum)</span>';
		}elseif($version == 4 AND mb_substr($test, 2, 2) >= 3){
			$retour = '<span style="color: green;">'.phpversion().'(Attention, Gees ne fonctionne pas avec cette version, elle est trop ancienne)</span>';
		}else{
