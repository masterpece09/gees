					...
					if(mysqli_num_rows($res_jec)==0){
						...
						if(mysqli_num_rows($res_class_test)==0){
							...
							if(mysqli_num_rows($res_class)!=0){
								...
								$info="<br />\n";
								$info.="Si <b>$lig_ele->login</b> n'est pas dans une de ces classes, il faudrait l'affecter dans la classe sur une période au moins pour pouvoir supprimer son appartenance à ces groupes, ou procéder à un nettoyage des tables de la base GEES.";
								$info.="</p>\n";
								echo $info;
								$chaine_rapport.=$info;
							}
						...
						if(mysqli_num_rows($res_jec)==1){
							...
							while($lig_grp=mysqli_fetch_object($res_jeg)){
								...
								if(mysqli_num_rows($res_test_grp_clas)==0){
									...
									$info="il faudra effectuer un <a href='clean_tables.php?maj=9'>nettoyage des tables de la base de données GEES</a> (<i>après une <a href='../gestion/accueil_sauve.php?action=dump' target='blank'>sauvegarde de la base</a></i>).<br />\n";
									$info.="</p>\n";
									echo $info;
									$chaine_rapport.=$info;
