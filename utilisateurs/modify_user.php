	...
	if ($_POST['reg_nom'] == '')  {
				...
				if ($resultat != "NON") {
					...
					if (LDAPServer::is_setup() && $gepiSettings["ldap_write_access"] == "yes" && ($session_gepi->auth_ldap || $session_gepi->auth_sso) && ($_POST['reg_auth_mode'] == 'ldap' || $_POST['reg_auth_mode'] == 'sso')) {
						...
						if ($ldap_server->test_user($_POST['new_login'])) {
							...
							$msg.= "L'utilisateur n'a pas pu être ajouté à l'annuaire LDAP, car il y est déjà présent. Il va néanmoins être créé dans la base Gees.";
						} else {
		...
		//
			...
			if ($old_auth_mode == "gepi" && ($_POST['reg_auth_mode'] == "ldap" || $_POST['reg_auth_mode'] == "sso")) {
				// On passe du mode Gees à un mode externe : il faut supprimer le mot de passe
				...
				// On passe d'un mode externe à un mode Gees. On prévient l'admin qu'il faut modifier le mot de passe.
...
$themessage2 = "Êtes-vous sûr de vouloir effectuer cette opération ?\\n Actuellement cet utilisateur se connecte à GEES en s\'authentifiant auprès d\'un SSO.\\n En attribuant un mot de passe, vous lancerez la procédure, qui génèrera un mot de passe local. Cet utilisateur ne pourra donc plus se connecter à GEES via le SSO mais uniquement localement.";
//**************** EN-TETE *****************
...
if ($ldap_write_access) {
	echo "<p><strong><span style='color: red;'>Attention !</strong> Un accès LDAP en écriture a été défini.
			...
			Si l'utilisateur existe déjà dans l'annuaire LDAP, ses informations ne seront pas mises à jour dans l'annuaire mais il sera tout de même créé dans la base Gees.
			En cas de modification d'un utilisateur existant à la fois dans l'annuaire et dans Gees, les modifications seront répercutées sur l'annuaire LDAP.</strong></p>";
}
?>
...
if (!$session_gepi->auth_ldap || !$session_gepi->auth_sso) {
	...
	if (!$session_gepi->auth_ldap && !$session_gepi->auth_sso) {
		$remarque .= "les modes d'authentification LDAP et SSO sont actuellement inactifs. Si vous choisissez l'un de ces modes, l'utilisateur ne disposera d'aucun moyen de s'authentifier dans Gees.</em></p>";
	} else {
		...
		$remarque .= "est actuellement inactive. Si vous choisissez ce mode d'authentification, l'utilisateur ne disposera d'aucun moyen de s'authentifier dans Gees.</em></p>";
	}
	echo $remarque;
}
...
<option value='gepi' <?php if ($user_auth_mode=='gepi') echo " selected ";  ?>>Locale (base Gees)</option>
