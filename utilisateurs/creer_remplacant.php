...
if ($valid!='yes') {
	...
	echo "<table>\n";
	echo "<tr><td>Identifiant : </td><td><input type=text name='user_login' size=20 /> <span title='Laisser vide pour un identifiant (login) généré par Gees.'>(*)</span></td></tr>\n";
	...
	if (!(isset($user_login)) or ($user_login=='')) {
		if (getSettingValue("use_sso") == "lcs") {
			...
			echo "<br /><i>Un utilisateur local doit systématiquement s'identifier sur GEES avec le mot de passe ci-dessous, même s'il est un utilisateur authentifié par LCS.</i>";
				...
				<tr>
					...
					<td>
						<select id='select_auth_mode' name='reg_auth_mode' size='1'>
							<option value='gepi' selected>Locale (base Gees)</option>
