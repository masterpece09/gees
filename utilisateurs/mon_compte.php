...
if ((isset($_POST['valid'])) and ($_POST['valid'] == "yes"))  {
	...
	*/
	if((getSettingValue("active_module_trombino_pers")=='y')&&
		((($_SESSION['statut']=='administrateur')&&(getSettingValue("GeesAccesModifMaPhotoAdministrateur")=='yes'))||
		(($_SESSION['statut']=='scolarite')&&(getSettingValue("GeesAccesModifMaPhotoScolarite")=='yes'))||
		(($_SESSION['statut']=='cpe')&&(getSettingValue("GeesAccesModifMaPhotoCpe")=='yes'))||
		(($_SESSION['statut']=='professeur')&&(getSettingValue("GeesAccesModifMaPhotoProfesseur")=='yes')))) {
	...
	elseif(($_SESSION['statut']=='eleve')&&(getSettingValue("active_module_trombinoscopes")=='y')&&(getSettingValue("GeesAccesModifMaPhotoEleve")=='yes')) {
...
($_SESSION['statut']=='eleve')
	...
	if((($_SESSION['statut']=='eleve')&&(getSettingValue("active_module_trombinoscopes")=='y'))||
		...
		$GeesAccesModifMaPhoto='GeesAccesModifMaPhoto'.ucfirst(mb_strtolower($_SESSION['statut']));
		...
		if($_SESSION['statut']=='eleve') {
			...
			if($reg_no_gep!="") {
				...
				if ((getSettingValue("GeesAccesModifMaPhotoEleve")=='yes') and ($_SESSION['statut']=='eleve')) {
				...
				if ((getSettingValue($GeesAccesModifMaPhoto)=='yes') and ($test_eleve!=0)) {
					$affiche_bouton_submit ='yes';
					echo "<div>\n";
			...
			echo "<td>\n";*/
				...
				if(getSettingValue($GeesAccesModifMaPhoto)=='yes') {
					$affiche_bouton_submit ='yes';
					echo "<div>\n";
...
?>
<p class='small'>
    * Toutes les données nominatives présentes dans la base GEES et vous concernant vous sont communiquées sur cette page.
    <br />
...
<p class='small'>
    ** Votre email sera affichée sur certaines pages seulement si leur affichage a été activé de manière globale par l'administrateur 
    ...
    et/ou leurs responsables légaux disposant d'un identifiant pour se connecter à Gees.
</p>
<?php 
}
...
<p>Gees permet d'afficher si vous le souhaitez un témoin indiquant que de nouveaux incidents ont été saisis depuis votre dernière connexion.</p>\n";
	if($_SESSION['statut']=='professeur') {
		$checked="";
...
if(in_array($_SESSION['statut'], $tab_statuts_barre)) {
	...
	echo "<p style='text-indent:-4em; margin-left:4em;'><em>NOTE&nbsp;:</em> Ce paramétrage concerne les fichiers produits par Gees et proposés au téléchargement.<br />Cela ne concerne pas les fichiers que vous uploadez/envoyez vers Gees.</p>\n";
