...
while ($p < $nb_users) {
	...
	if ($cas_traite!=0) {
		...
		if(($liste_elv_resp=="")&&(getSettingAOui('GeesMemesDroitsRespNonLegaux'))) {
			$sql_resp_eleves="SELECT DISTINCT c.id, e.* , c.*
								FROM responsables2 r2, eleves e, classes c, j_eleves_classes jec
								WHERE (
								r2.pers_id = '$resp_pers_id'
...
width:".$largeur1."%;\n";
					...
					if ($user_auth_mode != "gepi" && $gepiSettings['ldap_write_access'] != 'yes') {
						// En mode SSO ou LDAP sans accès en écriture, le mot de passe n'est pas modifiable par Gees.
						echo "<tr><td>Le mot de passe de cet utilisateur n'est pas géré par Gees.</td></tr>\n";
						$texte_email.="Le mot de passe de cet utilisateur n'est pas géré par Gees.\n";
					}
					else {
