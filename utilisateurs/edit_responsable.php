				...
				if ($test > 0) {
					...
					if ($_POST['reg_auth_mode'] != $old_auth_mode) {
						...
						if ($old_auth_mode == "gepi" && ($_POST['reg_auth_mode'] == "ldap" || $_POST['reg_auth_mode'] == "sso")) {
							...
							// Passage au mode Gees, rien de spécial à faire, si ce n'est annoncer à l'administrateur
...
<?php
if (mysqli_num_rows($quels_parents) == "0") {
	echo "<p>Pour créer de nouveaux comptes d'accès associés aux responsables d'élèves définis dans Gees, vous devez cliquer sur le lien 'Ajouter de nouveaux comptes' ci-dessus.</p>";
}
