...
class Session {
	...
	private $etat = false; 	# actif/inactif. Utilisé simplement en interne pour vérifier que
							# l'utilisateur authentifié de source externe est bien actif dans Gees.
  ...
  private $cas_extra_attributes = false; # D'éventuels attributs chargés depuis la réponse CAS
	...
	# 7 : l'administrateur a désactivé les connexions à Gees.
	...
	public function authenticate($_login = null, $_password = null) {
		...
		switch ($auth_mode) {
			case "gepi":
			  # Authentification locale sur la base de données Gees
...
*/
    ...
    private function test_loginsso()
  {
	...
	public function logout_cas() {
		...
		if ($cas_use_logout) {
			...
			if ($cas_logout_url != '') {
				...
				echo '<html><head><title>GEES</title></head><body><h2>Vous &ecirc;tes d&eacute;connect&eacute;.</h2></body></html>';
				exit();
			}
    ...
    $sql_login = "SELECT login FROM utilisateurs WHERE (login = '".$_login."')";
		...
		if ($test_login != "0") {
            ...
            tentative_intrusion(1, "Tentative de connexion avec un login incorrect (n'existe pas dans la base Gees). Ce peut être simplement une faute de frappe. Cette alerte n'est significative qu'en cas de répétition. (login utilisé : ".$_login.")");
          ...
          for ($i=0;$i<$nb_eleves_a_charge;$i++) {
              ...
              $eleve_sql = "SELECT ele_id FROM eleves WHERE login = '$eleve_associe_login'";
                    ...
                    if ($eleve_query->num_rows == 1) {
                        ...
                        // Gees donne un ordre aux responsables, il faut donc verifier combien de responsables sont deja enregistres pour l'eleve
  ...
  private function update_user_with_cas_attributes(){
    ...
    if (!empty($this->cas_extra_attributes)) {
      ...
      foreach($this->cas_extra_attributes as $attribute => $value) {				
				// On compare la valeur envoyée avec la valeur présente dans Gees
  ...
  # Renvoie 'true' si l'accès à Gees se fait en https
