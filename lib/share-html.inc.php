					...
					if(($vtn_coloriser_resultats=='y')&&($j>=$num_debut_colonnes_matieres)&&($i>=$num_debut_lignes_eleves)) {
						if(mb_strlen(preg_replace('/[0-9.,]/','',$col[$j][$i]))==0) {
							for($loop=0;$loop<count($vtn_borne_couleur);$loop++) {
                                ...
                                $eleve_moyenne["moyenne"][$i] =  $col[$j][$i];
				echo ">{$col[$j][$i]}</td>\n";
...
function make_classes_select_html($link, $current, $year, $month, $day, $domaine='')
{
  ...
  if (isset($_SESSION['statut']) && ($_SESSION['statut']=='scolarite'
		  && getSettingValue('GeesAccesCdtScolRestreint')=="yes")) {
  $sql = "SELECT DISTINCT c.id, c.classe
	...
	ORDER BY classe ;";
		  ...
		  && getSettingValue('GeesAccesCdtCpeRestreint')=="yes")) {
	$sql = "SELECT DISTINCT c.id, c.classe
	...
	if(isset($_SESSION['statut']) && ($_SESSION['statut']=='professeur')&&(!getSettingAOui('GeesAccesCDTToutesClasses'))) {
		$sql = "SELECT DISTINCT c.id, c.classe
 ...
 *
 * Par défaut les dossiers nécessaires à GEES
 * 
...
function alerte_config_suhosin() {
	...
	if($suhosin_post_max_totalname_length!='') {
		...
		$retour.="<p>Le module suhosin n'est pas activé.<br />Il ne peut pas perturber Gees.</p>\n";
	}

	$max_input_vars=ini_get('max_input_vars');
	if(($max_input_vars!="")&&($max_input_vars>0)) {
		$retour.="<p class='bold'>Limitation propre à PHP&nbsp;:</p><p>La variable PHP 'max_input_vars' est limitée à $max_input_vars.<br />Si le nombre de variables transmises dépasse cette valeur, Gees peut être perturbé.<br />Tentez de soumettre moins de valeurs ou augmentez la valeur de 'max_input_vars' dans le fichier php.ini.</p>\n";
	}
...
Avec cette URL, vous pourrez consulter les travaux à faire sans devoir vous connecter dans Gees.
...
function affiche_infos_adresse_et_tel($login_user, $tab_user=array()) {
	...
	<tbody>
		<tr>
			...
			<td>".(isset($tab_user['email']) ? "<a href='mailto:".$tab_user['email']."?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail')."[GEES]")."' title='Envoyer un mail'>".$tab_user['email']."</a>" : "")."</td>
