...
// On affiche tout le temps brainjar sauf quand on dit à Gees de ne pas le faire
...
//=== statut utilisateur ===
	...
	*/
	...
	if (isset($_SESSION['statut'])) {
		if ($_SESSION['statut'] == 'administrateur') {
			$tbs_deux_menu[]=array("lien"=>"http://gepi.mutualibre.org" , "onclick"=> "onclick=\"window.open(this.href, '_blank'); return false;\""  , "texte"=>"Visiter le site de GEES");
		}else{
			if (getSettingValue("contact_admin_mailto")=='y') {
				$gepiAdminAdress=getSettingValue("gepiAdminAdress");
				$tmp_date=getdate();
				$lien="mailto:$gepiAdminAdress?Subject=Gees&amp;body=";
	...
	$tbs_deux_menu[]=array("lien"=>"$gepiPath/gestion/info_vie_privee.php" , "onclick"=> "onclick=\"centrerpopup('$gepiPath/gestion/info_vie_privee.php',600,480,'scrollbars=yes,statusbar=no,resizable=yes'); return false;\""  , "texte"=>"Vie privée");
		 ...
		 //=== Affichage de la version de Gees ===
		 //=== Affichage de la version de Gees ===
