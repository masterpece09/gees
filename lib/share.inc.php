...
function envoi_mail($sujet, $message, $destinataire, $ajout_headers='', $plain_ou_html="plain", $tab_param_mail=array()) {
	...
	$subject = $gepiPrefixeSujetMail."GEES : $sujet";
	...
	if((getSettingAOui('utiliser_phpmailer'))&&(getSettingValue('phpmailer_smtp_host')!="")&&(getSettingValue('phpmailer_smtp_port')!="")&&(getSettingValue('phpmailer_from')!="")) {
		...
		*/
		...
		if((isset($tab_param_mail['from']))&&(check_mail($tab_param_mail['from']))) {
			...
			$mail->FromName = 'Mail automatique Gees';
		}
		...
		*/
		...
		$headers .= "From: Mail automatique Gees <ne-pas-repondre@".$_SERVER['SERVER_NAME'].">\r\n";
 ...
 * 
 * name9_p          à partir du nom, réduit à 9 caractères + _ + première lettre du prénom (format historique du login élève dans Gees)
 * 
...
function generate_unique_login_old($_nom, $_prenom, $_mode, $_casse='') {
	...
	if ($_mode == "name") {
		...
		// Format d'origine des comptes élèves dans Gees
		$temp1 = $_nom;
		$temp1 = preg_replace("/[ '-]/","", $temp1);
 ...
 * d'utilisation illégale de Gees est manifestement avérée.
...
function tentative_intrusion($_niveau, $_description, $_login_a_enregistrer="") {
	...
	// Si la tentative d'intrusion a été effectuée par un utilisateur connecté à Gees,
	...
	if ($send_email) {
		$message = "** Alerte automatique sécurité Gees **\n\n";
		$message .= "Une nouvelle tentative d'intrusion a été détectée par Gees. Les détails suivants ont été enregistrés dans la base de données :\n\n";
		$message .= "Date : ".$date."\n";
		...
		if ($user_login == "-") {
			$message .= "La tentative d'intrusion a été effectuée par un utilisateur non connecté à Gees.\n";
		...
		$subject = $gepiPrefixeSujetMail."GEES : Alerte sécurité -- Tentative d'intrusion";
		$subject = "=?UTF-8?B?".base64_encode($subject)."?=\r\n";
 ...
 * 
 * Test le dossier en écriture et le crée au besoin
 * La fonction est appelée depuis la racine de l'arborescence GEES (sinon ça peut bugger)
 *
		...
		if ($res_user->num_rows > 0) {
			$lig_user = $res_user->fetch_object();
			 if(check_mail($lig_user->email)) {
				...
				$message = "** Mail connexion Gees **\n\n";
				$message .= "\n";
				$message .= "Vous (*) vous êtes connecté à GEES :\n\n";
...
function liste_prof_suivi($id_classe, $pour_qui="", $avec_lien_mail="") {
	...
	for($loop=0;$loop<count($tab);$loop++) {
		...
		if($mail_user!="") {
			if($pour_qui=="profs") {
				$retour.="<a href='mailto:$mail_user?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail'))."[GEES]: ...' title=\"Envoyer un mail.\">".civ_nom_prenom($tab[$loop])."</a>";
			}
			else {
				$retour.="<a href='mailto:$mail_user?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail'))."[GEES]: ...' title=\"Envoyer un mail.\">".affiche_utilisateur($tab[$loop], $id_classe)."</a>";
			}
		}
		else {
			if($pour_qui=="profs") {
 ...
 * @param int $niveau_arbo Niveau dans l'arborescence GEES
	...
	if(getSettingAOui('sso_cas_table')) {
		...
		if($test->num_rows > 0) {
			...
			if($test2->num_rows == 0) {
				...
				if($lig->login_sso=="") {
					$retour.="<img src='".$gepiPath."/images/icons/sens_interdit.png' width='16' height='16' alt=\"Correspondance SSO vide\" title=\"La correspondance SSO est vide dans la table 'sso_table_correspondance' pour ce compte Gees.\" />";
				}
				else {
...
function acces_cdt_eleve($login_user, $login_eleve) {
	...
	if($res->num_rows > 0) {
		...
		if(($statut=="eleve")&&($login_user==$login_eleve)&&(getSettingAOui('GeesAccesCahierTexteEleve'))) {
			$retour=true;
		}
		elseif(($statut=="responsable")&&(getSettingAOui('GeesAccesCahierTexteParent'))&&(getSettingAOui('GeesMemesDroitsRespNonLegaux'))&&(is_responsable($login_eleve, $login_user, "", "yy"))) {
			$retour=true;
		}
		elseif(($statut=="responsable")&&(getSettingAOui('GeesAccesCahierTexteParent'))&&(is_responsable($login_eleve, $login_user))) {
			$retour=true;
		}
		elseif(($statut=="professeur")&&(getSettingAOui('GeesAccesCDTToutesClasses'))) {
			$retour=true;
		}
		...
		elseif(($statut=="cpe")&&(getSettingAOui('GeesAccesCdtCpe'))) {
			$retour=true;
		}
		elseif(($statut=="cpe")&&(getSettingAOui('GeesAccesCdtCpeRestreint'))) {
		...
		elseif(($statut=="scolarite")&&(getSettingAOui('GeesAccesCdtScol'))) {
			$retour=true;
		}
		elseif(($statut=="scolarite")&&(getSettingAOui('GeesAccesCdtScolRestreint'))) {
...
function acces_saisie_avertissement_fin_periode($login_ele) {
	...
	if(getSettingValue('mod_disc_acces_avertissements')=="n") {
		...
		elseif($_SESSION['statut']=='scolarite') {
			if(getSettingAOui('GeesRubConseilScol')) {
				return true;
			}
		}
		elseif($_SESSION['statut']=='cpe') {
	...
	if(getSettingValue('mod_disc_acces_avertissements')=="n") {
		...
		if(acces('/mod_discipline/imprimer_bilan_periode.php', $_SESSION['statut'])) {
			...
			elseif($_SESSION['statut']=='cpe') {
				...
				/*
				elseif((!getSettingAOui('GeesRubConseilCpe'))&&(is_cpe($_SESSION['login'], "", $login_ele))) {
					return true;
				}
				*/
				else {
...
function affiche_lien_mailto($mail_user, $info_user) {
	$retour=" <a href='mailto:".$mail_user."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES&amp;body=";
	$tmp_date=getdate();
...
function acces_impression_bulletin($login_eleve, $id_classe="") {
	...
	if(($_SESSION['statut']=='professeur')&&(getSettingAOui('GeesProfImprBul'))&&(is_pp($_SESSION['login']))) {
		if($login_eleve!="") {
	...
	elseif(($_SESSION['statut']=='cpe')&&(getSettingAOui('GeesCpeImprBul'))) {
...
function acces_impression_releve_notes($login_eleve, $id_classe="") {
	...
	if(($_SESSION['statut']=='professeur')&&(getSettingAOui('GeesAccesReleveProfToutesClasses'))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='professeur')&&($login_eleve!="")&&(getSettingAOui('GeesAccesReleveProf'))&&(is_prof_ele($_SESSION['login'], $login_eleve, "", $id_classe))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='professeur')&&($login_eleve!="")&&(getSettingAOui('GeesAccesReleveProfTousEleves'))&&(is_prof_classe_ele($_SESSION['login'], $login_eleve))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='professeur')&&($id_classe!="")&&(getSettingAOui('GeesAccesReleveProfTousEleves'))&&(is_prof_classe($_SESSION['login'], $id_classe))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='professeur')&&($id_classe!="")&&(getSettingAOui('GeesAccesReleveProfP'))&&(is_pp($_SESSION['login'], $id_classe))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='professeur')&&($login_eleve!="")&&(getSettingAOui('GeesAccesReleveProfP'))&&(is_pp($_SESSION['login']))) {
		// PP: Le test est fait sur l'association avec la classe (même si l'élève a changé de classe en cours d'année)
		//     On ne se contente pas de is_pp(is_pp($_SESSION['login'], '', $login_ele)
		$sql="SELECT DISTINCT jec.id_classe FROM j_eleves_classes jec WHERE jec.login='$login_eleve';";
		$res=mysqli_query($GLOBALS["mysqli"], $sql);
	...
	elseif(($_SESSION['statut']=='cpe')&&(getSettingAOui('GeesAccesReleveCpeTousEleves'))) {
		$retour=true;
	}
	elseif(($_SESSION['statut']=='cpe')&&(getSettingAOui('GeesAccesReleveCpe'))) {
		if(is_cpe($_SESSION['login'], "", $login_eleve)) {
			$retour=true;
		}
	}
...
function renseigner_tab_rn($tab_id_classe) {
	...
	if((($_SESSION['statut']!='eleve')&&($_SESSION['statut']!='responsable'))||
		(($_SESSION['statut']=='eleve')&&(getSettingAOui('GeesAccesColMoyReleveEleve')))||
		(($_SESSION['statut']=='responsable')&&(getSettingAOui('GeesAccesColMoyReleveParent')))
		) {
		$tab_item[]='rn_col_moy';
	}
...
/** Fonction destinée à corriger des chemins du type https://NOM_SERVEUR/CHEMIN_GEES/documents/cl1234/XXX en ../documents/cl1234/XXX
 *
 * @param string $texte Le texte à traiter
...
/** Fonction destinée à retourner un tableau des fichiers présents dans le dossier de sauvegarde de Gees (backup\$backup_directory)
 *
...
function retourne_sql_mes_classes() {
	if($_SESSION['statut']=='cpe'){
		if(getSettingAOui('GeesAccesTouteFicheEleveCpe')) {
...
function acces_messagerie($statut_user) {
	if(!acces("/messagerie/index.php", $statut_user)) {
		...
		if(($_SESSION['statut']=='cpe')&&(!getSettingAOui('GeesAccesPanneauAffichageCpe'))) {
			return false;
		}
		else {
			return true;
...
function acces_carnet_notes($statut_user) {
	...
	elseif($_SESSION['statut']=='responsable') {
		if(getSettingAOui('GeesAccesReleveParent')) {
			return true;
		}
		else {
			return false;
	...
	elseif($_SESSION['statut']=='eleve') {
		if(getSettingAOui('GeesAccesReleveEleve')) {
			return true;
		}
		else {
			return false;
...
function acces_moyenne_chaque_devoir_carnet_notes($statut_user) {
	if($_SESSION['statut']=='responsable') {
		if(getSettingAOui('GeesAccesMoyClasseReleveParent')) {
			return true;
		}
		else {
			return false;
	...
	elseif($_SESSION['statut']=='eleve') {
		if(getSettingAOui('GeesAccesMoyClasseReleveEleve')) {
			return true;
		}
		else {
			return false;
...
function acces_moyenne_min_max_chaque_devoir_carnet_notes($statut_user) {
	if($_SESSION['statut']=='responsable') {
		if(getSettingAOui('GeesAccesMoyMinClasseMaxReleveParent')) {
			return true;
		}
		else {
			return false;
	...
	elseif($_SESSION['statut']=='eleve') {
		if(getSettingAOui('GeesAccesMoyMinClasseMaxReleveEleve')) {
			return true;
		}
		else {
			return false;
...
/** Fonction destinée à retourner un message informant l'utilisateur que la version de Gees a été mise à jour
 *
...
<p>Gees a été mis à jour en version <strong>".$gepiVersion."</strong></p>
...
Consultez la liste des droits dans <a href='$gepiPath/gestion/droits_acces.php'>Droits d'accès</a> pour adapter le comportement de Gees aux préférences de l'établissement.</p>
