...
secours='F',
autre='F',
description='Rapprochement des comptes ENT/GEES : ENT ITOP',
statut='';";
...
if(isset($_POST['enregistrement_eleves'])) {
		...
		for($loop=0;$loop<count($ligne);$loop++) {
			...
			if((isset($_POST['login_'.$ligne[$loop]]))&&($_POST['login_'.$ligne[$loop]]!='')) {
				if(isset($ligne_tempo2[$ligne[$loop]])) {
					...
					$test=mysqli_query($GLOBALS["mysqli"], $sql);
					if(mysqli_num_rows($test)==0) {
						$msg.="Le login élève Gees ".$_POST['login_'.$ligne[$loop]]." proposé n'existe pas.<br />\n";
					}
					else {
	...
	if($nb_pas_dans_eleves>0) {
		$msg.="<br />$nb_pas_dans_eleves comptes de l'ENT n'ont pas été trouvés dans la table 'eleves' de Gees.<br />Sont-ce des élèves de l'année précédente ?<br />\n";
	}
				...
				for($loop=0;$loop<count($ligne);$loop++) {
					...
					if((isset($_POST['login_'.$ligne[$loop]]))&&($_POST['login_'.$ligne[$loop]]!='')) {
						if(isset($ligne_tempo2[$ligne[$loop]])) {
							...
							$test=mysqli_query($GLOBALS["mysqli"], $sql);
							if(mysqli_num_rows($test)==0) {
								$msg.="Le login responsable Gees ".$_POST['login_'.$ligne[$loop]]." proposé n'existe pas.<br />\n";
							}
							else {
		...
		for($loop=0;$loop<count($ligne);$loop++) {
			...
			if((isset($_POST['login_'.$ligne[$loop]]))&&($_POST['login_'.$ligne[$loop]]!='')) {
				if(isset($ligne_tempo2[$ligne[$loop]])) {
					...
					$test=mysqli_query($GLOBALS["mysqli"], $sql);
					if(mysqli_num_rows($test)==0) {
						$msg.="Le login de personnel Gees ".$_POST['login_'.$ligne[$loop]]." proposé n'existe pas.<br />\n";
					}
					else {
						$sql="SELECT * FROM sso_table_correspondance WHERE login_sso='".mysqli_real_escape_string($GLOBALS["mysqli"], $tab[0])."';";
						$test=mysqli_query($GLOBALS["mysqli"], $sql);
...
<h2>Rapprochement des comptes ENT ITOP/GEES</h2>
...
<p>Cette rubrique permet de fournir les fichiers CSV de rénitialisation de mots de passe générés par l'ENT, ou les CSV des nouveaux élèves.</p>";
	...
	if($nb_scories>0) {
		echo "
<br />
<p style='text-indent:-6em; margin-left:6em;'><strong style='color:red;'>SCORIES&nbsp;:</strong> ".$nb_scories." association(s) existent dans la table 'sso_table_correspondance' pour des login qui n'existent plus dans Gees.<br />
Ces scories peuvent perturber l'association GUID_ENT/Login_GEES.<br />
...
<form action='".$_SERVER['PHP_SELF']."' method='post'>
	<fieldset class='fieldset_opacite50'>
		<p style='text-indent:-6em; margin-left:6em;'><strong style='color:red;'>ANOMALIE&nbsp;:</strong> Vous avez ".$nb_scories." identifiant(s) ENT associés à plusieurs logins Gees.<br />Cela ne devrait pas arriver.<br />
...
<p>Il manque $nb_assoc_manquantes_pers association(s) personnel(s)&nbsp;: ";
		...
		if(($nb_assoc_manquantes_ele==0)&&($nb_assoc_manquantes_resp==0)&&($nb_assoc_manquantes_pers==0)) {
			echo "<br /><p>Les utilisateurs, disposant d'un compte dans Gees avec mode d'authentification SSO, ont tous une association dans la table 'sso_table_correspondance'.</p>";
		}
		else {
			echo "<br /><p>Ces utilisateurs disposent d'un compte dans Gees, mais n'ont pas d'association SSO.<br />
...
<div style='margin-left:4em;'>
	<p>Si l'accès SSO de l'ENT vers Gees tarde à être mis en place, vous pouvez ouvrir l'accès aux parents en limitant les difficultés&nbsp;:<br />
	Il s'agit de créer des comptes dans Gees avec les logins et mots de passe proposés par l'ENT.<br />
	Les parents auront donc les mêmes comptes et mots de passe initiaux dans l'ENT et dans Gees<br />(<em>s'ils changent leur mot de passe d'un côté ou de l'autre, la synchronisation des mots de passe n'est pas assurée</em>)</p>
	<p>L'authentification des parents sera locale (<em>sur la base GEES et non en SSO CAS</em>).</p>
...
<div style='margin-left:4em;'>
	<p>Si l'accès SSO de l'ENT vers Gees pose parfois des problèmes, il est possible d'activer une fonctionnalité dans <a href='../gestion/options_connexct.php'>Options de connexion</a> pour conserver des mots de passe dans la base Gees bien que les comptes soient en mode d'authentification SSO.<br />
	(<em>par défaut, en mode SSO, il n'y a plus de mot de passe dans la base Gees pour l'utilisateur; l'authentification est effectuée côté ENT</em>)</p>
	<p>Vous disposez alors d'un accès de secours pour ces utilisateurs.</p>
	<p>Si vous conservez tout de même des mots de passe dans Gees pour les élèves, vous pouvez forcer ici les mots de passe d'après un fichier CSV au format&nbsp;:<br />
	&nbsp;&nbsp;&nbsp;&nbsp;LOGIN_GEES;MOT_DE_PASSE_GEES<br />
	...
	(<em>ATTENTION&nbsp;: en cas de changement de mot de passe dans Gees ou dans l'ENT, il n'y aura pas de synchronisation automatique des mots de passe</em>).</p>
	...
	<form action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'>
		<fieldset style='border: 1px solid grey; background-image: url(\"../images/background/opacite50.png\");'>
			".add_token_field()."
			<p>Fournir le fichier CSV des comptes et mots de passe Gees.<br />
			</p>
	...
	<ul>
		...
		&nbsp;&nbsp;&nbsp;&nbsp;LOGIN_GEES;MOT_DE_PASSE_GEES</li>
		...
		Les élèves qui ont déjà un mot de passe dans Gees seront signalés (<em>en revanche, le mot de passe en lui-même ne sera pas affiché</em>).</li>
		<li>Seuls les élèves qui disposent d'un compte utilisateur seront affichés.</li>
		<li><strong>ATTENTION&nbsp;:</strong> Il est ici question d'<strong>imposer des mots de passe</strong> dans Gees pour un accès de secours élève.<br />Les <strong>logins sont inchangés</strong> et il est peu probable que les logins Gees et les logins ENT coïncident.<br />Cela ne sera pas modifié par le présent dispositif.</li>
	</ul>
...
<form action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'>
	...
	<table border='0'>
		<tr><th style='text-align:left;'>Login Gees&nbsp;:</th><td><input type=\"text\" name=\"login_gepi\" size=\"50\" value=\"".(isset($_GET['login_gepi']) ? $_GET['login_gepi'] : "")."\" /></td></tr>
...
<ul>
	...
	La deuxième pour voir quels élèves du CSV n'ont pas été associés à un login Gees.</li>
</ul>\n";
	}
	else {
		check_token(false);
...
","",32,0,"y","y","n","n");
		...
		echo "
<h2>Rapprochement des comptes élèves ENT ITOP/GEES</h2>
...
<h2>Rapprochement actuels des comptes élèves ENT ITOP/GEES</h2>
";
...
<form action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'>
	...
	<table class='boireaus'>
		<tr>
			...
			<th colspan='6'>Informations GEES</th>
...
";
	...
	while($lig=mysqli_fetch_object($res)) {
		...
		$test_u=mysqli_query($GLOBALS["mysqli"], $sql);
		if(mysqli_num_rows($test_u)==0) {
			echo "<a href='../utilisateurs/create_eleve.php?filtrage=Afficher&amp;critere_recherche=".preg_replace("/[^A-Za-z]/", "%", $lig->nom)."&amp;afficher_tous_les_eleves=n' title='Pas de compte utilisateur dans Gees pour cet élève.
...
if($mode=="import_responsables") {
 ...
 | <a href='".$_SERVER['PHP_SELF']."'>Index rapprochement ENT ITOP</a>
	...
	if(!isset($csv_file)) {
		echo "
<h2>Rapprochement des comptes responsables ENT ITOP/GEES</h2>
			...
			<a href='#' onClick=\"document.getElementById('nom_rech').value=document.getElementById('nom_$cpt').innerHTML;
				...
				<img src='../images/icons/chercher.png' width='16' height='16' alt='Chercher' title='Effectuer une recherche' />
							...
							if(array_key_exists($tab_resp[0]['login'], $tab_guid_associe_a_un_login)) {
								echo "<img src='../images/icons/ico_attention.png' width='22' height='19' title=\"Ce login est associé à un autre GUID ENT\n".$tab_guid_associe_a_un_login[$tab_resp[0]['login']]['login_sso']."\net concerne l'utilisateur Gees ".$tab_guid_associe_a_un_login[$tab_resp[0]['login']]['info']."\" />";
							}
...
<h2>Rapprochement actuels des comptes responsables ENT ITOP/GEES</h2>
";
...
<form action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'>
	...
	<table class='boireaus'>
		<tr>
			...
			<th>Informations ENT</th>
			<th colspan='5'>Informations GEES</th>
			<th rowspan='2'>Corriger</th>
		</tr>
...
","",32,0,"y","y","n","n");
		...
		echo "
<h2>Rapprochement des comptes de personnels ENT ITOP/GEES</h2>
...
<h2>Rapprochement actuels des comptes de personnels ENT ITOP/GEES</h2>
";
...
<ul>
	...
	<!--
		Ne seront pris en compte que les comptes ENT pour lesquels le rapprochement avec un compte Gees est effectué.
		Ce n'est pas le cas.
...
Vous seriez-vous trompé de fichier&nbsp;?</span>";
	...
	<tr>
		...
		<td>".$tab[$tabindice['Mot de passe']]."</td>
						...
						echo "
	<tr>
		<th style='text-align:left;'>Email Gees</th>
		<th>: </th>
		<td>".$tab[4]."</td>
	</tr>";
						*/
...
<table class='boireaus boireaus_alt' summary='Tableau des responsables'>
	...
	<tr>
		...
		<th>Nom prénom</th>
		<th title=\"Existence ou non d'un compte dans la base GEES\">C.</th>
		<th>Adresse</th>
		<th>Enfants</th>
	</tr>";
		$cpt=0;
	...
	<p>
		...
		<input type='radio' name='auth_mode' id='auth_mode_gepi' value='gepi' checked /><label for='auth_mode_gepi'>base Gees</label> - 
	...
	<p><em>NOTES&nbsp;:</em></p>
	<ul>
		<li>Les comptes vont être créés dans Gees d'après les login/mdp ENT pour les responsables sélectionnés.</li>
...
<h2>Imposer les mots de passe élèves</h2>";
	...
	if((!isset($csv_file))||($csv_file['tmp_name']=='')) {
		...
		$tabchamps = array("LOGIN_GEES", "MOT_DE_PASSE_GEES");
		...
		if((!isset($tabindice['LOGIN_GEES']))||(!isset($tabindice['MOT_DE_PASSE_GEES']))) {
			echo "<p style='color:red'>La ligne d'entête ne comporte pas un des champs indispensables (<em>LOGIN_GEES, MOT_DE_PASSE_GEES</em>).</p>";
			require("../lib/footer.inc.php");
			die();
		}
		...
		while (!feof($fp)) {
			...
			if($ligne!='') {
				...
				$ligne_a_prendre_en_compte="y";
				if(preg_match("/LOGIN_GEES/i", trim($ligne))) {
					$ligne_a_prendre_en_compte="n";
				}
				elseif(preg_match("/MOT_DE_PASSE_GEES/i", trim($ligne))) {
					$ligne_a_prendre_en_compte="n";
				}

				if($ligne_a_prendre_en_compte=="y") {
					$tab_eleve[$cpt2]["LOGIN_GEES"]=$tab[$tabindice['LOGIN_GEES']];
					$tab_eleve[$cpt2]["MOT_DE_PASSE_GEES"]=$tab[$tabindice['MOT_DE_PASSE_GEES']];
					...
					$sql="SELECT e.nom, e.prenom, u.password, u.auth_mode FROM eleves e, utilisateurs u WHERE e.login=u.login AND u.login='".$tab[$tabindice['LOGIN_GEES']]."';";
					// DEBUG:
					//echo "$sql<br />";
					$res=mysqli_query($GLOBALS["mysqli"], $sql);
					if(mysqli_num_rows($res)>0) {
						...
						if($lig->password!="") {
							$tab_eleve[$cpt2]["password"]="<span title=\"Le mot de passe dans la base Gees n'est pas vide.\">XXXXXXXXXX</span>";
						}
						$tab_eleve[$cpt2]["auth_mode"]=$lig->auth_mode;

						$tmp_tab_classe=get_class_from_ele_login($tab[$tabindice['LOGIN_GEES']]);
						if(isset($tmp_tab_classe['liste_nbsp'])) {
							$tab_eleve[$cpt2]["classe"]=$tmp_tab_classe['liste_nbsp'];
						}
					}

					$sql="INSERT INTO tempo4 SET col1='$cpt2', col2='".$tab[$tabindice['LOGIN_GEES']]."', col3=MD5('".$tab[$tabindice['MOT_DE_PASSE_GEES']]."');";
					// DEBUG:
					//echo "$sql<br />";
					$insert=mysqli_query($GLOBALS["mysqli"], $sql);
...
<table class='boireaus boireaus_alt' summary='Tableau des élèves'>
	<tr>
		...
		<th colspan='4'>Informations Gees</th>
	...
	<tr>
		...
		foreach($tab_eleve as $key => $eleve_courant) {
			...
			*/
			...
			echo "
<tr class='white_hover'".$style_css.">
	<td><input type='checkbox' name='ligne[".$key."]' id='ligne_$cpt' value='".$eleve_courant['LOGIN_GEES']."' onchange=\"change_graisse($cpt)\" />$ancre_doublon_ou_pas</td>
	...
	<td><label for='ligne_$cpt'>".$eleve_courant['LOGIN_GEES']."</label>";
			if($eleve_courant['nom']=="") {
				$sql="SELECT nom, prenom FROM eleves WHERE login='".$eleve_courant['LOGIN_GEES']."';";
				...
				if(mysqli_num_rows($res_ele_nom)==0) {
					...
					echo " <a href='../utilisateurs/create_eleve.php?filtrage=Afficher&amp;critere_recherche=".preg_replace("/[^A-Za-z]/", "%", $lig_ele_nom->nom)."' title=\"L'élève $lig_ele_nom->nom $lig_ele_nom->prenom existe dans Gees, mais il n'a pas de compte d'utilisateur.
...
Suivez ce lien pour contrôler et créer un compte pour cet élève.\"><img src='../images/icons/buddy_plus.png' class='icone16' alt='Créer' /></a>";
			...
			echo "</td>
	<td>".$eleve_courant['MOT_DE_PASSE_GEES']."</td>";
...
<h2 class='noprint'>Envoi par mail des fiches bienvenue responsables</h2>";
		...
		echo "<p><strong style='color:red'>ATTENTION&nbsp;:</strong> Cette démarche ne fonctionne que dans le cas où les logins Gees des responsables et leurs logins ENT coïncident (<em>et si les adresses mail sont correctement renseignées dans votre table 'resp_pers'</em>).</p>";
...
$impression";
				...
				if((isset($tab_parent[$loop]['email_gepi']))&&($envoi=envoi_mail("Compte Gees", "Bonjour(soir),\n".$chaine, $tab_parent[$loop]['email_gepi'], "", "html", $tab_param_mail))) {
