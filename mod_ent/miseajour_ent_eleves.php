...
/**
 *
 * Module d'intégration de Gees dans un ENT réalisé au moment de l'intégration de Gees dans ARGOS dans l'académie de Bordeaux
	...
	*/
	...
	for($i = 0 ; $i < $nbre_a_traiter ; $i++){
		...
		if (isset($maj[$i])) {
			...
			// On peut alors ajouter cet élève à la base de Gees
			/*echo '
			...
			if ($query_eleves) {
				...
				if ($query_reg) {
					$msg .= '<p style="color: green;">L\'élève ' . $_nom[$i] . ' ' . $_prenom[$i] . ' a bien été enregistré dans la base des élèves de GEES.</p>';
				}else{
					$msg .= '<p style="color: red;">L\'élève ' . $_nom[$i] . ' ' . $_prenom[$i] . ' n\'a pas été enregistré dans la base des élèves de GEES.</p>';
				}
			}else{
				$msg .= '<p style="color: red;">L\'élève ' . $_nom[$i] . ' ' . $_prenom[$i] . ' n\'a pas été enregistré dans la base des élèves de GEES.</p>';
			}
