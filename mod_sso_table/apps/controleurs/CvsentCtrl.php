		...
		if (is_null($this->erreurs_lignes)) {
			...
			while (($this->ligne = fgetcsv($this->fic, 1024, ";")) !== FALSE) {
				...
				if (mysqli_num_rows($this->res) == 1) {
					// on a un seul utilisateur dans Gees
			...
			// on a bien un compte tuteur dans l'ENT, on peut le supprimer il n'a pas de compte dans Gees
			...
			*/
			...
			// il reste encore les erreurs : 2 comptes ENT -> 1 compte Gees, on peut nettoyer quand les 2 comptes ne sont pas des comptes parents
			...
			// On récupère tous les membres de l'ENT ayant un login Gees
			...
			// On récupère les membres de l'ENT sans login présents dans Gees
			...
			if (mysqli_num_rows($this->res) != 0) {
				while ($this->ligne = mysqli_fetch_array($this->res)) {
					...
					if (mysqli_num_rows($this->res2) > 0) {
						$this->message = 'Il y a plusieurs personnes dans Gees ayant les mêmes noms et prénoms';
			...
			// On récupère les membres de l'ENT sans login absents de Gees
			$this->res = $data->get_sans_login();
			if (mysqli_num_rows($this->res) != 0) {
				while ($this->ligne = mysqli_fetch_array($this->res)) {
					$this->res2 = $data->cherche_login($this->ligne);
					if (mysqli_num_rows($this->res2) == 0) {
						...
						if ($probable) {
							...
							$this->message = "L'utilisateur n'existe probablement pas dans Gees.";
							$this->class = "message_red";
							$nomPrenom = $this->ligne['nom'] . " " . $this->ligne['prenom'];
							$this->table[] = array('login_gepi' => $nomPrenom, 'login_sso' => $this->ligne['uid'], 'couleur' => $this->class, 'message' => $this->message);
						}
        ...
        switch ($code) {
            case 0:
                ...
                if ($row[0] == $this->ligne['uid']) {
                    ...
                    $this->message = 'Une entrée identique existe déjà dans la table pour ce login Gees';
                } else {
                    $this->class = "message_red";
                    $this->message = 'Une entrée différente existe déjà dans la table pour ce login Gees';
                }
                break;
            case 1:
                $this->class = "message_red";
            ...
            case 2:
                $this->class = "message_red";
                $this->message = 'L\'utilisateur n\'existe pas dans Gees.';
                break;
            case 3:
