...
if ($test_infos_serveur->versionGd()) {
	...
	<li style="list-style-type:circle">La taille maximum d\'une variable envoyée à Gees ne doit pas dépasser '.$test_infos_serveur->maxSize().' (<i>post_max_size</i>).
	</li>
	...
	<li style="list-style-type:circle">La taille maximum d\'un fichier envoyé à Gees est de '.$test_infos_serveur->tailleMaxFichier().' (<i>upload_max_filesize</i>).
	</li>';
	...
	if($suhosin_post_max_totalname_length!='') {
		...
		echo "<p>Le module suhosin est activé.<br />\nUn paramétrage trop restrictif de ce module peut perturber le fonctionnement de Gees, particulièrement dans les pages comportant de nombreux champs de formulaire (<i>comme par exemple dans la page de saisie des appréciations par les professeurs</i>)</p>\n";
	...
	echo "<p>Il peut être nécessaire <b>momentanément</b> de configurer Gees pour forcer l'affichage des erreurs PHP afin de résoudre des dysfonctionnements. Attention ! En temps normal l'affichage des erreurs PHP doit être désactivé.</p>\n";
	...
	echo "Certains dossiers doivent être accessibles en écriture pour Gees.<br />\n";
	...
	if(file_exists('../style_screen_ajout.css')){
            ...
            if($test_ecriture_style_screen_ajout) {
                echo "Le fichier style_screen_ajout.css à la racine de l'arborescence Gees est accessible en écriture.\n";
            } else {
                echo "<span style='color:red'><b>ERREUR</b>&nbsp;: Le fichier style_screen_ajout.css à la racine de l'arborescence Gees n'a pas pu être créé ou n'est pas accessible en écriture.</span>\n";
            }
        }elseif(file_exists('../style_screen_ajout.css.ori')) {
            echo "<span style='color:red'> Le fichier style_screen_ajout.css.ori à la racine de l'arborescence Gees doit être renommé en style_screen_ajout.css et être accessible en écriture.</span>\n";
        }else{
            echo "<span style='color:red'><b>ERREUR</b>&nbsp;: Le fichier style_screen_ajout.css à la racine de l'arborescence Gees est manquant. Il faut en créer un vide qui doit être accessible en écriture.</span>\n";
        } 
	echo "</p>\n";
