...
if($_SESSION['statut']=='professeur') {
	if(!getSettingAOui('GeesAccesGestElevesProf')) {
		$acces_visu_eleve="n";
	}
}
	...
	for($loop_tab_login=0;$loop_tab_login<count($tab_login);$loop_tab_login++) {
		...
		if(mysqli_num_rows($res)>0) {
			...
			if(check_mail($lig->email)) {
				$tab_result_recherche['personnel'][$cpt_pers]['td_email']="<a href='mailto:".$lig->email."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES' title='Envoyer un mail à $lig->email'><img src='../images/icons/courrier_envoi.png' class='icone16' alt='Mail' /></a><span id='email_personnel_".$cpt_pers."' style='display:none;'>$lig->email</span>";
			}
...
function extract_eleves($tab_login) {
	...
	$GeesAccesGestElevesProf=getSettingAOui('GeesAccesGestElevesProf');
	...
	for($loop_tab_login=0;$loop_tab_login<count($tab_login);$loop_tab_login++) {
		...
		if(mysqli_num_rows($res)>0) {
			...
			$restriction_acces="n";
			if(($_SESSION['statut']=='professeur')&&
			((!getSettingAOui('GeesAccesGestElevesProf'))||(!is_prof_ele($_SESSION['login'], $lig->login)))) {
				if((getSettingAOui('GeesAccesGestElevesProfP'))&&(is_pp($_SESSION['login'], "", $lig->login))) {
					$restriction_acces="n";
				}
				else {
					$restriction_acces="y";
			...
			if((($_SESSION['statut']=='professeur')&&($GeesAccesGestElevesProf))||(in_array($_SESSION['statut'], array('scolarite', 'cpe', 'administrateur', 'autre')))) {
				$cpt_resp=0;
...
<script type='text/javascript'>
	...
	function recherche_envoi_mail(categorie) {
		...
		//alert(liste_mail);
		document.getElementById('span_mail_infobulle').innerHTML=\"<a href='mailto:\"+liste_mail+\"?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES'>\"+liste_mail+\"</a>\";
