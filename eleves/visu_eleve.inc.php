...
if(document.getElementById('rech_nom')) {document.getElementById('rech_nom').focus();}
	...
	elseif(($_SESSION['statut']=='cpe')&&
		(getSettingAOui('GeesAccesReleveCpeTousEleves'))||
		(getSettingAOui('GeesRubConseilCpeTous'))||
		(getSettingAOui('GeesAccesCdtCpe'))||
		(getSettingAOui('AACpeTout'))||
		(getSettingAOui('GeesAccesTouteFicheEleveCpe'))||
		(getSettingAOui('GeesAccesAbsTouteClasseCpe'))
	) {
...
Patientez pendant l'extraction des données... merci.
	...
	if(mysqli_num_rows($res_ele)==0){
		...
		elseif($_SESSION['statut']=='scolarite') {
			if (getSettingValue("GeesAccesTouteFicheEleveScolarite")!='yes') {
			...
			$GeesAccesReleveScol=getSettingValue('GeesAccesReleveScol');
			if($GeesAccesReleveScol=="yes") {
				$acces_releves="y";
			}
		...
		elseif($_SESSION['statut']=='cpe') {
			if (getSettingValue("GeesAccesTouteFicheEleveCpe")!='yes') {
			...
			$GeesAccesReleveCpeTousEleves=getSettingValue('GeesAccesReleveCpeTousEleves');
			$GeesAccesReleveCpe=getSettingValue('GeesAccesReleveCpe');
			if($GeesAccesReleveCpeTousEleves=='yes') {
				$acces_releves="y";
			}
			elseif($GeesAccesReleveCpe=="yes") {
				$sql="SELECT 1=1 FROM j_eleves_cpe WHERE cpe_login='".$_SESSION['login']."' AND e_login='".$ele_login."';";
				$test=mysqli_query($GLOBALS["mysqli"], $sql);
		...
		elseif($_SESSION['statut']=='professeur') {
			...
			$GeesAccesGestElevesProf=getSettingValue('GeesAccesGestElevesProf');
			if($GeesAccesGestElevesProf=="yes") {
				$acces_responsables="y";
			}
			else {
				$GeesAccesGestElevesProfP=getSettingValue('GeesAccesGestElevesProfP');
				if(($GeesAccesGestElevesProfP=="yes")&&($is_pp=="y")) {
					$acces_responsables="y";
				}
			}

			// Contrôle de l'accès du prof au relevé de notes:
			$GeesAccesReleveProfP=getSettingValue('GeesAccesReleveProfP');
			if(($GeesAccesReleveProfP=="yes")&&($is_pp=="y")) {
				$acces_releves="y";
			}
			...
			if($acces_releves=='n') {
				$GeesAccesReleveProfToutesClasses=getSettingValue('GeesAccesReleveProfToutesClasses');
				if($GeesAccesReleveProfToutesClasses=='yes') {
					$acces_releves="y";
				}
				else {
					$GeesAccesReleveProfTousEleves=getSettingValue('GeesAccesReleveProfTousEleves');
					if($GeesAccesReleveProfTousEleves=='yes') {
					...
					if($acces_releves=='n') {
						$GeesAccesReleveProf=getSettingValue('GeesAccesReleveProf');
						if($GeesAccesReleveProf=='yes') {
			...
			$GeesAccesBulletinSimplePP=getSettingValue('GeesAccesBulletinSimplePP');
			if(($GeesAccesBulletinSimplePP=="yes")&&($is_pp=="y")) {
				$acces_bulletins="y";
			}

			if($acces_bulletins=='n') {
				$GeesAccesBulletinSimpleProfToutesClasses=getSettingValue('GeesAccesBulletinSimpleProfToutesClasses');
				if($GeesAccesBulletinSimpleProfToutesClasses=='yes') {
					$acces_bulletins="y";
				}
				else {
					$GeesAccesBulletinSimpleProfTousEleves=getSettingValue('GeesAccesBulletinSimpleProfTousEleves');
					if($GeesAccesBulletinSimpleProfTousEleves=='yes') {
						if ($eleve_classe_prof=="y") {
							$acces_bulletins="y";
						}
						else {
					...
					if($acces_bulletins=='n') {
						$GeesAccesBulletinSimpleProf=getSettingValue('GeesAccesBulletinSimpleProf');
						if($GeesAccesBulletinSimpleProf=='yes') {
							if ($eleve_groupe_prof=="y") {
								$acces_bulletins="y";
							}
							else {
		...
		$releve_formule_bas=getSettingValue("releve_formule_bas") ? getSettingValue("releve_formule_bas") : "Relevé à conserver précieusement. Aucun duplicata ne sera délivré. - GEES : solution libre de gestion et de suivi des résultats scolaires.";
		...
		if(isset($tab_ele['compte_utilisateur'])) {
			...
			(($_SESSION['statut']=='professeur')&&((isset($tab_ele['compte_utilisateur']['DerniereConnexionEle']))||(isset($tab_ele['compte_utilisateur']['DerniereConnexionEle_Echec']))))) {
				echo "<div style='float:right; width:20em; text-align:center;'>\n";
					...
					$alt=$alt*(-1);
					echo "<tr class='lig$alt'><th style='text-align: left;'>Authentification&nbsp;:</th><td title=\"Gees permet selon les configurations plusieurs modes d'authentification:
- gepi : Authentification sur la base mysql de Gees,
...
- ldap : Authentification en recherchant la correspondance login/mot_de_passe dans un annuaire LDAP.\">".$tab_ele['compte_utilisateur']['auth_mode'];
		...
		echo "<td valign='top'>\n";
			...
			if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||
			(($_SESSION['statut']=='cpe')&&(getSettingAOui('GeesAccesTouteFicheEleveCpe')))||
			(($_SESSION['statut']=='cpe')&&(is_cpe($_SESSION['login'],'',$ele_login)))||
			(($_SESSION['statut']=='professeur')&&(is_pp($_SESSION['login'],"",$ele_login))&&(getSettingAOui('GeesAccesGestElevesProfP')))) {
			...
			$tmp_date=getdate();
			//echo "<a href='mailto:".$tab_ele['email']."?subject=GEES&amp;body=";
			echo "<a href='mailto:".$tab_ele['email']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES&amp;body=";
				...
				for($i=0;$i<count($tab_ele['resp']);$i++) {
					if($tab_ele['resp'][$i]['resp_legal']!=0) {
						...
						if($tab_ele['resp'][$i]['mel']!='') {
							...
							echo "<a href='mailto:".$tab_ele['resp'][$i]['mel']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES&amp;body=";
							if($tmp_date['hours']>=18) {echo "Bonsoir";} else {echo "Bonjour";}
							echo ",%0d%0aCordialement.'>";
						...
						if(!isset($tab_ele['resp'][$i]['etat'])) {
							...
							if(in_array($_SESSION['statut'], array('administrateur', 'scolarite', 'cpe'))) {
								echo "<br />\n";
								echo "<span title=\"Gees permet selon les configurations plusieurs modes d'authentification:
- gepi : Authentification sur la base mysql de Gees,
- sso : Authentification CAS ou LCS assurée par une autre machine,
							...
							if($tab_ele['resp'][$i]['envoi_bulletin']!='') {
								...
								if(in_array($_SESSION['statut'], array('administrateur', 'scolarite'))) {
									if($tab_ele['resp'][$i]['envoi_bulletin']=="y") {
										...
										echo "><img src='../images/icons/bulletin.png' width='16' height='16' title=\"Le responsable non légal ".$tab_ele['resp'][$i]['prenom']." ".$tab_ele['resp'][$i]['nom']." est destinataire des bulletins générés dans Gees.
...
Cliquez pour supprimer la génération de bulletins à destination de ce responsable.\" /></a>";
										...
										echo " onclick=\"return confirm_abandon (this, change, '$themessage')\"";
										echo "><img src='../images/icons/bulletin_barre.png' width='16' height='16' title=\"Le responsable non légal ".$tab_ele['resp'][$i]['prenom']." ".$tab_ele['resp'][$i]['nom']." n'est pas destinataire des bulletins générés dans Gees.
		...
		if($acces_enseignements=="y") {
			...
			if((!isset($tab_ele['periodes']))||(!isset($tab_ele['groupes']))) {
				...
				for($i=0;$i<count($tab_ele['groupes']);$i++) {
					...
					for($j=0;$j<$nbre_professeurs;$j++) {
						if($tab_ele['groupes'][$i]['prof'][$j]['email']!='') {
							echo "<a href='mailto:".$tab_ele['groupes'][$i]['prof'][$j]['email']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES - [".remplace_accents($tab_ele['nom'],'all')." ".remplace_accents($tab_ele['prenom'],'all')."]&amp;body=";
							if($tmp_date['hours']>=18) {echo "Bonsoir";} else {echo "Bonjour";}
				...
				for($loop=0;$loop<count($tab_ele['classe']);$loop++) {
					if(isset($tab_ele['classe'][$loop]['pp'])) {
						...
						if($tab_ele['classe'][$loop]['pp']['email']!="") {
							...
							//echo "<a href='mailto:".$tab_ele['classe'][$loop]['pp']['email']."'>";
							echo "<a href='mailto:".$tab_ele['classe'][$loop]['pp']['email']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES - [".remplace_accents($tab_ele['nom'],'all')." ".remplace_accents($tab_ele['prenom'],'all')."]&amp;body=";
							if($tmp_date['hours']>=18) {echo "Bonsoir";} else {echo "Bonjour";}
				...
				if($tab_ele['cpe']['email']!="") {
					...
					//echo "<a href='mailto:".$tab_ele['cpe']['email']."'>";
					echo "<a href='mailto:".$tab_ele['cpe']['email']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES - [".remplace_accents($tab_ele['nom'],'all')." ".remplace_accents($tab_ele['prenom'],'all')."]&amp;body=";
					if($tmp_date['hours']>=18) {echo "Bonsoir";} else {echo "Bonjour";}
				...
				if($tab_ele['equipe_liste_email']!="") {
					$tmp_date=getdate();
					//echo "<p>Ecrire un email à <a href='mailto:".$tab_ele['equipe_liste_email']."?subject=GEES&amp;body=";
					echo "<p>Ecrire un email à <a href='mailto:".$tab_ele['equipe_liste_email']."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES - [".remplace_accents($tab_ele['nom'],'all')." ".remplace_accents($tab_ele['prenom'],'all')."]&amp;body=";
					if($tmp_date['hours']>=18) {echo "Bonsoir";} else {echo "Bonjour";}
