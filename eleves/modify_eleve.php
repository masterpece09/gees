...
if($_SESSION['statut']=='professeur') {
	if((!getSettingAOui('GeesAccesGestElevesProf'))&&(!getSettingAOui('GeesAccesGestElevesProfP'))) {
		header("Location: ../accueil.php?msg=Accès aux fiches élèves non autorisé.");
		die();
	}

	if((getSettingAOui('GeesAccesGestElevesProfP'))&&(is_pp($_SESSION['login'], "", $eleve_login))) {
		// C'est OK
	}
	else {
		...
		*/
		...
		if(mysqli_num_rows($test)==0) {
			...
			if((isset($reg_no_gep))&&(isset($eleve_login))) {
				if($reg_no_gep!="") {
					if(mb_strlen(preg_replace("/[0-9]/","",$reg_no_gep))==0) {
						if(($_SESSION['statut']=='professeur')&&(getSettingValue("GeesAccesGestPhotoElevesProfP")!='yes')) {
...
if (isset($eleve_login)) {
	echo "<th style='text-align:left;'>Identifiant GEES * : </th>
	<td>";
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	...
	*/
	...
	if((isset($eleve_email))&&($eleve_email!='')) {
		$tmp_date=getdate();
		echo " <a href='mailto:".$eleve_email."?subject=".getSettingValue('gepiPrefixeSujetMail')."GEES&amp;body=";
...
if(isset($reg_no_gep)){
	...
	//echo "getSettingValue(\"GeesAccesGestPhotoElevesProfP\")=".getSettingValue("GeesAccesGestPhotoElevesProfP")."<br />";
  ...
  (($_SESSION['statut']=="professeur")&&(getSettingValue("GeesAccesGestPhotoElevesProfP")=='yes')&&(isset($eleve_login))&&($is_pp)))) {
		echo "<div align='center'>\n";
...
if($_SESSION['statut']=='professeur'){
  if (($is_pp)&&(getSettingValue("active_module_trombinoscopes")=='y') && (getSettingValue("GeesAccesGestPhotoElevesProfP")=='yes')){
