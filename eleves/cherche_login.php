...
if(($type_server=='scribe')&&(($nom!="")||($prenom!=""))) {
	...
	if($filtre!="") {
		...
		if ($result) {
			...
			if($info["count"]==0) {
				echo "<span style='color:red;'>Aucun enregistrement n'a été trouvé dans le LDAP pour ".$nom." ".$prenom.". Il est <strong>indispensable</strong>, pour un accès de l'élève à Gees, de créer le compte dans l'annuaire avant de le créer dans Gees&nbsp;!!!</span><br />\n";
				$erreur++;
			}
			else {
				/*
