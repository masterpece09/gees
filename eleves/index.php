...
if($_SESSION['statut']=="professeur") {
	if(getSettingValue('GeesAccesGestElevesProfP')!='yes') {
...
if (isset($action) and ($action == 'depot_photo') and $total_photo != 0)  {
	...
	while($cpt_photo < $total_photo)
	{
		...
		if((isset($_FILES['photo']['type'][$cpt_photo]))&&($_FILES['photo']['type'][$cpt_photo] != ""))
		{
			...
			elseif(($_SESSION['statut']=='professeur')&&(!getSettingAOui('GeesAccesGestPhotoElevesProfP'))) {
				$acces_upload_photo="n";
			}
...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	...
	if ($droits == "V") {
		echo " | <a href=\"import_eleves_csv.php\" title=\"Télécharger le fichier des noms, prénoms, identifiants GEES et classes\">Télécharger le fichier des élèves au format csv.</a>\n";
	}
	...
	if((getSettingValue("import_maj_xml_sconet")==1)&&
		(
			...
			(($_SESSION['statut']=='scolarite')&&(getSettingAOui('GeesAccesMajSconetScol')))
		)
	) {
...
if ($test == '0') {
	echo "<p class='grand'>Attention : il n'y a aucun élève dans la base GEES !</p>\n";
	if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
...
if(isset($quelles_classes)) {
	...
	echo "<p class='small'><em>Remarque&nbsp;:</em> l'identifiant mentionné ici ne permet pas aux élèves de se connecter à Gees, il sert simplement d'identifiant unique.";
	//if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	if($_SESSION['statut']=="administrateur") {
		echo " Pour permettre aux élèves de se connecter à Gees, vous devez leur créer des comptes d'accès, en passant par la page Gestion des bases -> Gestion des comptes d'accès utilisateurs -> <a href='../utilisateurs/edit_eleve.php'>Elèves</a>.";
	}
	elseif($_SESSION['statut']=="scolarite") {
		echo " Pour permettre aux élèves de se connecter à Gees, vous devez vous connecter en 'administrateur' et leur créer des comptes d'accès, en passant par la page Gestion des bases -> Gestion des comptes d'accès utilisateurs -> Elèves.";
	}
	echo "</p>\n";
...
//    echo "<th><p>Classe</p></th>";
	...
	if (getSettingValue("active_module_trombinoscopes")=='y') {
		if($_SESSION['statut']=="professeur") {
			if (getSettingValue("GeesAccesGestPhotoElevesProfP")=='yes') {
...
/*
	...
	echo "<p>Remarque : le login ne permet pas aux élèves de se connecter à Gees. Il sert simplement d'identifiant unique.</p>\n";
*/
	...
	while ($i < $nombreligne){
		...
		if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')||($_SESSION['statut']=='autre')||
			...
			(($_SESSION['statut']=='professeur')&&(is_pp($_SESSION['login'],"",$eleve_login))&&(getSettingAOui('GeesAccesGestElevesProfP')))||
			((getSettingAOui('GeesAccesPPTousElevesDeLaClasse'))&&(is_pp($_SESSION['login'], $quelles_classes)))) {
		...
		if ((getSettingValue("active_module_trombinoscopes")=='y')&&
			((($_SESSION['statut']=="professeur")&&(getSettingValue("GeesAccesGestPhotoElevesProfP")=='yes'))||
