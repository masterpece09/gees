...
if ((isset($_GET['creer_structure'])) and (($current_group["classe"]["ver_periode"]["all"][$periode_num] >= 2)||($acces_exceptionnel_saisie)) and (getSettingAOui('GeesPeutCreerBoitesProf'))) {
  check_token();
...
<ul>
	...
	if((isset($id_racine))&&(getPref($_SESSION['login'], 'cnBoitesModeMoy', '')=="")) {
		...
		if(mysqli_num_rows($res_nb_conteneurs)>1) {
			...
			echo "<p><br /></p>\n";
			if((isset($id_racine))&&(getSettingAOui('GeesPeutCreerBoitesProf'))) {
