...
if($_SESSION['statut']=='eleve') {
	if(getSettingValue("GeesAccesReleveEleve") != "yes") {
...
elseif($_SESSION['statut']=='responsable') {
	if(getSettingValue("GeesAccesReleveParent") != "yes") {
		echo "<p>Vous n'êtes pas autorisé à accéder aux relevés de notes.</p>\n";
		require("../lib/footer.inc.php");
		die();
	}
...
if ((!isset($tab_id_classe))&&(!isset($id_groupe))) {
	...
	if($_SESSION['statut']=='professeur') {
		...
		if(getSettingAOui('GeesProfImprRelSettings')) {
			echo " | ";
	...
	elseif($_SESSION['statut']=='scolarite') {
		...
		if(getSettingAOui('GeesScolImprRelSettings')) {
			echo " | ";
			echo "<a href='param_releve_html.php' target='_blank'><img src='../images/icons/configure.png' class='icone16' alt='Paramétrer' /> Paramètres du relevé HTML</a>";
		}
		//echo " | ";
	...
	elseif($_SESSION['statut']=='cpe') {
		echo "<a href='../accueil.php'><img src='../images/icons/back.png' alt='Retour' class='back_link'/> Retour à l'accueil</a>";
		if(getSettingAOui('GeesCpeImprRelSettings')) {
			echo " | ";
			echo "<a href='param_releve_html.php' target='_blank'><img src='../images/icons/configure.png' class='icone16' alt='Paramétrer' /> Paramètres du relevé HTML</a>";
		}
		//echo " | ";
	...
	elseif (($_SESSION['statut'] == 'cpe') AND (getSettingValue("GeesAccesReleveCpeTousEleves") == "yes")) {
		$sql="SELECT DISTINCT c.* FROM classes c ORDER BY classe";
	}
	elseif(($_SESSION['statut'] == 'cpe') AND (getSettingValue("GeesAccesReleveCpe") == "yes")) {
		...
		// - on restreint scolarité à ses classes alors que GeesAccesReleveScol correspond dans Droits d'accès à Toutes les classes
		// - on ne restreint pas le CPE de semblable façon aux élèves dont il est responsable -> CORRIGé
	}
	elseif($_SESSION['statut']=='professeur') {
		// GeesAccesReleveProf               -> que les élèves de ses groupes
		// GeesAccesReleveProfTousEleves     -> tous les élèves de ses classes
		// GeesAccesReleveProfToutesClasses  -> tous les élèves de toutes les classes
		...
		elseif ((getSettingValue("GeesAccesReleveProf") == "yes")||(getSettingValue("GeesAccesReleveProfTousEleves") == "yes")) {
		...
		elseif(getSettingValue("GeesAccesReleveProfP")=="yes") {
	...
	elseif(($_SESSION['statut']=='eleve')&&(getSettingValue("GeesAccesReleveEleve") == "yes")) {
	...
	elseif(($_SESSION['statut']=='responsable')&&(getSettingValue("GeesAccesReleveParent") == "yes")) {
		$sql="(SELECT DISTINCT c.* FROM eleves e, j_eleves_classes jec, classes c, responsables2 r, resp_pers rp
				WHERE (e.ele_id=r.ele_id AND
						r.pers_id=rp.pers_id AND
						rp.login='".$_SESSION['login']."' AND
...
echo "<script type='text/javascript'>
	...
	$nb_grp_prof=0;
	if(($_SESSION['statut']=='professeur')&&((getSettingValue("GeesAccesReleveProfToutesClasses") == "yes")||(getSettingValue("GeesAccesReleveProf") == "yes")||(getSettingValue("GeesAccesReleveProfTousEleves") == "yes"))) {
...
echo "</p>";
	...
	if ((($_SESSION['statut']=='eleve') AND (getSettingValue("GeesAccesOptionsReleveEleve") != "yes"))||
		(($_SESSION['statut']=='responsable') AND (getSettingValue("GeesAccesOptionsReleveParent") != "yes"))) {
		echo "<p>\n";
...
echo "</script>\n";
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		elseif (($_SESSION['statut'] == 'professeur') AND
				(
					(getSettingValue("GeesAccesReleveProf") == "yes") ||
					(getSettingValue("GeesAccesReleveProfTousEleves") == "yes") ||
		...
		elseif(($_SESSION['statut'] == 'professeur')&&(getSettingValue("GeesAccesReleveProfP")=="yes")) {
