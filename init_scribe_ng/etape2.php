...
if ($_POST['step'] == "2") {
    ...
    if (((is_object($GLOBALS["mysqli"])) ? mysqli_errno($GLOBALS["mysqli"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false)) != 0) {
        echo "<br>Impossible de r&eacute;cup&eacute;rer les classes dans la base GEES.<br>";
        require("../lib/footer.inc.php");
        die();
    }
    else {
        ...
        if (mysqli_num_rows($res) != 0) {
            echo "<p>Voici la liste des classes présentes dans GEES pour lesquelles aucune p&eacute;riode n'a &eacute;t&eacute; d&eacute;finie,<br>";
