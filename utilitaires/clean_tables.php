...
//==================================
// header
$titre_page = "Vérification/nettoyage des tables de la base de données GEES";
...
elseif (isset($_POST['action']) AND $_POST['action'] == 'check_auto_increment') {
	...
	if (isset($_POST['is_confirmed']) and $_POST['is_confirmed'] == "yes") {
		...
		echo "<p>Ce script ne doit être exécuté que si vous avez restauré sur votre Gees une sauvegarde réalisée avec une version 1.4.4 en utilisant la méthode 'mysqldump' (et non la méthode classique de sauvegarde Gees sans mysqldump).</p>\n";
	...
	*/
	...
	echo "<div style='margin-left: 3em;'>\n";
		...
		echo "<p>Il est également vivement conseillé de <b><a href='../gestion/gestion_connect.php'>désactiver les connexions à GEES</a> durant la phase de nettoyage</b>.</p>
	...
	echo "<div style='margin-left: 3em;'>\n";
		...
		echo "<p>Cette procédure opère un <b>nettoyage</b> des lignes inutiles dans les <b>tables de liaison</b> de la base MySql de GEES et dans les tables des données scolaires des élèves (notes, appréciations, absences).";
		echo "<br />Les tables de liaison contiennent des informations qui mettent en relation les tables principales de GEES
	(élèves, professeurs, matières, classes).<br /><br />
	Du fait de bugs mineurs (éventuellement déjà réglés mais présents dans des versions antérieures de GEES) ou de mauvaises manipulations,
	ces tables de liaison peuvent contenir des données obsolètes ou des doublons qui peuvent nuire à un fonctionnement optimal de GEES.";
...
<form action='".$_SERVER['PHP_SELF']."' method='post'>
	<fieldset class='fieldset_opacite50'>
		<p style='text-indent:-6em; margin-left:6em;'><strong style='color:red;'>ANOMALIE&nbsp;:</strong> Vous avez ".$nb_scories." identifiant(s) ENT associés à plusieurs logins Gees dans la table 'sso_table_correspondance'.<br />Cela ne devrait pas arriver.<br />
...
<script type='text/javascript'>
	...
	//===================================================
		...
		echo "<p>Gees a un temps contenu un bug sur le format des login.<br />La présence de 'point' dans un nom de login par exemple pouvait provoquer des dysfonctionnements.<br />Le contenu des tables 'j_eleves_cpe' et 'j_eleves_professeurs' pouvait être affecté.</p>\n";
		echo "<p><b>Il est vivement recommandé de faire une sauvegarde de la base avant d'effectuer cette opération !</b></p>\n";
		echo "<form action=\"clean_tables.php\" method=\"post\">\n";
		echo add_token_field();
		...
		echo "<p>Correction des interclassements (<i>COLLATION</i>) des champs des tables.<br />Si des anomalies ont été relevées lors d'un contrôle des interclassements, vous pouvez effectuer une correction&nbsp;:<br />Elle consistera à forcer l'interclassement 'utf8_general_ci' sur les tables Gees.</p>\n";
		echo "<form action=\"clean_tables.php\" method=\"post\">\n";
		echo add_token_field();
		echo "<center>\n";
	...
	echo "<div style='margin-left: 3em;'>\n";
		...
		echo "<p>Au changement d'année, il est recommandé de vider les entrées des tables 'edt_classes', 'edt_cours', 'edt_calendrier' du module emploi du temps de Gees.</p>\n";
		echo "<form action=\"clean_tables.php\" method=\"post\">\n";
		echo add_token_field();
		...
		echo "<p>Au changement d'année, il est recommandé de vider les entrées des tables 'absences_rb', 'absences_repas' et 'absences_eleves' du module abs1 de Gees, ainsi que les tables a_agregation_decompte, a_notifications, a_saisies, a_saisies_version, a_traitements du module abs2 : </p>\n";
		...
		echo "<p>Au changement d'année, il est recommandé de vider les entrées des tables du module Discipline de Gees.</p>\n";
		echo "<form action=\"clean_tables.php\" method=\"post\">\n";
		echo add_token_field();
		echo "<center>\n";
