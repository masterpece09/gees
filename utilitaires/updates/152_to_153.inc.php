...
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'GeesAccesSaisieEctsProf'");
$res_test=mysqli_num_rows($req_test);
if ($res_test==0){
  $result_inter = traite_requete("INSERT INTO setting VALUES ('GeesAccesSaisieEctsProf', 'no');");
  if ($result_inter == '') {
    $result.=msj_ok("Initialisation du paramètre GeesAccesSaisieEctsProf à 'n' : Ok");
  } else {
    $result.=msj_erreur(": Initialisation du paramètre GeesAccesSaisieEctsProf à 'n'");
  }
} else {
  $result .= msj_present("Le paramètre GeesAccesSaisieEctsProf existe déjà dans la table setting.");
}


// Ajout d'un paramètre de droits d'accès au tableau récapitulatif des ECTS pour les profs
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'GeesAccesRecapitulatifEctsProf'");
$res_test=mysqli_num_rows($req_test);
if ($res_test==0){
  $result_inter = traite_requete("INSERT INTO setting VALUES ('GeesAccesRecapitulatifEctsProf', 'yes');");
  if ($result_inter == '') {
    $result.=msj_ok("Initialisation du paramètre GeesAccesRecapitulatifEctsProf à 'yes' : Ok");
  } else {
    $result.=msj_erreur(": Initialisation du paramètre GeesAccesRecapitulatifEctsProf à 'yes'");
  }
} else {
  $result .= msj_present("Le paramètre GeesAccesRecapitulatifEctsProf existe déjà dans la table setting.");
}

// Ajout d'un paramètre de droits d'accès au tableau récapitulatif des ECTS pour la scolarité
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'GeesAccesRecapitulatifEctsScolarite'");
$res_test=mysqli_num_rows($req_test);
if ($res_test==0){
  $result_inter = traite_requete("INSERT INTO setting VALUES ('GeesAccesRecapitulatifEctsScolarite', 'yes');");
  if ($result_inter == '') {
    $result.=msj_ok("Initialisation du paramètre GeesAccesRecapitulatifEctsScolarite à 'yes' : Ok");
  } else {
    $result.=msj_erreur(": Initialisation du paramètre GeesAccesRecapitulatifEctsScolarite à 'yes'");
  }
} else {
  $result .= msj_present("Le paramètre GeesAccesRecapitulatifEctsScolarite existe déjà dans la table setting.");
}
