...
if(getSettingValue('gepi_en_production')=="") {
	if(!saveSetting('gepi_en_production', 'y')) {
		$result .= "Initialisation d'un témoin comme quoi le serveur Gees n'est pas juste un serveur de test, mais un serveur en production : ".msj_erreur("ECHEC !");
	}
	else {
		$result .= "Initialisation d'un témoin comme quoi le serveur Gees n'est pas juste un serveur de test, mais un serveur en production : ".msj_ok("Ok !");
	}
}

if(getSettingValue('GeesAccesCDTToutesClasses')=="") {
	if(!saveSetting('GeesAccesCDTToutesClasses', 'yes')) {
...
$test = sql_query1("SELECT 1=1 FROM setting WHERE name='GeesAccesBulletinSimpleColonneMoyClasseResp';");
if ($test == -1) {
	$result_inter = traite_requete("INSERT INTO setting SET name='GeesAccesBulletinSimpleColonneMoyClasseResp', value='y';");
	if ($result_inter == '') {
		$result .= msj_ok("SUCCES !");
	}
	else {
...
$test = sql_query1("SELECT 1=1 FROM setting WHERE name='GeesAccesBulletinSimpleColonneMoyClasseEleve';");
if ($test == -1) {
	$result_inter = traite_requete("INSERT INTO setting SET name='GeesAccesBulletinSimpleColonneMoyClasseEleve', value='y';");
	if ($result_inter == '') {
		$result .= msj_ok("SUCCES !");
	}
	else {
