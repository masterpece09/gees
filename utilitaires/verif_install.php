    ...
    if (@($GLOBALS["mysqli"] = mysqli_connect("$dbHost",  "$dbUser",  "$dbPass"))) {
        if (@((bool)mysqli_query($GLOBALS["mysqli"], "USE `$dbDb`"))) {
            ...
            */
            ...
            $msg = "<p>La connexion au serveur Mysql est établie mais impossible de sélectionner la base contenant les tables GEES.</p>";
            $correct_install = 'no';
        }
    } else {
...
if ($correct_install=='no') {
    ...
    <body>
    <?php
    echo "<h1 class='gepi'>GEES</h1>";
    echo $msg;
    if ($maj == 'no') {
        echo "<p>L'installation de GEES n'est peut-être pas terminée.</p>";
        echo "<center><a href='./utilitaires/install.php'>Installer la base Mysql</a></center>";
    } else if ($maj == 'yes') {
        echo "<p>Il s'agit sans doute d'une mise à jour vers une nouvelle version de GEES. Dans ce cas, vous devez procéder à une mise à jour de la base de données MySql.</p>";
        echo "<center><b><a href='./utilitaires/maj.php'>Mettre à jour la base Mysql</a></b></center>";
        echo "<hr />";
        echo "<p>Sinon, l'installation de GEES n'est peut-être pas terminée. Vous pouvez procéder à une installation/réinstallation de la base.</p>";
