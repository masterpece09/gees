...
if(
	(
		(($_SESSION['statut']=='professeur')&&(getSettingValue('GeesRubConseilProf')=="yes"))||
		(($_SESSION['statut']=='scolarite')&&(getSettingValue('GeesRubConseilScol')=="yes"))||
		(($_SESSION['statut']=='cpe')&&((getSettingValue('GeesRubConseilCpe')=="yes")||(getSettingValue('GeesRubConseilCpeTous')=="yes")))
	...
	if(preg_match("/^[0-9]{1,}$/",$num_periode_saisie)) {
		...
		elseif($_SESSION['statut']=='cpe') {
			if(getSettingValue('GeesRubConseilCpeTous')!="yes") {
		...
		if((isset($_POST['passer_a_recapitulatif_avis']))&&($_POST['passer_a_recapitulatif_avis']=='y')&&(acces('/saisie/saisie_avis2.php', $_SESSION['statut']))) {
			...
			if(
				...
				elseif(($_SESSION['statut']=='cpe')&&(getSettingValue('GeesRubConseilCpeTous')!="yes")) {
					$droit_saisie_avis="n";
...
if (
	($_SESSION['statut'] == "responsable" AND getSettingValue("GeesAccesGraphParent") != "yes") OR
	($_SESSION['statut'] == "eleve" AND getSettingValue("GeesAccesGraphEleve") != "yes")
	) {
...
if ($_SESSION['statut'] == "responsable") {
	...
	if(getSettingAOui('GeesMemesDroitsRespNonLegaux')) {
...
if (!isset($id_classe) and $_SESSION['statut'] != "responsable" AND $_SESSION['statut'] != "eleve") {
	...
	if(((getSettingValue("GeesAccesReleveProfToutesClasses")=="yes")&&($_SESSION['statut']=='professeur'))||
		((getSettingValue("GeesAccesReleveScol")=='yes')&&($_SESSION['statut']=='scolarite'))) {
		$sql="SELECT DISTINCT c.* FROM classes c, periodes p WHERE p.id_classe = c.id ORDER BY classe";
	}
	/*
	if(((getSettingValue("GeesAccesReleveProfToutesClasses")=="yes")&&($_SESSION['statut']=='professeur'))||
		((getSettingValue("GeesAccesReleveScol")=='yes')&&($_SESSION['statut']=='scolarite'))||
		((getSettingValue("GeesAccesReleveCpeTousEleves")=='yes')&&($_SESSION['statut']=='cpe'))) {
		$sql="SELECT DISTINCT c.* FROM classes c, periodes p WHERE p.id_classe = c.id ORDER BY classe";
	}
	elseif((getSettingValue("GeesAccesReleveCpe")=='yes')&&($_SESSION['statut']=='cpe')) {
		$sql="SELECT DISTINCT c.* FROM classes c, periodes p, j_eleves_classes jec, j_eleves_cpe jecpe WHERE
			p.id_classe = c.id AND
			jec.id_classe=c.id AND
			jec.periode=p.num_periode AND
	...
	*/
	...
	if ($_SESSION['statut'] == "eleve" OR $_SESSION['statut'] == "responsable") {
		...
		if((($_SESSION['statut'] == "eleve")&&(getSettingAOui('GeesAccesGraphRangEleve')))||(($_SESSION['statut'] == "responsable")&&(getSettingAOui('GeesAccesGraphRangParent')))) {
	...
	*/
	...
	if((($_SESSION['statut']=='responsable')&&(!getSettingAOui('GeesAccesGraphRangParent')))||
	(($_SESSION['statut']=='eleve')&&(!getSettingAOui('GeesAccesGraphRangEleve')))) {$affiche_choix_rang="n";}
		...
		if($_POST['parametrer_affichage']=='y') {
			...
			*/
			...
			if(($_SESSION['statut']=="administrateur")||
				...
				(($_SESSION['statut']=='professeur')&&(isset($id_classe))&&(is_pp($_SESSION['login'],$id_classe))&&(getSettingAOui('GeesRubConseilProf')))) {
				echo "<tr>\n";
	...
	*/
	...
	if ($_SESSION['statut'] != "responsable" and $_SESSION['statut'] != "eleve") {
		...
		/*
		if((($_SESSION['statut']!='eleve')&&($_SESSION['statut']!='responsable'))||
		(($_SESSION['statut']='responsable')&&(getSettingAOui('GeesAccesGraphRangParent')))||
		(($_SESSION['statut']='responsable')&&(getSettingAOui('GeesAccesGraphRangEleve')))) {
		*/
...
et le suivant est $eleve_suivant\">&nbsp;<img src='../images/icons/forward.png' class='icone20' alt='Suivant' />&nbsp;&nbsp;</a>";
	...
	if((($_SESSION['statut']!='eleve')&&($_SESSION['statut']!='responsable'))||
	(($_SESSION['statut']=='eleve')&&(getSettingAOui('GeesAccesGraphParamEleve')))||
	(($_SESSION['statut']=='responsable')&&(getSettingAOui('GeesAccesGraphParamParent')))) {
	...
	//if(($_SESSION['statut']=='professeur')&&(getSettingValue('GeesRubConseilProf')=="yes")) {
	if(
		(($_SESSION['statut']=='professeur')&&(getSettingValue('GeesRubConseilProf')=="yes"))||
		(($_SESSION['statut']=='scolarite')&&(getSettingValue('GeesRubConseilScol')=="yes"))||
		(($_SESSION['statut']=='cpe')&&((getSettingValue('GeesRubConseilCpe')=="yes")||(getSettingValue('GeesRubConseilCpeTous')=="yes")))
		...
		function save_avis(mode) {
			...
			<a href='../mod_discipline/saisie_avertissement_fin_periode.php?login_ele=$eleve1&amp;periode=$num_periode_choisie&amp;lien_refermer=y' target='_blank'>
				<img src='../images/icons/balance_justice.png' class='icone20' alt=\"Avertissements de fin de période\" />
							...
							if((($_SESSION['statut'] == 'professeur')&&(getSettingValue("GeesRubConseilProf")=='yes')&&(getSettingValue('CommentairesTypesPP')=='yes'))
							||(($_SESSION['statut'] == 'scolarite')&&(getSettingValue("GeesRubConseilScol")=='yes')&&(getSettingValue('CommentairesTypesScol')=='yes'))
							||(($_SESSION['statut'] == 'cpe')&&((getSettingValue("GeesRubConseilCpe")=='yes')||(getSettingValue("GeesRubConseilCpeTous")=='yes'))&&(getSettingValue('CommentairesTypesCpe')=='yes'))) {
								$texte.=div_cmnt_type();
							}
...
<input type='hidden' name='enregistrer_mention' id='enregistrer_mention' value='' />
	...
	$acces_bull_simp="n";
	if(($_SESSION['statut']=="responsable")&&(getSettingValue('GeesAccesBulletinSimpleParent')=='yes')) {
		$acces_bull_simp="y";
	}
	elseif(($_SESSION['statut']=="eleve")&&(getSettingValue('GeesAccesBulletinSimpleEleve')=='yes')) {
		$acces_bull_simp="y";
	}
	elseif($_SESSION['statut']=="professeur") {

		if(getSettingValue('GeesAccesBulletinSimplePP')=='yes') {
		...
		if(getSettingValue('GeesAccesBulletinSimpleProfToutesClasses')=='yes') {
			$acces_bull_simp="y";
		}
		elseif(getSettingValue('GeesAccesBulletinSimpleProfTousEleves')=='yes') {
		...
		elseif(getSettingValue('GeesAccesBulletinSimpleProf')=='yes') {
	...
	elseif($_SESSION['statut']=='professeur') {
		if(getSettingAOui('GeesAccesReleveProfToutesClasses')) {
			$acces_cn=true;
		}
		elseif((getSettingAOui('GeesAccesReleveProfP'))&&(is_pp($_SESSION['login'], $id_classe, $eleve1))) {
			$acces_cn=true;
		}
		elseif((getSettingAOui('GeesAccesReleveProfTousEleves'))&&(is_prof_classe_ele($_SESSION['login'], $eleve1))) {
			$acces_cn=true;
		}
		elseif((getSettingAOui('GeesAccesReleveProf'))&&(is_prof_ele($_SESSION['login'], $eleve1, "", $id_classe))) {
			$acces_cn=true;
		}
		else {
			$acces_cn=false;
...
le bulletin simplifié de la période $num_periode_choisie.\">";
			...
			if(($_SESSION['statut']=='scolarite')||
				(($_SESSION['statut']=='professeur')&&(getSettingAOui('GeesProfImprBul'))&&(is_pp($_SESSION['login'], $id_classe)))
			) {
