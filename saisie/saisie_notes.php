...
include("../bulletin/dbconnection.php");
...
if ($periode_cn != 0) {
	$login_prof = $_SESSION['login'];
        ...
        $currentClasse = $current_group["classes"]["list"][0];
        $currentCourse = $current_group["id"];
        $currentPeriode = $periode_cn;
        
        echo "classe = ".$currentClasse.' course = '.$currentCourse.' periode = '.$currentPeriode."<br/>";
...
?>
	<script type="text/javascript" language="javascript">
	<!--
	alert("Attention, les notes importées ne sont pas encore enregistrées dans la base GEES. Vous devez confirmer l'importation (bouton \"enregistrer\") !");
	//-->
	</script>
<?php
}
...
<?php
    //Célestin 26-01-2016
    //Affichage des stattistiques des notes
if($is_posted!='yes'){
    $eleve_par_sexe = nombreEleveParSexe($currentClasse);
    $nbrFilles = $eleve_par_sexe['femme'];
    $nbrGarcons = $eleve_par_sexe['homme'];
    $total = $nbrFilles + $nbrGarcons;
    ...
    if($nbrFilles!=0){
        $pourcentageFille = round(100/$nbrFilles,2);
    }
    
    if($nbrGarcons!=0){
        $pourcentageGarcon = round(100/$nbrGarcons,2);
    }
    
    if($total!=0){
        $pourcentageTotal = round(100/$total,2);
    }
    
    $note["[10-21["] = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 10, 21);
    $note['[0-5[']   = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 0, 5);
    $note["[5-7["]   = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 5, 7);
    $note["[7-9["]   = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 7, 9);
    $note["[9-10["]  = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 9, 10);
    $note["[10-12["] = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 10, 12);
    $note["[12-14["] = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 12, 14);
    $note["[14-20]"] = nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, 14, 21);
    ...
    echo "<table border='1' cellspacing='2' cellpadding='1' class='boireaus' summary='Saisie'>\n"
        ."<tr>"
        ."<td colspan='2'>Nbr d'élèves</td><td>Moy>=10</td><td>%</td><td>[0-5[</td><td>%</td><td>[5-7[</td><td>%</td><td>[7-9[</td><td>%</td><td>[9-10[</td><td>%</td><td>[10-12[</td><td>%</td><td>[12-14[</td><td>%</td><td>[14-20]</td><td>%</td>"
        ."</tr>"
        ."<tr>"
        ."<td>F</td><td>$nbrFilles</td><td>".$note['[10-21[']['femme']."</td><td>".($pourcentageFille * $note['[10-21[']['femme'])."%</td><td>".$note['[0-5[']['femme']."</td><td>".($pourcentageFille * $note['[0-5[']['femme'])."%</td><td>".$note['[5-7[']['femme']."</td><td>".($pourcentageFille * $note['[5-7[']['femme'])."%</td><td>".$note['[7-9[']['femme']."</td><td>".($pourcentageFille * $note['[7-9[']['femme'])."%</td><td>".$note['[9-10[']['femme']."</td><td>".($pourcentageFille * $note['[9-10[']['femme'])."%</td><td>".$note['[10-12[']['femme']."</td><td>".($pourcentageFille * $note['[10-12[']['femme'])."%</td><td>".$note['[12-14[']['femme']."</td><td>".($pourcentageFille * $note['[12-14[']['femme'])."%</td><td>".$note['[14-20]']['femme']."</td><td>".($pourcentageFille * $note['[14-20]']['femme'])."%</td>"
        ."</tr>"
        ."<tr>"
        ."<td>M</td><td>$nbrGarcons</td><td>".$note['[10-21[']['homme']."</td><td>".($pourcentageGarcon*$note['[10-21[']['homme'])."%</td><td>".$note['[0-5[']['homme']."</td><td>".($pourcentageGarcon*$note['[0-5[']['homme'])."%</td><td>".$note['[5-7[']['homme']."</td><td>".($pourcentageGarcon*$note['[5-7[']['homme'])."%</td><td>".$note['[7-9[']['homme']."</td><td>".($pourcentageGarcon*$note['[7-9[']['homme'])."%</td><td>".$note['[9-10[']['homme']."</td><td>".($pourcentageGarcon*$note['[9-10[']['homme'])."%</td><td>".$note['[10-12[']['homme']."</td><td>".($pourcentageGarcon*$note['[10-12[']['homme'])."%</td><td>".$note['[12-14[']['homme']."</td><td>".($pourcentageGarcon*$note['[12-14[']['homme'])."%</td><td>".$note['[14-20]']['homme']."</td><td>".($pourcentageGarcon*$note['[14-20]']['homme'])."%</td>"
        ."</tr>"
        ."<tr>"
        ."<td>T</td><td>$total</td><td>".($note['[10-21[']['homme']+$note['[10-21[']['femme'])."</td><td>".($pourcentageTotal*($note['[10-21[']['homme']+$note['[10-21[']['femme']))."%</td><td>".($note['[0-5[']['homme']+$note['[0-5[']['femme'])."</td><td>".($pourcentageTotal*($note['[0-5[']['homme']+$note['[0-5[']['femme']))."%</td><td>".($note['[5-7[']['homme']+$note['[5-7[']['femme'])."</td><td>".($pourcentageTotal*($note['[5-7[']['homme']+$note['[5-7[']['femme']))."%</td><td>".($note['[7-9[']['homme']+$note['[7-9[']['femme'])."</td><td>".($pourcentageTotal*($note['[7-9[']['homme']+$note['[7-9[']['femme']))."%</td><td>".($note['[9-10[']['homme']+$note['[9-10[']['femme'])."</td><td>".($pourcentageTotal*($note['[9-10[']['homme']+$note['[9-10[']['femme']))."%</td><td>".($note['[10-12[']['homme']+$note['[10-12[']['femme'])."</td><td>".($pourcentageTotal*($note['[10-12[']['homme']+$note['[10-12[']['femme']))."%</td><td>".($note['[12-14[']['homme']+$note['[12-14[']['femme'])."</td><td>".($pourcentageTotal*($note['[12-14[']['homme']+$note['[12-14[']['femme']))."%</td><td>".($note['[14-20]']['homme']+$note['[14-20]']['femme'])."</td><td>".($pourcentageTotal*($note['[14-20]']['homme']+$note['[14-20]']['femme']))."%</td>"
        //."<td>T</td><td>$total</td><td>".$note['[10-21[']['homme']."</td><td>%</td><td>".($note['[0-5[']['homme']+$note['[0-5[']['homme'])."</td><td>%</td><td>".$note['[5-7[']['homme']."</td><td>%</td><td>".$note['[7-9[']['homme']."</td><td>%</td><td>".$note['[9-10[']['homme']."</td><td>%</td><td>".$note['[10-12[']['homme']."</td><td>%</td><td>".$note['[12-14[']['homme']."</td><td>%</td>"
        . "<tr>"
        . "</table></div>";
}
    ...
    //Célestin
    //Calcul le nombre d'élèves par sexe
    function nombreEleveParSexe($currentClasse) {
        $result = array('femme'=>0 ,'homme'=>0);
        
        $con = new Connexion();
    
        $sql = "SELECT `sexe`, count(*) as nbr "
                . "FROM `eleves` "
                . "WHERE `login` IN (SELECT DISTINCT(`login`) FROM `j_eleves_classes` WHERE `id_classe` = :classe) "
                . "GROUP BY (sexe)";

        $stmt = $con->connection->prepare($sql);

        $stmt->bindValue(':classe', $currentClasse);  
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        
        while ($row = $stmt->fetch()){
            if($row['sexe'] == 'F'){
                $result['femme']=$row['nbr'];
            }else{
                $result['homme']=$row['nbr'];
            }
        }
    ...
    //Célestin 27-01-2016
    //Calcule du nombre de notes sur une plage pour une classe, une matière et une période
    function nombreDeNoteEntre($currentClasse, $currentCourse, $currentPeriode, $min, $max) {
        ...
        $sql = "SELECT `sexe`, count(*) as nbr  
                ...
                WHERE `periodes`.`num_periode` = :periode 
                AND `cn_cahier_notes`.`periode` = `periodes`.`num_periode`
                AND `cn_cahier_notes`.`id_groupe` = :cours
                AND `periodes`.`id_classe` = :classe
                ...
                AND `cn_notes_conteneurs`.`note`>=:min
                AND `cn_notes_conteneurs`.`note`<:max
                GROUP BY (`sexe`)";

        $stmt = $con->connection->prepare($sql);

        $stmt->bindValue(':classe', $currentClasse);  
        $stmt->bindValue(':cours', $currentCourse);  
        $stmt->bindValue(':periode', $currentPeriode);  
