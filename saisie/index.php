...
if ($current_group) {
	...
	while ($i < $nb_periode) {
		...
		if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
			($acces_exceptionnel_saisie)||
			((($current_group["classe"]["ver_periode"]["all"][$i]!=0))&&($_SESSION['statut']=='secours'))) {
			$tabdiv_infobulle[]=creer_div_infobulle("info_export_csv_periode_$i","","","<center>Export CSV des identifiants GEES, avec les colonnes Moyennes et Appréciations de cette classe, avec ligne d'entête.</center>","",15,0,"n","n","y","n");
		...
		if($temoin_ods=="y"){
			...
			while ($i < $nb_periode) {
				...
				if (($current_group["classe"]["ver_periode"]["all"][$i] >= 2)||
					...
					$tabdiv_infobulle[]=creer_div_infobulle("info_export_ods_periode_$i","","","<center>Export tableur openDocument (<i>ODS</i>) des identifiants GEES, avec les colonnes Moyennes et Appréciations de cette classe, avec ligne d'entête.</center>","",15,0,"n","n","y","n");
