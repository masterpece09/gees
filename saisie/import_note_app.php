...
if (!isset($is_posted)) {
    ...
    echo "--> <B>IDENTIFIANT</B> : L'identifiant GEES de l'élève (<b>voir les explications plus bas</b>).<br />\n";
    echo "--> <B>NOTE</B> : note entre 0 et 20 avec le point ou la virgule comme symbole décimal.<br />Autres codes possibles (sans les guillemets) : \"<b>abs</b>\" pour \"absent\", \"<b>disp</b>\" pour \"dispensé\", \"<b>-</b>\" pour absence de note.<br />Si ce champ est vide, Il n'y aura pas modification de la note déjà enregistrée dans GEES pour l'élève en question.<br />\n";
    echo "--> <B>Appréciation</B> : le texte de l'appréciation de l'élève.<br />Si ce champ est vide, Il n'y aura pas modification de l'appréciation enregistrée dans GEES pour l'élève en question.</p>\n";
    echo "<p>Pour constituer le fichier d'importation vous avez besoin de connaître l'identifiant <b>GEES</b> de chaque élève. Vous pouvez télécharger:</p>\n";
    echo "<ul>\n";
    echo "<li>le fichier élèves (identifiant GEES, sans nom et prénom) en <a href='import_class_csv.php?id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;champs=3&amp;ligne_entete=y&amp;mode=Id_Note_App'><b>cliquant ici</b></a></li>\n";
    echo "<li>ou bien le fichier élèves (nom - prénom - identifiant GEES) en <a href='import_class_csv.php?id_groupe=$id_groupe&amp;periode_num=$periode_num&amp;champs=5&amp;ligne_entete=y&amp;mode=Nom_Prenom_Id_Note_App'><b>cliquant ici</b></a><br />(<i>ce deuxième fichier n'est pas directement adapté à l'import<br />(il faudra en supprimer les colonnes Nom et Prénom avant import)</i>)</li>\n";
    echo "</ul>\n";
...
if (isset($is_posted )) {
    ...
    if($csv_file['tmp_name'] != "") {
        echo "<p><b>Attention</b>, les données ne sont pas encore enregistrées dans la base GEES. Vous devez confirmer l'importation (bouton en bas de la page) !</p>";
...
//                            echo "<INPUT TYPE=HIDDEN name='$reg_app' value = $data_app>";
            ...
            if ($row > 0) {
                if ($valid == '1') {
                    ...
                    <!--
                    alert("Attention, les données ne sont pas encore enregistrées dans la base GEES. Vous devez confirmer l'importation (bouton en bas de la page) !");
                    //-->
                    </script>
                    <?php
                } else {
                ...
                if ($non_def == 'yes') {
                    echo "<p class='bold'>Les symboles <font color=green>ND</font> signifient que le champ en question sera ignoré. Il n'y aura donc pas modification de la donnée existante dans la base de GEES.<br /></p>\n";
                }
            } else {
