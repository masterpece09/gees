...
if (($_SESSION['statut'] == 'professeur') and getSettingValue("GeesRubConseilProf")!='yes') {
	die("Droits insuffisants pour effectuer cette opération");
}

// On teste si le service scolarité peut saisir les avis
if (($_SESSION['statut'] == 'scolarite') and getSettingValue("GeesRubConseilScol")!='yes') {
	die("Droits insuffisants pour effectuer cette opération");
}

// On teste si le service cpe peut saisir les avis
if (($_SESSION['statut'] == 'cpe') and getSettingValue("GeesRubConseilCpe")!='yes' and getSettingValue("GeesRubConseilCpeTous")!='yes') {
   die("Droits insuffisants pour effectuer cette opération");
}
...
(($_SESSION['statut']=='professeur')&&(getSettingAOui('GeesAccesRestrAccesAppProfP'))&&(is_pp($_SESSION['login'], $id_classe)))) {
...
if (isset($_POST['is_posted'])) {
	...
	if (($_SESSION['statut'] == 'scolarite') or ($_SESSION['statut'] == 'secours') or (($_SESSION['statut'] == 'cpe')&&(getSettingAOui('GeesRubConseilCpeTous')))) {
		$sql="SELECT DISTINCT e.* FROM eleves e, j_eleves_classes c
		WHERE (c.id_classe='$id_classe' AND
		c.login = e.login
		) ORDER BY nom, prenom";
	}
	elseif(($_SESSION['statut'] == 'cpe')&&(getSettingAOui('GeesRubConseilCpe'))) {
		...
		if(getSettingAOui('GeesAccesPPTousElevesDeLaClasse')) {
...
((getSettingValue("GeesAccesBulletinSimpleProf")=="yes")||
(getSettingValue("GeesAccesBulletinSimpleProfToutesClasses")=="yes")||
(getSettingValue("GeesAccesBulletinSimpleProfTousEleves")=="yes")
)) {
	$acces_bull_simp='y';
}
...
elseif($_SESSION['statut']=='professeur') {
	...
	// On a filtré plus haut les profs qui n'ont pas getSettingValue("GeesRubConseilProf")=='yes'
...
if ($id_classe) {
	...
	?>
	<?php
	if (($_SESSION['statut'] == 'scolarite') or ($_SESSION['statut'] == 'secours') or (($_SESSION['statut'] == 'cpe')&&(getSettingValue("GeesRubConseilCpeTous")=='yes'))) {
		$sql="SELECT DISTINCT e.* FROM eleves e, j_eleves_classes c
		WHERE (c.id_classe='$id_classe' AND
		c.login = e.login
		) ORDER BY nom, prenom";
...
//=========================
	...
	if(getSettingAOui('GeesAccesBulletinSimpleClasseEleve')) {
	...
	if(getSettingAOui('GeesAccesBulletinSimpleClasseResp')) {
		...
		*/
		...
		while ($k < $nb_periode) {
			...
			if ($ver_periode[$k] != "N") {
				...
				if((getSettingAOui('GeesAccesBulletinSimpleParent'))||
				(getSettingAOui('GeesAccesGraphParent'))||
				(getSettingAOui('GeesAccesBulletinSimpleEleve'))||
				(getSettingAOui('GeesAccesGraphEleve'))) {
					$affiche_slash_a="n";
	...
	<a href='../mod_discipline/saisie_avertissement_fin_periode.php?login_ele=$current_eleve_login&amp;periode=$k&amp;lien_refermer=y'";
		...
		<span class='bold'>".liste_avertissements_fin_periode($current_eleve_login, $k)."</span>
					...
					if((file_exists('saisie_commentaires_types.php'))
						&&(($_SESSION['statut'] == 'professeur')&&(getSettingValue("GeesRubConseilProf")=='yes')&&(getSettingValue('CommentairesTypesPP')=='yes'))
