...
*/
		...
		for($i=0;$i<count($mail);$i++) {
			...
			elseif(!check_mail($tmp_tab['email'])) {
				...
				if(isset($date_conseil[$id_classe[$loop]])) {
					...
					$subject = "[GEES]: Convocation conseil de classe de ".$classe;
	...
	if($mail_erreur!="") {
		...
		$subject = "[GEES]: Erreur lors de l envoi des convocations aux conseils de classe";
		...
		<table class='boireaus boireaus_alt'>
			...
			<tbody>";
				...
				<tr>
					...
					//GeesAccesGestElevesProfP
					//GeesAccesGestElevesProf
					//GeesAccesGestElevesProfesseur n'existe pas mais serait testé par AccesInfoResp()
					// AccesInfoResp("GeesAccesGestEleves", $value)
					...
					// Ajouter un lien vers une page avec les infos dans le cas resp pour les PP avec GeesAccesGestElevesProfP, pour les scol,...
					if(count($dates_conseils[$id_classe[$i]])>0) {
						$cpt1++;
					}
					$cpt2++;
