...
<p>Pour obtenir un EDT plus lisible, ne pas afficher les prénoms des professeur,... il est recommandé de procéder aux rapprochements ci-dessous (<em>rapprochements entre le contenu du fichier ICAL/ICS et les matières et professeurs déclarés dans Gees</em>).<br />
...
<form enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."' id='form_envoi' method='post'>
	...
	".add_token_field();
		...
		$res=mysqli_query($GLOBALS["mysqli"], $sql);
		if(mysqli_num_rows($res)==0) {
			echo "<p>Tous les professeurs du fichier ICS fourni sont associés à un professeur dans Gees.</p>";
		}
		else {
			echo "<p>".mysqli_num_rows($res)." professeur(s) du fichier ICS fourni n'est(ne sont) pas associé(s) à un professeur dans Gees.<br />
Vous pouvez effectuer le ou les rapprochements ci-dessous&nbsp;:</p>";
			...
			if(mysqli_num_rows($res_prof)==0) {
				echo "<p>Aucun professeur n'a été trouvé dans Gees.<br />Il faut initialiser l'année d'abord.</p>";
				require("../lib/footer.inc.php");
				die();
			}
	...
	<tr>
		...
		$res=mysqli_query($GLOBALS["mysqli"], $sql);
		if(mysqli_num_rows($res)==0) {
			echo "<p>Toutes les matières du fichier ICS fourni sont associés à une matière dans Gees.</p>";
		}
		else {
			echo "<p>".mysqli_num_rows($res)." matière(s) du fichier ICS fourni n'est(ne sont) pas associé(s) à une matière dans Gees.<br />
Vous pouvez effectuer le ou les rapprochements ci-dessous&nbsp;:</p>";

			$sql="SELECT * FROM matieres ORDER BY matiere;";
			$res_mat=mysqli_query($GLOBALS["mysqli"], $sql);
			if(mysqli_num_rows($res_mat)==0) {
				echo "<p>Aucune matière n'a été trouvée dans Gees.<br />Il faut initialiser l'année d'abord.</p>";
				require("../lib/footer.inc.php");
				die();
			}
