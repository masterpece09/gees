		...
		if(!isset($step)){
			...
			if(!isset($verif_tables_non_vides)) {
				...
				if ($flag != 0){
					...
					echo "Des données concernant les matières sont actuellement présentes dans la base GEES<br /></p>\n";
			...
			echo "<p>Cette page permet d'uploader un fichier qui servira à remplir les tables de GEES avec les informations professeurs, matières,...</p>\n";
			...
			if($step==0){
				...
				if(!is_uploaded_file($xml_file['tmp_name'])) {
					...
					if(!$res_copy){
						...
						echo "<p>Dans le tableau ci-dessous, les identifiants en rouge correspondent à des nouvelles matières dans la base GEES. les identifiants en vert correspondent à des identifiants de matières détectés dans le fichier GEP mais déjà présents dans la base GEES.<br /><br />Il est possible que certaines matières ci-dessous, bien que figurant dans le fichier CSV, ne soient pas utilisées dans votre établissement cette année. C'est pourquoi il vous sera proposé en fin de procédure d'initialisation, un nettoyage de la base afin de supprimer ces données inutiles.</p>\n";
						...
						*/
						...
						if ($nb_reg_no != 0) {
							...
							echo "<p>L'importation des matières dans la base GEES a été effectuée avec succès !<br />Vous pouvez procéder à la quatrième phase d'importation des professeurs.</p>";
						}
