			...
			if($step==0) {
				...
				if(!is_uploaded_file($csv_file['tmp_name'])) {
					...
					if(!$res_copy) {
						...
						echo "<p>Veuillez maintenant compléter les correspondances utiles entre UDT et GEES&nbsp;:</p>\n";
						...
						echo "<th>Professeur dans Gees</th>\n";
						echo "<th>Info</th>\n";
						echo "</tr>\n";
						$alt=1;
						...
						echo "<p style='color:red'>Si vous ne souhaitez pas créer de groupes de VIE DE CLASSE, REMEDIATION, SOUTIEN,... parce qu'aucune note ne doit être saisie dans ces 'matières', il suffit de ne faire correspondre aucune matière GEES avec la matière UDT.</p>\n";
					}
				}
			}
			elseif($step==1) {
...
*/
	...
	// Il faudrait alors les récupérer... et ne pas traiter à nouveau les lignes correspondantes de udt_lignes quand on va les atteindre...
							...
							if($lig->regroup!="") {
								...
								$res_clas=mysqli_query($GLOBALS["mysqli"], $sql);
								if(mysqli_num_rows($res_clas)==0) {
									echo "<p style='color:red'>Aucune classe n'est associée à $lig->division dans Gees.</p>\n";
								}
								elseif(mysqli_num_rows($res_clas)>1) {
									echo "<p style='color:red'>ANOMALIE: Plus d'une classe est associée à $lig->division dans Gees.</p>\n";
								}
								else {
							...
							$res_mat=mysqli_query($GLOBALS["mysqli"], $sql);
							if(mysqli_num_rows($res_mat)==0) {
								echo "<p style='color:red'>Aucune matière n'est associée dans Gees à la matière $lig->matiere d'UDT.</p>\n";
							}
							elseif(mysqli_num_rows($res_mat)>1) {
								echo "<p style='color:red'>ANOMALIE: Plus d'une matière est associée dans Gees à la matière $lig->matiere d'UDT.</p>\n";
							}
							else {
								$lig_mat=mysqli_fetch_object($res_mat);
								$mat=$lig_mat->matiere;
