		...
		if(!isset($step)) {
			if(!isset($verif_tables_non_vides)) {
				...
				if ($flag != 0){
					...
					echo "Des données concernant les responsables sont actuellement présentes dans la base GEES<br /></p>\n";
			...
			if ($gepiSettings['unzipped_max_filesize']>=0) {
				echo "<p style=\"font-size:small; color: red;\"><em>REMARQUE&nbsp;:</em> Vous pouvez fournir à Gees le fichier compressé issu directement de SCONET (<em>Ex&nbsp;: ResponsablesAvecAdresses.zip</em>).</p>";
			}
					...
					if(!$res_copy){
						...
						*/
						...
						if ($nb_err != 0) {
							...
							echo "<p>L'importation des personnes (<em>responsables</em>) dans la base GEES a été effectuée avec succès (<em>".$stat." enregistrements au total</em>).</p>\n";
						}
