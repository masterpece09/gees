...
if (isset($is_posted) and ($is_posted == "yes")) {
		...
		if($reg_login=="") {
		    ...
		    if (($reg_etab != '')&&($reg_elenoet != '')) {
				...
				if($gepiSchoolRne!="") {
					...
					// Si le RNE de l'établissement courant (celui du GEES) n'est pas renseigné, on insère les nouveaux enregistrements, mais on ne met pas à jour au risque d'écraser un enregistrement correct avec l'info que l'élève de 1ère était en 2nde dans le même établissement.
    ...
    echo "<p>Le tableau suivant affiche les données qui vont être enregistrées dans la base de donnée GEES lorsque vous aurez confirmé ce choix tout en bas de la page.<br /><b>Tant que vous n'avez pas validé en bas de la page, aucune donnée n'est enregistrée !</b></p>\n";
    echo "<p>Les valeurs en rouge signalent d'éventuelles données manquantes (<em>ND pour \"non défini\"</em>) dans le fichier <b>ElevesSansAdresses.xml</b> fourni ! Ceci n'est pas gênant pour l'enregistrement dans la base <b>GEES</b>. Vous aurez en effet la possibilité de compléter les données manquantes avec les outils fournis dans <b>GEES</b></p>\n";
		...
		while ($i < $nb) {
			...
			*/
			...
			if (($no_gep == '') or ($nouv_login=='yes')) {
				...
				// Dans le cas où Gees est intégré à un ENT, il ne doit pas générer de login mais récupérer celui qui existe déjà
	...
	if ((getSettingValue("use_ent") == "y")&&(getSettingValue('afficher_liaison_ent')=='argos_bordeaux')) {
		...
		echo "<p>--&gt; Pour Enregistrer toutes les données dans la base <b>GEES</b>, cliquez sur le bouton \"Enregistrer\" !</p>\n";
		echo "<form enctype='multipart/form-data' action='step3.php' method='post'>\n";
