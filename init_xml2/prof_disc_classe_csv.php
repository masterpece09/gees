...
if (!isset($step1)) {
	...
	if ($test != 0) {
		...
		echo "Des données concernant l'affectation de professeurs dans des classes sont actuellement présentes dans la base GEES<br /></p>";
...
if (!isset($suite)) {
	...
	if(!isset($groupes)) {
		...
		for ($i=0;$i<count($groupes);$i++) {
			...
			if(!isset($groupes[$i]['code'])) {
				echo "<p style='color:red'>Le groupe n°$i n'a pas la forme standard dans le XML de STS.<br />Il ne peut pas être créé dans GEES.</p>\n";
			}
	...
	*/
	...
	if($temoin_div_sans_services==0) {
		echo "<p>L'importation des relations professeurs/matières et professeurs/classes dans la base GEES a été effectuée avec succès !<br />Vous pouvez procéder à l'étape suivante d'importation des options suivies par les élèves.</p>";
	}
			...
			if(!feof($fp)){
				...
				if(trim($ligne)!=""){
					...
					if ($login_prof != '') {
						...
						// On relie prof, matières et classes dans un nouveau groupe de Gees
