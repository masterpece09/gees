...
if (!isset($step1)) {
	...
	if ($flag != 0){
		...
		echo "Des données concernant les professeurs sont actuellement présentes dans la base GEES<br /></p>\n";
		...
		echo "<li>Les professeurs de l'année passée présents dans la base GEES et non présents dans le fichier XML de cette année ne sont pas effacés de la base GEES mais simplement déclarés \"inactifs\".</li>\n";
		echo "</ul>\n";
...
if (!isset($is_posted)) {
	...
	if(getSettingValue('auth_sso')=="lcs") {
		echo "<span style='color:red'>Votre Gees utilise une authentification LCS; Le format de login ci-dessous ne sera pas pris en compte. Les comptes doivent avoir été importés dans l'annuaire LDAP du LCS avant d'effectuer l'import dans GEES.</span><br />\n";
	}
	elseif($gepi_non_plugin_lcs_mais_recherche_ldap) {
		echo "<span style='color:red'>Bien que votre Gees ne soit pas en plugin du LCS, vous avez choisi d'utiliser pour vos élèves et professeurs les mêmes logins que ceux du LCS; Le format de login ci-dessous ne sera pas pris en compte. Les comptes doivent avoir été importés dans l'annuaire LDAP du LCS avant d'effectuer l'import dans GEES.</span><br />\n";
	}
	...
	echo "<p>Dans le tableau ci-dessous, les identifiants en rouge correspondent à des professeurs nouveaux dans la base GEES. les identifiants en vert correspondent à des professeurs détectés dans les fichiers CSV mais déjà présents dans la base GEES.<br /><br />Il est possible que certains professeurs ci-dessous, bien que figurant dans le fichier CSV, ne soient plus en exercice dans votre établissement cette année. C'est pourquoi il vous sera proposé en fin de procédure d'initialsation, un nettoyage de la base afin de supprimer ces données inutiles.</p>\n";
			...
			((!isset($prof[$k]["fonction"]))&&(isset($prof[$k]["nom_usage"]))&&(isset($prof[$k]["prenom"])))) {
				...
				*/
					...
					if ($exist == 'no') {
						...
						// Aucun professeur ne porte le même nom dans la base GEES. On va donc rentrer ce professeur dans la base
						...
						if($_POST['login_gen_type'] == 'ent'){
							...
							if ((getSettingValue("use_ent") == 'y')&&(getSettingValue('afficher_liaison_ent')!="netcollege")) {
								...
								// le code suivant n'est qu'une méthode proposée pour relier Gees à un ENT
								$bx = 'oui';
								...
								die('Vous n\'avez pas autorisé Gees à utiliser un ENT');
							}
						}
						else {
	...
	if ($nb_reg_no != 0) {
		...
		echo "<p>L'importation des professeurs dans la base GEES a été effectuée avec succès !</p>\n";

		echo "<p><b>* Précision sur les mots de passe (<em>en non-SSO</em>) :</b></p>\n";
		echo "<ul>
		<li>Lorsqu'un nouveau professeur est inséré dans la base GEES, son mot de passe lors de la première
		connexion à GEES est celui choisi à l'étape précédente:<br />
			<ul>
			<li>Mot de passe daprès la date de naissance au format '<em>aaaammjj</em>', ou</li>
			<li>un mot de passe génèré aléatoirement par GEES.<br />(<em>il est alors conseillé d'imprimer cette page</em>)</li>
			</ul>
		</ul>\n";
		if ($_POST['sso'] != "yes") {
