	...
	// =======================================================
		...
		if(!isset($step)) {
			...
			if ($gepiSettings['unzipped_max_filesize']>=0) {
				echo "<p style=\"font-size:small; color: red;\"><em>REMARQUE&nbsp;:</em> Vous pouvez fournir à Gees le fichier compressé issu directement de SCONET. (<em>Ex : ElevesSansAdresses.zip</em>)</p>";
			}
				...
				if(mysqli_num_rows($res_etab)==0){
					...
					while($lig=mysqli_fetch_object($res_etab)){
						...
						if(mysqli_num_rows($res_etab2)==0){
							...
							// Entrée existante
							$lig2=mysqli_fetch_object($res_etab2);
